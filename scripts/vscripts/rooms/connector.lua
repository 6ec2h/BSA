...
function Connector:Init()
	...
	CustomGameEventManager:RegisterListener("bsa_room_addr", function(_, t)
		...
		if pid ~= self.host_pid then return end
		local addr = tostring(t.address or "")
		if addr == "" or self.address then return end
		self.address = addr
	...
	CustomGameEventManager:RegisterListener("bsa_room_queue", function(_, t)
		local pid = tonumber(t and t.PlayerID)
		if pid ~= self.host_pid or self.address then return end
	...
	ListenToGameEvent("player_connect_full", function(e)
		if not e or e.PlayerID == nil or e.PlayerID < 0 then return end
		if PlayerResource:IsFakeClient(e.PlayerID) then return end
		if self.host_pid == nil then self.host_pid = e.PlayerID end
		self.last_join = Time()
		if self.published then
			...
			self:RefreshGroup()
			if self.address then self:SendConnect(e.PlayerID) end
		end
	end, nil)
	...
	Timers:CreateTimer({
		...
		useGameTime = false,
		callback = function()
			if self.fallback_done then return nil end
			if self.published then return nil end
...
function Connector:RefreshGroup()
	if self.fallback_done then return end
	...
	if csv == self.group_sids then return end
	self.group_sids = csv
...
function Connector:SendAll()
	...
	for _, pid in ipairs(pids) do
		if pid ~= self.host_pid then self:SendConnect(pid) end
	end
	if self.host_pid ~= nil and not self.sent[self.host_pid] then
		Timers:CreateTimer({
			endTime = self.HOST_DELAY,
			useGameTime = false,
			callback = function() self:SendConnect(self.host_pid) end,
		})
	end
end

function Connector:SendConnect(pid)
	if self.sent[pid] then return end
	self.sent[pid] = true
...
function Connector:Fallback(reason)
	if self.fallback_done or self.address then return end
	...
	ServerMode:LoadServerLua(5, function(ok)
		if not ok then print("[Connector] local server lua: gave up") end
	end)
end
