...
if IsDedicatedServer() then SendToServerConsole("exec bsa_server") end
if IsInToolsMode() then SendToServerConsole("exec bsa_dev") end

local function read_cfg()
	local host = Convars:GetStr("bsa_api_host") or ""
	local key  = Convars:GetStr("bsa_api_key") or ""
	if key == "" then return false end
	if host ~= "" then _G.host = host end
	_G.key = key
	return true
end
...
function ServerMode:Resolve(cb)
	...
	if forced ~= "" and forced ~= self.mode then
		if forced == "room" then read_cfg() end
		self.mode = forced
	...
	if IsInToolsMode() then return done("tools") end
	if not IsDedicatedServer() then return done("listen") end

	local tries = 0
	local function poll()
		if read_cfg() then return done("room") end
		tries = tries + 1
		if tries >= ServerMode.CFG_TRIES then return done("valve") end
...
function ServerMode:Boot(trigger)
	...
	self:Resolve(function(mode)
		if self.booted then return end
		if trigger == "init" and mode ~= "room" then return end
		self.booted = true
		if mode == "room" then
			RoomServer:Init()
		elseif mode == "listen" then
			...
			self:LoadServerLua(5, function(ok)
				if not ok then print("[ServerMode] load server lua: gave up") end
			end)
		end
	end)
end
...
function ServerMode:LoadServerLua(max_tries, cb, skip_web_init)
	...
	local function try()
		...
		attempt = attempt + 1
		local url = _G.host .. "/api_game_load_lua/?key=" .. _G.key
			.. "&t=" .. RandomInt(1, 2000000000) .. "&a=" .. attempt
		...
		req:Send(function(res)
			...
			if chunk then
				loaded = true
				chunk()
				if not skip_web_init then web:init() end
				Shop:init()
				Casino:init()
