...
for _, name in ipairs({
	"INIT", "WAIT_FOR_PLAYERS_TO_LOAD", "CUSTOM_GAME_SETUP", "PLAYER_DRAFT",
	"HERO_SELECTION", "STRATEGY_TIME", "TEAM_SHOWCASE", "WAIT_FOR_MAP_TO_LOAD",
	"PRE_GAME", "GAME_IN_PROGRESS", "POST_GAME", "DISCONNECT", "SCENARIO_SETUP",
}) do
	local v = _G["DOTA_GAMERULES_STATE_" .. name]
	if v ~= nil then STATE_NAMES[v] = name end
end

local function fmt_id(n)
	if type(n) == "number" then return string.format("%.0f", n) end
	return tostring(n)
end
...
function RoomServer:Init()
	...
	ListenToGameEvent("player_connect_full", function(e) self:OnConnectFull(e) end, nil)
	ListenToGameEvent("game_rules_state_change", function() self:OnStateChanged() end, nil)
	ListenToGameEvent("player_disconnect", function(e) self:OnDisconnect(e) end, nil)
...
function RoomServer:IsIdleState()
	local st = self:State()
	return st == DOTA_GAMERULES_STATE_INIT or st == DOTA_GAMERULES_STATE_WAIT_FOR_PLAYERS_TO_LOAD
		or st == DOTA_GAMERULES_STATE_CUSTOM_GAME_SETUP
end

function RoomServer:IsHuman(pid)
	return PlayerResource:IsValidPlayer(pid) and not PlayerResource:IsFakeClient(pid)
...
function RoomServer:Heartbeat()
	...
	if self:IsIdleState() and next(self.admitted) ~= nil then self:Publish() end
...
function RoomServer:OnHeartbeat(data)
	...
	if type(data.sids) == "table" then
		for _, s in pairs(data.sids) do sids[tostring(s)] = true end
	end
	if next(sids) ~= nil then
		if not self.gathered then self.allowed = sids end
...
function RoomServer:OnConnectFull(e)
	...
	self:Claim()
	if self.loaded then self:Heartbeat() end
	Timers:CreateTimer({
		endTime = self.HOLD_KICK,
		useGameTime = false,
		callback = function()
			if self.arrived[pid] == sid and not self.allowed[sid]
				and not self.sent_away[pid] and not self.in_queue then
...
function RoomServer:Claim()
	if self.claiming or self.gathered or not self:IsIdleState() then return end
	local sids = {}
	for pid, sid in pairs(self.arrived) do
		if self:IsHuman(pid) and not self.allowed[sid] then sids[#sids + 1] = sid end
	end
...
function RoomServer:ClaimRetry()
	if self.gathered then return end
	Timers:CreateTimer({ endTime = self.CLAIM_RETRY, useGameTime = false, callback = function() self:Claim() end })
end
...
function RoomServer:OnClaim(data)
	...
	if data.status == "keep" then
		...
		local sids = {}
		if type(data.sids) == "table" then
			for _, v in pairs(data.sids) do sids[tostring(v)] = true end
		end
		self.allowed = sids
		CustomNetTables:SetTableValue("server", "room", { state = "gathering", updated = Time() })
		for pid, sid in pairs(self.arrived) do
			if self.allowed[sid] then self:Admit(pid) end
		end
	...
	if data.status == "queued" then
		...
		CustomNetTables:SetTableValue("server", "room", {
			state = "queued", position = pos, free = tonumber(data.free) or 0, updated = Time(),
		})
		return self:ClaimRetry()
	end
...
function RoomServer:OnDisconnect(e)
	...
	if not self:IsIdleState() then return end
...
function RoomServer:Admit(pid)
	if self.admitted[pid] then return end
...
function RoomServer:Publish()
	...
	if self.starting then state = "starting" elseif self.gathered then state = "difficulty" end
	local players = {}
	for pid in pairs(self.admitted) do
		if self:IsHuman(pid) then
			players[tostring(pid)] = { name = PlayerResource:GetPlayerName(pid) or "", sid = tostring(PlayerResource:GetSteamID(pid)) }
		end
	end
	CustomNetTables:SetTableValue("server", "room", {
		state = state, present = present, expected = expected, players = players, updated = Time(),
	})
end
...
function RoomServer:CheckGather()
	if self.gathered or not self.loaded or not self:IsIdleState() then return end
	local present, expected = self:Present()
	...
	if self.first_arrival and Time() - self.first_arrival >= self.GATHER_TIMEOUT then
		print("[Room] не дождались " .. (expected - present) .. " игроков за " .. self.GATHER_TIMEOUT .. "с, стартуем")
		return self:Gather(false)
	end
...
function RoomServer:Gather(force)
	if self.gathered then return end
	if not force and not self:IsIdleState() then return end
	self.gathered = true
	local present, expected = self:Present()
	print("[Room] gathered " .. present .. "/" .. expected .. ", match " .. fmt_id(self.match_id) .. ", loading profiles")
	web:init()
	self:Publish()
	Timers:CreateTimer({
...
function RoomServer:OnDiffChosen()
	if not self.gathered or self.starting or self.diff_chosen then return end
	self.diff_chosen = true
	print("[Room] difficulty chosen, start in 3s")
	Timers:CreateTimer({ endTime = 3, useGameTime = false, callback = function() self:Start() end })
end
...
function RoomServer:Start()
	...
	local list = {}
	for pid, sid in pairs(self.arrived) do
		if self.allowed[sid] and self:IsHuman(pid) then list[#list + 1] = sid end
	end
