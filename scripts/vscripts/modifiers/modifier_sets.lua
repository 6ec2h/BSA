...
function modifier_sets:OnAbilityFullyCast( params )
	...
	if params.ability:IsRefreshable() and params.ability:GetName() ~= "hero_pangolier_blade_of_the_exile" and params.ability:IsItem() == false and RandomInt(1, 100) <= self.multicast * self.full_set then
	
		self:GetCaster():AddNewModifier(
			self:GetCaster(),
			self,
			"modifier_multicast_lua_proc",
			{
	...
	if self.desolator > 0 then
		...
		if mod_desolator then
			if mod_desolator.armor_reduction < self.desolator * self.full_set then
				params.target:AddNewModifier(self:GetParent(), self, "modifier_desolator_set", {duration = 5, armor_reduction = self.desolator * self.full_set})
			end
		else
			params.target:AddNewModifier(self:GetParent(), self, "modifier_desolator_set", {duration = 5, armor_reduction = self.desolator * self.full_set})
		end	
...
function modifier_sets:GetModifierProcAttack_BonusDamage_Magical(keys)
	if not self:GetParent():IsIllusion() and not keys.target:IsBuilding() and RandomInt(1, 100) <= 40 and self.mkb > 0  then
...
function modifier_sets:OnAttackLanded( params )
	...
	if RandomInt(1, 100) <= 10 and self.mjolnir > 0 and params.attacker == self:GetParent() and self.bChainCooldown == false and self:GetParent():IsAlive() and not self:GetParent():IsIllusion() and not params.target:IsMagicImmune() and not params.target:IsBuilding() and not params.target:IsOther() and self:GetParent():GetTeamNumber() ~= params.target:GetTeamNumber() then
		self:GetParent():EmitSound("Item.Maelstrom.Chain_Lightning")
		self:GetParent():AddNewModifier(self:GetCaster(), self, "modifier_set_mjolnir_strike", {starting_unit_entindex	= params.target:entindex(), damage = self.mjolnir * self.full_set})
...
function modifier_set_mjolnir_strike:IsHidden()		return true end
function modifier_set_mjolnir_strike:IsPurgable()		return false end
function modifier_set_mjolnir_strike:RemoveOnDeath()	return false end
function modifier_set_mjolnir_strike:GetAttributes()	return MODIFIER_ATTRIBUTE_MULTIPLE end
