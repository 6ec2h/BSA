...
function modifier_players_summary:IsHidden() return true end
function modifier_players_summary:IsPurgable()	return false end
function modifier_players_summary:RemoveOnDeath()	return false end
function modifier_players_summary:IsPermanent() return true end
...
function modifier_players_summary:OnTakeDamage(data)
	...
	if target:IsTower() or target:IsBuilding() or target:IsOther() then	return end

	if self.blacklistUnits[target:GetUnitName()] then return end
	...
	if attacker then
		if attacker:IsRealHero() then
			PlayersSummary:HandleDamage(PLAYER_SUMMARY_DAMAGE_HANDLE_TYPE.DEAL, attacker:GetPlayerOwnerID(), damage, data.damage_type, originalDamage)
		else
			...
			if attackerOwner then
				...
				if attackerOwnerHero then
					PlayersSummary:HandleDamage(PLAYER_SUMMARY_DAMAGE_HANDLE_TYPE.DEAL, attackerOwnerHero:GetPlayerOwnerID(), damage, data.damage_type, originalDamage)
				end
			end
		end
	end
	...
	if target:IsRealHero() then
		PlayersSummary:HandleDamage(PLAYER_SUMMARY_DAMAGE_HANDLE_TYPE.RECEIVE, target:GetPlayerOwnerID(), damage, data.damage_type, originalDamage)
	end
end
