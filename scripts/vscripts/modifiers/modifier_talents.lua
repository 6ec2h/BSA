...
local talent_keys = {
	"int", "spell_damage", "mana", "regen_mana", "cd", "exp",
	"str", "status", "mr", "armor", "hp", "hp_reg",
	"agi", "damage", "evasion", "attack_speed", "move_speed", "lifesteal"
}

local talent_keys_bonus = {
	"bonus_str", "bonus_int", "bonus_agi"
}

modifier_talents = class({})

function modifier_talents:IsHidden() return true end
function modifier_talents:IsPurgable() return false end
function modifier_talents:RemoveOnDeath() return false end
...
function modifier_talents:OnAttack(event)
    ...
    if not event.attacker:IsIllusion() then
		if self.values['bonus_agi_15'] and self.values['bonus_agi_15'] > 0 and RandomInt(1, 100) < 10 and not self:GetParent():HasModifier('modifier_bonus_agi_15') then
...
function modifier_talents:GetModifierSpellAmplify_Percentage()
	...
	return (table_data['spell_damage_1'] * (self.values['spell_damage_1'] or 0)) +
	       ...
	       (table_data['spell_damage_3'] * (self.values['spell_damage_3'] or 0)) + bonus
end
...
function modifier_talents:GetModifierConstantManaRegen()
	...
	return (table_data['regen_mana_1'] * (self.values['regen_mana_1'] or 0)) +
	       ...
	       (table_data['regen_mana_3'] * (self.values['regen_mana_3'] or 0)) + bonus
end
...
function modifier_talents:GetModifierMagicalResistanceBonus()
	...
	if self.values['bonus_int_2'] and self.values['bonus_int_2'] > 0 and GetManaPercentClient(self:GetCaster()) < 30 then
		bonus = bonus + 15
	end
	
	return (table_data['mr_1'] * (self.values['mr_1'] or 0)) +
	       (table_data['mr_2'] * (self.values['mr_2'] or 0)) +
	       (table_data['mr_3'] * (self.values['mr_3'] or 0)) + bonus
end
...
function modifier_talents:GetModifierPhysicalArmorBonus()
	...
	return (table_data['armor_1'] * (self.values['armor_1'] or 0)) +
	       ...
	       (table_data['armor_3'] * (self.values['armor_3'] or 0)) + bonus
end
...
function modifier_talents:GetModifierPreAttack_BonusDamage()
	...
	return (table_data['damage_1'] * (self.values['damage_1'] or 0)) +
		   ...
		   (table_data['damage_3'] * (self.values['damage_3'] or 0)) + bonus
end
...
function modifier_talents:GetModifierAttackSpeedBonus_Constant()
	...
	return (table_data['attack_speed_1'] * (self.values['attack_speed_1'] or 0)) +
	       ...
	       (table_data['attack_speed_3'] * (self.values['attack_speed_3'] or 0)) + bonus
end
...
function modifier_talents:GetModifierStatusResistanceStacking()
	...
	return (table_data['status_1'] * (self.values['status_1'] or 0)) +
	       ...
	       (table_data['status_3'] * (self.values['status_3'] or 0)) + bonus
end
...
function modifier_talents:GetModifierHealthBonus()
	...
	return (table_data['hp_1'] * (self.values['hp_1'] or 0)) +
	       ...
	       (table_data['hp_3'] * (self.values['hp_3'] or 0)) + bonus
end
...
function modifier_talents:GetModifierMoveSpeedBonus_Percentage()
	...
	if self.values['bonus_agi_9'] and self.values['bonus_agi_9'] > 0 and GetManaPercentClient(self:GetCaster()) > 50 then
		bonus = bonus + 10
	end
	
	if self.values['bonus_int_7'] and self.values['bonus_int_7'] > 0 and GetManaPercentClient(self:GetCaster()) > 50 then
		bonus = bonus + 10
	end
...
function modifier_talents:GetModifierPercentageManacost()
	...
	if self.values['bonus_int_12'] and self.values['bonus_int_12'] > 0 and GetManaPercentClient(self:GetCaster()) < 10 then
		bonus = bonus + 15
	end
...
function modifier_talents:GetModifierHealthRegenPercentage()
	...
	if self.values['bonus_str_3'] and self.values['bonus_str_3'] > 0 and not self:GetParent():HasModifier('modifier_in_fight_lua') then
		bonus = bonus + 2
	end
	return bonus
end
...
function modifier_talents:GetModifierIncomingDamage_Percentage(keys)
	...
	if self.values['bonus_int_10'] and self.values['bonus_int_10'] > 0 and GetManaPercentClient(self:GetParent()) > 90 then
		bonus = bonus + (-10)
	end

	if self.values['bonus_str_7'] and self.values['bonus_str_7'] > 0 and keys.damage > self:GetParent():GetMaxHealth() * 0.25 then
		bonus = bonus + (-15)
	end
	...
	if damage >= self:GetParent():GetHealth() and self.values['bonus_str_8'] and self.values['bonus_str_8'] > 0 and not self:GetParent():HasModifier('modifier_invu_lua') then
...
function modifier_talents:OnTakeDamage(keys)
	...
	if self:GetParent() == target then
		...
		if self:GetParent():GetHealthPercent() < 20 and self.values['bonus_str_5'] and self.values['bonus_str_5'] > 0 and not self:GetParent():HasModifier('modifier_fear_lua') then
			...
			for _, enemy in pairs(FindUnitsInRadius(self:GetCaster():GetTeamNumber(), self:GetParent():GetAbsOrigin(), nil, math.min(500 * (self:GetElapsedTime() -0.6), 500), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NONE, FIND_ANY_ORDER, false)) do
...
function modifier_talents:OnIntervalThink()
	if IsServer() then
		local allies = FindUnitsInRadius(self:GetCaster():GetTeamNumber(), self:GetCaster():GetAbsOrigin(), nil, 500, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_NONE, FIND_CLOSEST, false)
		local enemies = FindUnitsInRadius(self:GetCaster():GetTeamNumber(), self:GetCaster():GetAbsOrigin(), nil, 400, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NONE, FIND_CLOSEST, false)
