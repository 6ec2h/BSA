...
function modifier_guild:SetupTalents(guildData)
    ...
    self.attributesPerLevel = guilds:GetConfigTalentValueByLevel("attributes_per_level", guildData.talents.attributes_per_level or 0) * guildPowerMult
    self.increasedExp = guilds:GetConfigTalentValueByLevel("increased_exp", guildData.talents.increased_exp or 0) * guildPowerMult
    self.doubleBuffChance = math.floor(guilds:GetConfigTalentValueByLevel("double_buff_chance", guildData.talents.double_buff_chance or 0) * guildPowerMult)
    self.nonZeroStatsEquipmentChance = math.floor(guilds:GetConfigTalentValueByLevel("non_zero_stats_equipment_chance", guildData.talents.non_zero_stats_equipment_chance or 0) * guildPowerMult)
    self.startingAegis = math.floor(guilds:GetConfigTalentValueByLevel("starting_aegis", guildData.talents.starting_aegis or 0) * guildPowerMult)
    self.startingGold = guilds:GetConfigTalentValueByLevel("starting_gold", guildData.talents.starting_gold or 0) * guildPowerMult
end
...
function modifier_guild:GiveSpawnBonus()
	...
	if parent.__gotGuildSpawnBonus then return end
