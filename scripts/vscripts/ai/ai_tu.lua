...
AI.Init = function( thisEntity )
	...
	thisEntity:SetContextThink( "init_think", function() 
		...
		thisEntity.tiny_toss_lua = thisEntity:FindAbilityByName( "tiny_toss_lua" )
...
function HellbearThink()
	...
	if #enemies > 0 then
			...
			if thisEntity.tiny_toss_lua ~= nil and thisEntity.tiny_toss_lua:IsFullyCastable() then
					return CastToss( hAttackTarget )
			end
...
function CastToss( hEnemy )
      ExecuteOrderFromTable({
            UnitIndex = thisEntity:entindex(),
            OrderType = DOTA_UNIT_ORDER_CAST_NO_TARGET, 
            AbilityIndex = thisEntity.tiny_toss_lua:entindex(),
            Queue = false,
        })
    return 1
end
