...
function TempleGuardianThink()
	...
	local hCreatures = Entities:FindAllByClassnameWithin( "npc_dota_creature", thisEntity:GetAbsOrigin(), 3000 )
	local hGuardians = {}
	for _, hCreature in pairs( hCreatures ) do
		if ( hCreature:GetUnitName() == "npc_dota_boss_guardian" ) and hCreature:IsAlive() then
	...
	if WrathAbility ~= nil and WrathAbility:IsCooldownReady() and thisEntity:GetHealthPercent() < 50 then
