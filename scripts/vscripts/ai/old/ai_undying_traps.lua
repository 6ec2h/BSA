...
function NeutralThink()
	...
	if ability:IsItem() then
		if ability:GetName() == 'item_glimmer_cape' then
			thisEntity.self = 1
		end
    ...
    elseif bit.band(behavior, DOTA_ABILITY_BEHAVIOR_UNIT_TARGET) == DOTA_ABILITY_BEHAVIOR_UNIT_TARGET then
		...
		if ability:GetAbilityTargetTeam() == DOTA_UNIT_TARGET_TEAM_FRIENDLY then
			...
			if thisEntity.self == 1 then
				Cast(ability, thisEntity)
				thisEntity.self = 0
			end
		else
			-- Специальная логика для necrolyte_reapers_scythe
			if ability:GetName() == "necrolyte_reapers_scythe" then
				-- Ищем врагов с менее чем 60% здоровья
				local lowHealthEnemies = {}
				for _, enemy in ipairs(enemies) do
					local healthPercent = (enemy:GetHealth() / enemy:GetMaxHealth()) * 100
					if healthPercent < 65 then
						table.insert(lowHealthEnemies, enemy)
					end
				end
				
				if #lowHealthEnemies > 0 then
					local target = lowHealthEnemies[RandomInt(1, #lowHealthEnemies)]
					Cast(ability, target)
				end
			else
				-- Обычная логика для других способностей
				if target then
					Cast(ability, target)
				end	
			end
