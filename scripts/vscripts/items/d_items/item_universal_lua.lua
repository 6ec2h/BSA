...
function modifier_item_universal_lua:GetModifierBaseAttack_BonusDamage(params)
    return self.damage * (self:GetCaster():GetAgility() + self:GetCaster():GetStrength() + self:GetCaster():GetIntellect(true))
end
...
function modifier_item_universal_lua_effect:OnCreated( kv )
	...
	self.damage = self:GetAbility():GetSpecialValueFor( "damage" )
	self.ally = self.damage * (self:GetCaster():GetAgility() + self:GetCaster():GetStrength() + self:GetCaster():GetIntellect(true)) * self.aura / 100
end
