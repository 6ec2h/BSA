...
function modifier_item_cd_aura_cd:IsHidden() return true end
function modifier_item_cd_aura_cd:IsDebuff() return false end
function modifier_item_cd_aura_cd:IsPurgable() return false end
function modifier_item_cd_aura_cd:RemoveOnDeath() return false end
...
function item_cd_aura:OnSpellStart()
	if IsServer() then
		...
		self.caster,
		self,
		"modifier_item_cd_aura", 
		{duration = self.duration})
		end
		self.caster:AddNewModifier(self.caster, self, "modifier_item_cd_aura_cd", {duration = self:GetCooldown(self:GetLevel()) * self.caster:GetCooldownReduction()})
