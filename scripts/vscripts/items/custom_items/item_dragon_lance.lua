...
function item_dragon_lance_lua1:GetIntrinsicModifierName() return "modifier_item_custom_dragon_lance"  end

item_dragon_lance_lua2 = class({})
function item_dragon_lance_lua2:GetIntrinsicModifierName() return "modifier_item_custom_dragon_lance2" end

item_dragon_lance_lua3 = class({})
function item_dragon_lance_lua3:GetIntrinsicModifierName() return "modifier_item_custom_dragon_lance3" end
...
function modifier_item_custom_dragon_lance:IsHidden()   return true  end
function modifier_item_custom_dragon_lance:IsPurgable()  return false end
...
function modifier_item_custom_dragon_lance:GetModifierBonusStats_Strength() return self.bonus_str end
function modifier_item_custom_dragon_lance:GetModifierBonusStats_Agility()  return self.bonus_agi end

function modifier_item_custom_dragon_lance:GetModifierAttackRangeBonusUnique()
	if not self:GetCaster():IsRangedAttacker() then return 0 end
	-- Если надет lance2 или lance3 — их range уже считается, lance1 не добавляет
	if self:GetCaster():HasModifier("modifier_item_custom_dragon_lance2") or
	   self:GetCaster():HasModifier("modifier_item_custom_dragon_lance3") then
		return 0
	end
...
function modifier_item_custom_dragon_lance2:IsHidden()    return true              end
function modifier_item_custom_dragon_lance2:IsPurgable()  return false             end
function modifier_item_custom_dragon_lance2:GetAttributes() return MODIFIER_ATTRIBUTE_NONE end
...
function modifier_item_custom_dragon_lance2:GetModifierBonusStats_Strength()       return self.bonus_str   end
function modifier_item_custom_dragon_lance2:GetModifierBonusStats_Agility()        return self.bonus_agi   end
function modifier_item_custom_dragon_lance2:GetModifierAttackSpeedBonus_Constant() return self.bonus_as    end
...
function modifier_item_custom_dragon_lance3:GetModifierBonusStats_Strength()       return self.bonus_str end
function modifier_item_custom_dragon_lance3:GetModifierBonusStats_Agility()        return self.bonus_agi end
function modifier_item_custom_dragon_lance3:GetModifierAttackSpeedBonus_Constant() return self.bonus_as  end

function modifier_item_custom_dragon_lance3:GetModifierAttackRangeBonusUnique()
	if not self:GetCaster():IsRangedAttacker() then return 0 end
	-- Если lance2 тоже надет — range не стакается
	if self:GetCaster():HasModifier("modifier_item_custom_dragon_lance2") then return 0 end
	return self.bonus_range
end
...
function modifier_item_custom_dragon_lance3:OnAttack(keys)
	...
	if keys.target:GetTeamNumber() == parent:GetTeamNumber() then return end
	if keys.no_attack_cooldown           then return end
	if parent:HasFlyMovementCapability() then return end
	...
	for _, hEnemy in pairs(enemies) do
		if hEnemy ~= keys.target then
			...
			nTargets = nTargets + 1
			if nTargets >= maxTargets then break end
		end
	end
end
