...
function item_glaz_sudby_1:OnSpellStart()
    ...
    local enemies = FindUnitsInRadius(caster:GetTeam(), caster:GetOrigin(), nil, 700, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_BASIC, 0, 0, false)
    ...
    for _, enemy in ipairs(enemies) do
        enemy:AddNewModifier(caster, self, "modifier_glaz_sudby_effect", {duration =  self:GetSpecialValueFor("duration")})
    end
end
...
function modifier_glaz_sudby:GetModifierPreAttack_CriticalStrike(keys)
	if self:GetAbility() and (keys.target and not keys.target:IsOther() and not keys.target:IsBuilding() and keys.target:GetTeamNumber() ~= self:GetParent():GetTeamNumber()) and RandomInt(1,100) <= self.crit_chance then
...
function item_helmet_of_storm:OnSpellStart()
    ...
    -- Вихрь вокруг героя
    local enemies = FindUnitsInRadius(caster:GetTeam(), caster:GetOrigin(), nil, radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, 0, 0, false)
