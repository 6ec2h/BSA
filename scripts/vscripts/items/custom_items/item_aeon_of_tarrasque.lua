...
LinkLuaModifier("modifier_aeon_of_tarrasque_immunity", "items/custom_items/item_aeon_of_tarrasque", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_aeon_of_tarrasque_cooldown", "items/custom_items/item_aeon_of_tarrasque", LUA_MODIFIER_MOTION_NONE)
...
function modifier_aeon_of_tarrasque:GetModifierIncomingDamage_Percentage(kv)
	...
	if victim ~= self:GetParent() and self:GetParent():IsIllusion() then return end
	if self:GetAbility():IsCooldownReady() then
		...
		if (health_threshold < self.health_threshold_pct or ((self:GetParent():GetHealth() - kv.damage) / self:GetParent():GetMaxHealth()) <= self.health_threshold_pct) and bit.band(kv.damage_flags, DOTA_DAMAGE_FLAG_HPLOSS) ~= DOTA_DAMAGE_FLAG_HPLOSS then
			-- self:GetParent():Purge( false, true, false, true, true )
			self:GetParent():AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_aeon_of_tarrasque_immunity", { duration = 3.5})
			self:GetParent():SetHealth(math.min(self:GetParent():GetHealth(), self:GetParent():GetMaxHealth() * self.health_threshold_pct))
