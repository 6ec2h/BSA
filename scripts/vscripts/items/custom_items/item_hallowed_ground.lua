...
LinkLuaModifier("modifier_item_hallowed_ground_aura_emitter", "items/custom_items/item_hallowed_ground", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_item_hallowed_ground_aura", "items/custom_items/item_hallowed_ground", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_item_hallowed_ground_thinker", "items/custom_items/item_hallowed_ground", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_item_hallowed_ground_zone_aura", "items/custom_items/item_hallowed_ground", LUA_MODIFIER_MOTION_NONE)
...
function modifier_item_hallowed_ground:IsHidden() return true end
function modifier_item_hallowed_ground:IsPurgable() return false end
function modifier_item_hallowed_ground:RemoveOnDeath() return false end
function modifier_item_hallowed_ground:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE end
...
function modifier_item_hallowed_ground:GetModifierMoveSpeedBonus_Special_Boots() return self.bonus_movement end
function modifier_item_hallowed_ground:GetModifierManaBonus() return self.bonus_mana end
function modifier_item_hallowed_ground:GetModifierPhysicalArmorBonus() return self.bonus_armor end
function modifier_item_hallowed_ground:GetModifierConstantHealthRegen() return self.health_regen end
function modifier_item_hallowed_ground:GetModifierMagicalResistanceBonus() return self.magic_resistance end

modifier_item_hallowed_ground_thinker = class({})

function modifier_item_hallowed_ground_thinker:IsHidden() return true end
function modifier_item_hallowed_ground_thinker:IsPurgable() return false end
...
function modifier_item_hallowed_ground_thinker:IsAura() return true end
function modifier_item_hallowed_ground_thinker:GetAuraSearchTeam() return DOTA_UNIT_TARGET_TEAM_FRIENDLY end
function modifier_item_hallowed_ground_thinker:GetAuraSearchType() return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC end
function modifier_item_hallowed_ground_thinker:GetAuraSearchFlags() return DOTA_UNIT_TARGET_FLAG_NONE end
function modifier_item_hallowed_ground_thinker:GetAuraRadius()
    return self:GetAbility():GetSpecialValueFor("radius")
end
function modifier_item_hallowed_ground_thinker:GetModifierAura() return "modifier_item_hallowed_ground_zone_aura" end

---------------------------------------------------------------------------------------------------

modifier_item_hallowed_ground_aura_emitter = class({})
function modifier_item_hallowed_ground_aura_emitter:IsAura() return true end
function modifier_item_hallowed_ground_aura_emitter:IsHidden() return true end
function modifier_item_hallowed_ground_aura_emitter:IsDebuff() return false end
function modifier_item_hallowed_ground_aura_emitter:IsPurgable() return false end
...
function modifier_item_hallowed_ground_aura_emitter:GetAuraSearchTeam() return DOTA_UNIT_TARGET_TEAM_FRIENDLY end
function modifier_item_hallowed_ground_aura_emitter:GetAuraSearchType() return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC end
function modifier_item_hallowed_ground_aura_emitter:GetAuraSearchFlags() return DOTA_UNIT_TARGET_FLAG_NONE end
function modifier_item_hallowed_ground_aura_emitter:GetAuraRadius() return self.aura_radius or 0 end
function modifier_item_hallowed_ground_aura_emitter:GetModifierAura() return "modifier_item_hallowed_ground_aura" end

---------------------------------------------------------------------------------------------------

modifier_item_hallowed_ground_aura = class({})
function modifier_item_hallowed_ground_aura:IsHidden() return false end
function modifier_item_hallowed_ground_aura:IsPurgable() return false end
...
function modifier_item_hallowed_ground_zone_aura:IsHidden() return false end
function modifier_item_hallowed_ground_zone_aura:IsPurgable() return false end
...
function modifier_item_hallowed_ground_zone_aura:OnIntervalThink()
    local parent = self:GetParent()
    local caster = self:GetCaster()
    if not parent or not caster then return end
...
function modifier_item_hallowed_ground_zone_aura:OnTooltip()
    ...
    local thinker = self:GetAuraOwner()
    if not thinker then return self.barrier_block_total or 0 end
    local thinker_mod = thinker:FindModifierByName("modifier_item_hallowed_ground_thinker")
    if not thinker_mod or not thinker_mod._zone_barrier_remaining then return self.barrier_block_total or 0 end
...
function modifier_item_hallowed_ground_zone_aura:GetModifierIncomingSpellDamageConstant(keys)
    ...
    if keys.damage_type ~= DAMAGE_TYPE_MAGICAL then return 0 end
    local thinker = self:GetAuraOwner()
    if not thinker then return 0 end
    local thinker_mod = thinker:FindModifierByName("modifier_item_hallowed_ground_thinker")
    if not thinker_mod or not thinker_mod._zone_barrier_remaining then return 0 end
