...
LinkLuaModifier("modifier_item_mana_burners_sting",       "items/custom_items/item_mana_burners_sting.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_item_mana_burners_sting_debuff", "items/custom_items/item_mana_burners_sting.lua", LUA_MODIFIER_MOTION_NONE)
...
function modifier_item_mana_burners_sting:IsHidden()   return true  end
function modifier_item_mana_burners_sting:IsPurgable() return false end
function modifier_item_mana_burners_sting:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE end
...
function modifier_item_mana_burners_sting:GetModifierBonusStats_Agility()    return self.bonus_agility     end
function modifier_item_mana_burners_sting:GetModifierBonusStats_Intellect()  return self.bonus_intellect   end
function modifier_item_mana_burners_sting:GetModifierAttackSpeedBonus_Constant() return self.bonus_attack_speed end
function modifier_item_mana_burners_sting:GetModifierPreAttack_BonusDamage() return self.bonus_damage      end
function modifier_item_mana_burners_sting:GetModifierConstantManaRegen()     return self.bonus_mana_regen  end
function modifier_item_mana_burners_sting:GetModifierHealthRegenConstant()   return self.bonus_health_regen end

---------------------------------------------------------------------------------------------------

modifier_item_mana_burners_sting_debuff = modifier_item_mana_burners_sting_debuff or class({})

function modifier_item_mana_burners_sting_debuff:IsHidden()   return false end
function modifier_item_mana_burners_sting_debuff:IsPurgable() return true  end
function modifier_item_mana_burners_sting_debuff:IsDebuff()   return true  end
...
function modifier_item_mana_burners_sting_debuff:OnDestroy()
	...
	local orchid_end_pfx = ParticleManager:CreateParticle("particles/items2_fx/orchid_pop.vpcf", PATTACH_OVERHEAD_FOLLOW, parent)
...
function modifier_item_mana_burners_sting_debuff:OnAttackLanded(keys)
	...
	local ability = self:GetAbility()
	if not ability then return end
	if keys.attacker ~= ability:GetCaster() then return end

	local burned = math.min(parent:GetMana(), self.mana_burn or 0)
	if burned <= 0 then return end
