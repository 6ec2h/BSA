...
function modifier_sabre_blade:OnAttack(event)
    ...
    if not event.attacker:IsIllusion() then
        ...
        event.attacker:AddNewModifier(attacker, self:GetAbility(), "modifier_sabre_blade_doubleattack", {enemyEntIndex = event.target:GetEntityIndex(), duration = 0.3})
...
function modifier_sabre_blade_doubleattack:OnAttack(keys)
	...
	ApplyDamage({
		...
		damage_flags = DOTA_DAMAGE_FLAG_NO_SPELL_LIFESTEAL + DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION + DOTA_DAMAGE_FLAG_DONT_DISPLAY_DAMAGE_IF_SOURCE_HIDDEN,
	})
end
