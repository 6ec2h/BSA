...
function modifier_item_cosmic_focus:OnTakeDamage(keys)
    ...
    if not keys or not keys.unit or not keys.attacker then return end

    local victim = keys.unit
    if victim:IsAlive() then return end

    local parent = self:GetParent()
    if not IsMyKilledBadGuys2(parent, keys) then return end

    if victim._cosmic_focus_awarded then return end
    victim._cosmic_focus_awarded = true

    local primary = GetPrimaryCosmicFocus(parent)
    if self:GetAbility() ~= primary then return end
