LinkLuaModifier("modifier_item_trident_of_eternity", "items/custom_items/item_trident_of_eternity", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_item_trident_of_eternity_stacks", "items/custom_items/item_trident_of_eternity", LUA_MODIFIER_MOTION_NONE)
...
local function _has_higher_trident(unit, level)
	...
	for lvl = level + 1, 3 do
		for i = 0, 5 do
			...
			if item and item:GetName() == "item_trident_of_eternity_" .. lvl then return true end
		end
	end
	return false
end
...
function modifier_item_trident_of_eternity:GetModifierHealthBonus() return self.bonus_health end
function modifier_item_trident_of_eternity:GetModifierManaBonus() return self.bonus_mana end
function modifier_item_trident_of_eternity:GetModifierConstantManaRegen() return self.bonus_mana_regen end
function modifier_item_trident_of_eternity:GetModifierCastRangeBonus() return self.cast_range_bonus end
...
function modifier_item_trident_of_eternity:GetModifierBonusStats_Strength() return self.bonus_strength end
function modifier_item_trident_of_eternity:GetModifierBonusStats_Intellect() return self.bonus_intellect end
function modifier_item_trident_of_eternity:GetModifierBonusStats_Agility() return self.bonus_agility end
function modifier_item_trident_of_eternity:GetModifierAttackSpeedBonus_Constant() return self.bonus_attack_speed end
function modifier_item_trident_of_eternity:GetModifierStatusResistanceStacking() return self.status_resistance end
function modifier_item_trident_of_eternity:GetModifierMoveSpeedBonus_Percentage() return self.movement_speed_percent_bonus end
...
function modifier_item_trident_of_eternity:OnAbilityExecuted(keys)
    if not IsServer() then return end
    if not keys or keys.unit ~= self:GetParent() then return end
    ...
    if not stacks_mod then
        stacks_mod = parent:AddNewModifier(parent, self:GetAbility(), "modifier_item_trident_of_eternity_stacks", {duration = 5})
    end
    if not stacks_mod then return end
...
function modifier_item_trident_of_eternity_stacks:IsHidden() return false end
function modifier_item_trident_of_eternity_stacks:IsPurgable() return false end
function modifier_item_trident_of_eternity_stacks:RemoveOnDeath() return false end
