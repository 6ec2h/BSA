...
function nyx_room_trap(trigger)
	...
	if npc ~= nil then
		...
		npc:CastAbilityOnPosition(target:GetOrigin(), ability, -1 )
		DoEntFire( model, "SetAnimation", "fang_attack", .1, self, self )
	end
...
function fire_room_shot(trigger)
	...
	if npc ~= nil then
		...
		npc:CastAbilityOnPosition(target:GetOrigin(), ability, -1 )
		DoEntFire( model, "SetAnimation", "bark_attack", .1, self, self )
	end
...
function EnablePendulum( triggerName )
	if isPendulumReady == true then
		...
		npc:SetContextThink( "DisableTrap", function() return DisablePendulum( triggerName ) end, 5 )
	end
	return -1
end
