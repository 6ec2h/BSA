auto_shot_zone_2_kill_arrow = class({})

function auto_shot_zone_2_kill_arrow:OnSpellStart()
...
function auto_shot_zone_2_kill_arrow:OnProjectileHit( hTarget, vLocation )
	if hTarget ~= nil and ( not hTarget:IsMagicImmune() ) and ( not hTarget:IsInvulnerable() ) then
		local damage = {
			victim = hTarget,
			attacker = self:GetCaster(),
			damage = hTarget:GetMaxHealth()*0.7,
