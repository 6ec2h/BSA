...
function sniper_shrapnel_lua:OnSpellStart()
	...
	CreateModifierThinker(caster, self, "modifier_sniper_shrapnel_lua_thinker", {}, point, caster:GetTeamNumber(), false)
	self:PlayEffects( point )
end

function sniper_shrapnel_lua:PlayEffects( point )
	local effect_cast = ParticleManager:CreateParticle( "particles/units/heroes/hero_sniper/sniper_shrapnel_launch.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetCaster() )
...
function modifier_sniper_headshot_lua:GetModifierProcAttack_BonusDamage_Physical( params )
	if IsServer() then
		if RandomInt(1,100) <= self.proc_chance then
			params.target:AddNewModifier(self:GetParent(), self, "modifier_sniper_headshot_lua_slow", {duration = self.slow_duration, slow = self.slow})
			return self.damage
		end
	end
end
...
function modifier_sniper_headshot_lua:OnTakeDamage(keys)
	...
	if talent ~= nil and talent:GetLevel() > 0 and RandomInt(1,100) <= 50 then 
		if keys.attacker == self:GetParent() and not self:GetParent():PassivesDisabled() and keys.damage_category == DOTA_DAMAGE_CATEGORY_ATTACK and not keys.unit:IsBuilding() and (not keys.unit:IsOther() or (keys.unit:IsOther() and keys.damage > 0)) then
			for _, enemy in pairs(FindUnitsInLine(self:GetCaster():GetTeamNumber(), keys.unit:GetAbsOrigin(), keys.unit:GetAbsOrigin() + ((keys.unit:GetAbsOrigin() - self:GetParent():GetAbsOrigin()):Normalized() * 300), nil, 90, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES)) do
