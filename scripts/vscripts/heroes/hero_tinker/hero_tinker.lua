...
function tinker_laser_lua:OnSpellStart()
	...
	local damage = self:GetSpecialValueFor("laser_damage") + self:GetCaster():ExtraIntelligenceDamage() * self:GetSpecialValueFor("ExtraIntelligenceDamage") 
...
function tinker_rearm_lua:OnChannelFinish( bInterrupted )
	...
	for i=0,caster:GetAbilityCount()-1 do
		...
		if ability and ability:GetAbilityType()~=ABILITY_TYPE_ATTRIBUTES and ability ~= self and not self:IsItemException(ability) then
