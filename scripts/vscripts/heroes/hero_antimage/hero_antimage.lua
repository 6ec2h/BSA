...
function antimage_blink_lua:OnSpellStart()
    ...
    local illusions = CreateIllusions(caster, caster, {outgoing_damage = 0, duration = duration}, count, caster:GetHullRadius(), true, true)
    if illusions then
...
LinkLuaModifier( "modifier_antimage_spell_shield_lua_effect", "heroes/hero_antimage/hero_antimage", LUA_MODIFIER_MOTION_NONE )
...
function modifier_antimage_spell_shield_lua:IsHidden() return true end
function modifier_antimage_spell_shield_lua:IsPurgable() return false end
...
function modifier_antimage_spell_shield_lua_effect:IsHidden() return false end
function modifier_antimage_spell_shield_lua_effect:IsDebuff() return true end
function modifier_antimage_spell_shield_lua_effect:IsPurgable() return false end
...
function antimage_mana_time_lua:PlayEffects( target, radius )
    local effect_target = ParticleManager:CreateParticle( "particles/units/heroes/hero_antimage/antimage_manavoid.vpcf", PATTACH_POINT_FOLLOW, target )
...
function modifier_sleep_time:IsHidden() return false end
function modifier_sleep_time:IsDebuff() return true end
function modifier_sleep_time:IsPurgable() return false end
function modifier_sleep_time:GetAttributes() return MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE end
...
function modifier_sleep_time:GetOverrideAnimation() return ACT_DOTA_DISABLED end
function modifier_sleep_time:GetOverrideAnimationRate() return 0.55 end
