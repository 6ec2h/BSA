...
LinkLuaModifier("modifier_juggernaut_blade_fury_lua", "heroes/hero_juggernaut/hero_juggernaut", LUA_MODIFIER_MOTION_NONE)
...
function modifier_juggernaut_blade_fury_lua:IsHidden() return false end
function modifier_juggernaut_blade_fury_lua:IsPurgable() return false end
...
function modifier_juggernaut_blade_fury_lua:PlayEffects()
    local parent = self:GetParent()
    local effect_cast = ParticleManager:CreateParticle("particles/units/heroes/hero_juggernaut/juggernaut_blade_fury.vpcf", PATTACH_ABSORIGIN_FOLLOW, parent)
...
function modifier_juggernaut_blade_fury_lua:PlayEffects2(target)
    local effect_cast = ParticleManager:CreateParticle("particles/units/heroes/hero_juggernaut/juggernaut_blade_fury_tgt.vpcf", PATTACH_ABSORIGIN_FOLLOW, target)
...
LinkLuaModifier("modifier_juggernaut_healing_ward_thinker_lua", "heroes/hero_juggernaut/hero_juggernaut", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_juggernaut_healing_ward_aura_buff_lua", "heroes/hero_juggernaut/hero_juggernaut", LUA_MODIFIER_MOTION_NONE)
...
function juggernaut_heling_ward_lua:OnSpellStart()
    ...
    local ward = CreateUnitByName("npc_dota_juggernaut_healing_ward", caster:GetAbsOrigin(), false, caster, caster, caster:GetTeamNumber())
...
LinkLuaModifier("modifier_juggernaut_blade_dance_lua", "heroes/hero_juggernaut/hero_juggernaut", LUA_MODIFIER_MOTION_NONE)
...
function modifier_juggernaut_blade_dance_lua:GetModifierPreAttack_CriticalStrike(params)
    if not IsServer() or self:GetParent():PassivesDisabled() then return end
...
LinkLuaModifier("modifier_juggernaut_omni_slash_lua_caster", "heroes/hero_juggernaut/hero_juggernaut", LUA_MODIFIER_MOTION_NONE)
...
function juggernaut_omni_slash_lua:IsHiddenWhenStolen() return false end
...
function juggernaut_omni_slash_lua:OnSpellStart()
    ...
    caster:Purge(false, true, false, false, false)
    local modifier = caster:AddNewModifier(caster, self, "modifier_juggernaut_omni_slash_lua_caster", {duration = duration})
    if modifier then modifier.original_caster = caster end
...
function modifier_juggernaut_omni_slash_lua_caster:Bounce(bFirst)
    ...
    if target then
        ...
        local hPfx = ParticleManager:CreateParticle("particles/units/heroes/hero_juggernaut/juggernaut_omni_slash_tgt.vpcf", PATTACH_ABSORIGIN_FOLLOW, target)
        ParticleManager:SetParticleControl(hPfx, 0, curr_pos)
        ParticleManager:ReleaseParticleIndex(hPfx)

        local tPfx = ParticleManager:CreateParticle("particles/units/heroes/hero_juggernaut/juggernaut_omni_slash_trail.vpcf", PATTACH_ABSORIGIN, self.parent)
