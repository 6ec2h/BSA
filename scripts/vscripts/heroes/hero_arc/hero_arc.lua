...
function modifier_arc_flux_lua_debuff:GetModifierMoveSpeedBonus_Constant() return -self.movespeed end
function modifier_arc_flux_lua_debuff:GetModifierAttackSpeedBonus_Constant() return -self.attackspeed end
...
function arc_warden_magnetic_field_lua:OnSpellStart()
    ...
    local cast_particle = ParticleManager:CreateParticle("particles/units/heroes/hero_arc_warden/arc_warden_magnetic_cast.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster)
...
function modifier_arc_warden_magnetic_field_lua_thinker:OnCreated()
    ...
    local particle = ParticleManager:CreateParticle("particles/units/heroes/hero_arc_warden/arc_warden_magnetic.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
...
function modifier_arc_warden_magnetic_field_lua_thinker:IsAura() return true end
function modifier_arc_warden_magnetic_field_lua_thinker:GetAuraDuration() return 0.1 end
function modifier_arc_warden_magnetic_field_lua_thinker:GetAuraRadius() return self.radius end
function modifier_arc_warden_magnetic_field_lua_thinker:GetAuraSearchTeam() return DOTA_UNIT_TARGET_TEAM_FRIENDLY end
function modifier_arc_warden_magnetic_field_lua_thinker:GetAuraSearchType() return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BUILDING end
function modifier_arc_warden_magnetic_field_lua_thinker:GetModifierAura() return "modifier_arc_warden_magnetic_field_lua_buff" end
...
function modifier_arc_warden_magnetic_field_lua_buff:GetModifierEvasion_Constant(keys)
    if not IsServer() then return self.evasion end
...
function modifier_ark_spark_lua_thinker:OnIntervalThink()
    if self.is_flying then return end
...
function modifier_arc_geminate_attack:OnAttack(keys)
    ...
    if keys.attacker == parent and 
       ability:IsFullyCastable() and 
       not parent:IsIllusion() and 
       not parent:PassivesDisabled() and 
       not keys.no_attack_cooldown and 
       not (target:IsWard() or target:IsBuilding()) then
