...
LinkLuaModifier('modifier_brewmaster_cinder_brew_lua_debuff', "heroes/hero_brewmaster/brewmaster_cinder_brew_lua", LUA_MODIFIER_MOTION_NONE)
...
function brewmaster_cinder_brew_lua:OnProjectileHit(hTarget, vLocation)
    ...
    local units = FindUnitsInRadius(self:GetCaster():GetTeam(), vLocation, nil, self:GetSpecialValueFor('radius'), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, self:GetAbilityTargetFlags(), FIND_ANY_ORDER, false)
    ...
    for k,v in pairs(units) do 
        v:AddNewModifier(self:GetCaster(), self, 'modifier_brewmaster_cinder_brew_lua_debuff', {
            duration = duration,
