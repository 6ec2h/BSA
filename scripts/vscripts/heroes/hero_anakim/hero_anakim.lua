    ...
    if modifier and #modifier.spirits > 0 then
        ...
        for _, spirit in pairs(spirits_to_send) do
            if spirit and not spirit:IsNull() then
                spirit:AddNewModifier(caster, self, "modifier_anakim_wisp_projectile", { target_entindex = target:entindex() })
            end
        end
...
function modifier_anakim_wisp:IsHidden() return false end
function modifier_anakim_wisp:IsPurgable() return false end
function modifier_anakim_wisp:RemoveOnDeath() return false end
...
function modifier_anakim_wisp:OnDeath( params )
    ...
    if params.unit == parent then 
        ...
        for _, spirit in pairs(self.spirits) do
            if spirit and not spirit:IsNull() then UTIL_Remove(spirit) end
        end
        self.spirits = {}
        return
    end

    if parent:PassivesDisabled() or params.unit:IsIllusion() then return end
    if not params.unit:FindModifierByNameAndCaster( "modifier_anakim_wisp_debuff", parent ) then return end
    
    if not _G.excludedUnitsLookup[params.unit:GetUnitName()] or parent:HasModifier("modifier_guild_event") then return end
...
function modifier_anakim_wisp:CreateNewSpirit()
    ...
    local newSpirit = CreateUnitByName("npc_spitit_wisp", caster:GetAbsOrigin(), false, caster, caster, caster:GetTeam())
...
function modifier_anakim_wisp:IsAura() return (not self:GetCaster():PassivesDisabled()) end
function modifier_anakim_wisp:GetModifierAura() return "modifier_anakim_wisp_debuff" end
function modifier_anakim_wisp:GetAuraRadius() return 800 end
function modifier_anakim_wisp:GetAuraDuration() return 0.5 end
function modifier_anakim_wisp:GetAuraSearchTeam() return DOTA_UNIT_TARGET_TEAM_ENEMY end
function modifier_anakim_wisp:GetAuraSearchType() return DOTA_UNIT_TARGET_ALL end
function modifier_anakim_wisp:GetAuraSearchFlags() return DOTA_UNIT_TARGET_FLAG_INVULNERABLE end
function modifier_anakim_wisp:IsAuraActiveOnDeath() return false end
...
function modifier_anakim_wisp_debuff:IsHidden() return true end
function modifier_anakim_wisp_debuff:IsPurgable() return false end
...
function modifier_anakim_wisp_projectile:IsHidden() return true end
...
function modifier_anakim_final_sacrifice:IsHidden() return false end
function modifier_anakim_final_sacrifice:IsPurgable() return false end
...
function modifier_anakim_final_sacrifice:OnTakeDamage(params)
    ...
    if parent:GetHealth() <= 1 then
        ...
        local hero_pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_omniknight/omniknight_guardian_angel_ally.vpcf", PATTACH_ABSORIGIN_FOLLOW, parent)
...
function anakim_pulse_line:OnAbilityPhaseStart()
    local caster = self:GetCaster()
    self.cast_particle = ParticleManager:CreateParticle("particles/econ/items/elder_titan/elder_titan_ti7/elder_titan_echo_stomp_cast_combined_detail_ti7.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster)
...
function anakim_pulse_line:OnSpellStart()
    ...
    local effect_cast = ParticleManager:CreateParticle("particles/units/heroes/hero_phoenix/phoenix_sunray.vpcf", PATTACH_WORLDORIGIN, nil)
