...
function modifier_konan_paper_dance:IsHidden()    return true  end
function modifier_konan_paper_dance:IsPurgable()  return false end
function modifier_konan_paper_dance:IsPermanent() return true  end
...
function modifier_konan_paper_dance:OnDeath(params)
    ...
    if unit:IsRealHero() and unit ~= caster and unit:GetTeamNumber() == caster:GetTeamNumber() then
        ...
        if not RollPercentage(self._revive_chance) then return end
    ...
    if not IsMyKilledBadGuys2(caster, params) then return end
