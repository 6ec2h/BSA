...
function modifier_deidara_unstable_art:IsHidden()    return true  end
function modifier_deidara_unstable_art:IsPurgable()  return false end
function modifier_deidara_unstable_art:IsPermanent() return true  end
...
function modifier_deidara_unstable_art:OnAttackLanded(data)
    ...
    if not data.target or data.target:IsNull() then return end
    ...
    local nearby = FindUnitsInRadius(
        caster:GetTeamNumber(), data.target:GetAbsOrigin(), nil,
        self._splash_radius,
...
function modifier_deidara_unstable_art:OnDeath(params)
    if not IsServer() then return end
    if not IsMyKilledBadGuys2(self:GetParent(), params) then return end
