...
function modifier_pain_six_paths_authority:IsHidden()    return true  end
function modifier_pain_six_paths_authority:IsPurgable()  return false end
function modifier_pain_six_paths_authority:IsPermanent() return true  end
...
function modifier_pain_six_paths_authority:OnDeath(data)
    ...
    local fly_pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_invoker/invoker_chaos_meteor_fly.vpcf", PATTACH_WORLDORIGIN, nil)
    ...
    Timers:CreateTimer(DELAY, function()
        local crash_pfx = ParticleManager:CreateParticle(
            "particles/units/heroes/hero_ember_spirit/ember_spirit_hit.vpcf", PATTACH_WORLDORIGIN, nil)
...
function modifier_pain_six_paths_authority:OnIntervalThink()
    ...
    if ab:GetLevel() ~= new_level then ab:SetLevel(new_level) end

    local now        = GameRules:GetGameTime()
    local caster_pos = caster:GetAbsOrigin()

    local enemies = FindUnitsInRadius(
        caster:GetTeamNumber(), caster_pos, nil,
        self._aura_radius,
        DOTA_UNIT_TARGET_TEAM_ENEMY,
        DOTA_UNIT_TARGET_BASIC + DOTA_UNIT_TARGET_HERO,
        DOTA_UNIT_TARGET_FLAG_NONE,
...
function modifier_pain_aura_slow:IsHidden()   return false end
function modifier_pain_aura_slow:IsPurgable() return true  end
...
function modifier_pain_full_slow:IsHidden()   return true  end
function modifier_pain_full_slow:IsPurgable() return false end
