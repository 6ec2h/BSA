...
function modifier_kakuzu_earth_grudge_fear:OnIntervalThink()
    ...
    if ability:GetLevel() ~= new_level then ability:SetLevel(new_level) end
end
...
function modifier_kakuzu_earth_grudge_fear:OnDeath(params)
    ...
    if not IsMyKilledBadGuys2(hero, params) then return end
    ...
    if counter_mod then counter_mod:SetStackCount(self.kill_counter) end
end

function modifier_kakuzu_earth_grudge_fear:OnTakeDamage(data)
    if not IsServer() then return end
    local caster = self:GetParent()
    if data.unit ~= caster then return end
...
function modifier_kakuzu_earth_grudge_fear:ExplodeHeart()
    ...
    local enemies = FindUnitsInRadius(caster:GetTeamNumber(), caster:GetAbsOrigin(), nil,
        radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_BASIC + DOTA_UNIT_TARGET_HERO,
...
function modifier_kakuzu_kill_counter:IsHidden()    return false end
function modifier_kakuzu_kill_counter:IsPurgable()  return false end
function modifier_kakuzu_kill_counter:IsPermanent() return true  end
function modifier_kakuzu_kill_counter:RemoveOnDeath() return false end

---------------------------------------------------------------------------

function IsMyKilledBadGuys2(hero, params)
    if params.unit:GetTeamNumber() ~= DOTA_TEAM_NEUTRALS then return false end
    local attacker = params.attacker
    if hero ~= attacker or attacker:HasModifier('modifier_guild_event') then return false end
    if not _G.excludedUnitsLookup or not _G.excludedUnitsLookup[params.unit:GetUnitName()] then return false end
    return true
end
