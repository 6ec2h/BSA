...
function modifier_tobi_phantom_strike:OnAttackLanded(data)
    ...
    if data.attacker ~= caster then return end
    local target = data.target
    if not target or target:IsNull() or not target:IsAlive() then return end

    if not RollPercentage(self:_GetChance()) then return end
    ...
    target:AddNewModifier(caster, ability, 'modifier_tobi_turn_rate_slow',
