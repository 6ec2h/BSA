...
function modifier_itachi_crows:IsHidden()    return true  end
function modifier_itachi_crows:IsPurgable()  return false end
function modifier_itachi_crows:IsPermanent() return true  end
...
function modifier_itachi_crows:OnIntervalThink()
    ...
    if not caster:IsAlive() then
        if self._spawned then self:_DestroyCrows() end
        return
    end
    ...
    if self._last_caster_pos and (caster_pos - self._last_caster_pos):Length2D() > TELEPORT_DIST then
        for _, cd in ipairs(self._crows) do
            if cd.unit and not cd.unit:IsNull() then
                ...
                local near  = Vector(caster_pos.x + r * math.cos(angle), caster_pos.y + r * math.sin(angle), caster_pos.z)
    ...
    for _, cd in ipairs(self._crows) do
        local crow = cd.unit
        if not crow or crow:IsNull() then goto continue end
