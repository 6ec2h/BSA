...
function modifier_zetsu_root_assimilation:IsHidden()    return true  end
function modifier_zetsu_root_assimilation:IsPurgable()  return false end
function modifier_zetsu_root_assimilation:IsPermanent() return true  end
...
function modifier_zetsu_root_assimilation:_GetOwner()
    local parent = self:GetParent()
    if parent:IsRealHero() then return parent end
    local owner = parent:GetOwner()
    if owner then return owner:GetAssignedHero() end
    return nil
end
...
function modifier_zetsu_root_assimilation:OnAttackLanded(data)
    if not IsServer() then return end
    local parent = self:GetParent()
    if data.attacker ~= parent then return end
    ...
    if #tbl >= self._max_illusions then return end
    ...
    for _, ill in pairs(illusions) do
        ...
        if skin_mod then
            ill:AddNewModifier(owner, nil, 'modifier_skin_model_override', { model = skin_mod._model, scale = skin_mod._scale })
        end
