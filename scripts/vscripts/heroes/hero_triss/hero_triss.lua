...
function triss_shotgun:OnProjectileHit_ExtraData( target, location, extraData )
	...
	target:AddNewModifier( self:GetCaster(), self, "modifier_triss_shotgun", { duration = self:GetSpecialValueFor( "duration" ) } )
...
function modifier_triss_shotgun:IsHidden() return false end
function modifier_triss_shotgun:IsDebuff() return true end
function modifier_triss_shotgun:IsPurgable() return true end
...
function modifier_triss_turret:IsHidden() return true end
function modifier_triss_turret:IsPurgable() return false end
...
function modifier_triss_disguise:OnDestroy( kv )
	...
	if talent and talent:GetLevel() > 0 then
		self:GetCaster():AddNewModifier(self:GetCaster(), nil, "modifier_modifier_triss_disguise_immune", { duration = 3 })
	end
end
...
function modifier_modifier_triss_disguise_immune:IsHidden() return true end
function modifier_modifier_triss_disguise_immune:IsPurgable() return false end
...
function triss_granade:OnSpellStart()
	...
	self.projectile_table.vVelocity		= (self:GetCursorPosition() - self:GetCaster():GetAbsOrigin()):Normalized() * 600 * Vector(1, 1, 0)
...
function triss_granade:OnProjectileHit_ExtraData(target, location, data)	
	...
	for _, enemy in pairs(enemies) do
		...
		enemy:AddNewModifier(self:GetCaster(), self, "modifier_triss_granade", {duration=self:GetSpecialValueFor("duration")})
	end
end
...
function modifier_triss_granade:IsHidden() return false end
function modifier_triss_granade:IsPurgable() return true end
function modifier_triss_granade:IsDebuff() return true end
