LinkLuaModifier("modifier_time_walk_lua_damage_counter", "heroes/hero_faceless_void/hero_faceless_void", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_time_walk_lua_buff_as", "heroes/hero_faceless_void/hero_faceless_void", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_time_walk_lua_buff_ms", "heroes/hero_faceless_void/hero_faceless_void", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_time_walk_lua_cast", "heroes/hero_faceless_void/hero_faceless_void", LUA_MODIFIER_MOTION_HORIZONTAL)
LinkLuaModifier("modifier_time_walk_lua_slow", "heroes/hero_faceless_void/hero_faceless_void", LUA_MODIFIER_MOTION_NONE)

time_walk_lua = class({})

function time_walk_lua:IsHiddenWhenStolen() return false end
function time_walk_lua:IsNetherWardStealable() return false end
...
function time_walk_lua:OnSpellStart()
    ...
    caster:AddNewModifier(caster, self, "modifier_time_walk_lua_cast", {
        duration = math.min((position - caster:GetAbsOrigin()):Length2D(), max_cast_range) / self:GetSpecialValueFor("speed") + 0.5,
        x = position.x,
        y = position.y,
        z = position.z
    })
...
function modifier_time_walk_lua_damage_counter:IsPurgable() return false end
function modifier_time_walk_lua_damage_counter:IsDebuff() return false end
function modifier_time_walk_lua_damage_counter:IsHidden() return true end
function modifier_time_walk_lua_damage_counter:DeclareFunctions() return { MODIFIER_EVENT_ON_TAKEDAMAGE } end
...
function modifier_time_walk_lua_buff_as:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE end
function modifier_time_walk_lua_buff_as:IsPurgable() return false end
function modifier_time_walk_lua_buff_as:IsDebuff() return false end
function modifier_time_walk_lua_buff_as:IsHidden() return true end
function modifier_time_walk_lua_buff_as:DeclareFunctions() return { MODIFIER_PROPERTY_ATTACKSPEED_BONUS_CONSTANT } end
function modifier_time_walk_lua_buff_as:GetModifierAttackSpeedBonus_Constant() return self:GetStackCount() end

modifier_time_walk_lua_buff_ms = class({})
function modifier_time_walk_lua_buff_ms:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE end
function modifier_time_walk_lua_buff_ms:IsPurgable() return false end
function modifier_time_walk_lua_buff_ms:IsDebuff() return false end
function modifier_time_walk_lua_buff_ms:IsHidden() return true end
function modifier_time_walk_lua_buff_ms:DeclareFunctions() return { MODIFIER_PROPERTY_MOVESPEED_BONUS_CONSTANT } end
function modifier_time_walk_lua_buff_ms:GetModifierMoveSpeedBonus_Constant() return self:GetStackCount() end

--------------------------------------------------------------------------------

modifier_time_walk_lua_cast = class({})

function modifier_time_walk_lua_cast:IsPurgable() return false end
function modifier_time_walk_lua_cast:IsDebuff() return false end
function modifier_time_walk_lua_cast:IsHidden() return true end
function modifier_time_walk_lua_cast:IgnoreTenacity() return true end

function modifier_time_walk_lua_cast:GetEffectName() return "particles/units/heroes/hero_faceless_void/faceless_void_time_walk.vpcf" end
function modifier_time_walk_lua_cast:GetEffectAttachType() return PATTACH_ABSORIGIN_FOLLOW end
...
function modifier_time_walk_lua_cast:OnCreated(params)
    if IsServer() then
        ...
        local particle = ParticleManager:CreateParticle("particles/units/heroes/hero_faceless_void/faceless_void_time_walk_preimage.vpcf", PATTACH_ABSORIGIN, self.parent)
        ...
        ParticleManager:SetParticleControlEnt(particle, 2, self.parent, PATTACH_ABSORIGIN_FOLLOW, "attach_hitloc", self.parent:GetForwardVector(), true)
...
function modifier_time_walk_lua_cast:OnIntervalThink()
    ...
    for _, enemy in pairs(enemies) do
        if not enemy:HasModifier("modifier_time_walk_lua_slow") then
            ...
            local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_faceless_void/faceless_void_backtrack02.vpcf", PATTACH_ABSORIGIN, enemy)
            ParticleManager:ReleaseParticleIndex(pfx)
            
            enemy:AddNewModifier(caster, ability, "modifier_time_walk_lua_slow", { duration = duration * (1 - enemy:GetStatusResistance()) })
        end
    end
end
...
function modifier_time_walk_lua_cast:OnDestroy()
    ...
    if talent and talent:GetLevel() > 0 then
        local duration = ability:GetSpecialValueFor("duration")
        local as_buff = caster:AddNewModifier(caster, ability, "modifier_time_walk_lua_buff_as", { duration = duration })
        local ms_buff = caster:AddNewModifier(caster, ability, "modifier_time_walk_lua_buff_ms", { duration = duration })
        if as_buff then as_buff:SetStackCount(self.as_stolen) end
        if ms_buff then ms_buff:SetStackCount(self.ms_stolen) end
    end
    ...
    if talent and talent:GetLevel() > 0 then
        ...
        local final_enemies = FindUnitsInRadius(caster:GetTeamNumber(), parent:GetAbsOrigin(), nil, self.slow_radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, 0, 0, false)
    ...
    local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_faceless_void/faceless_void_time_walk_slow.vpcf", PATTACH_ABSORIGIN, parent)
...
function modifier_time_walk_lua_slow:IsPurgable() return true end
function modifier_time_walk_lua_slow:IsHidden() return false end
function modifier_time_walk_lua_slow:IsDebuff() return true end
function modifier_time_walk_lua_slow:GetEffectName() return "particles/units/heroes/hero_faceless_void/faceless_void_time_walk_debuff.vpcf" end
function modifier_time_walk_lua_slow:GetEffectAttachType() return PATTACH_ABSORIGIN_FOLLOW end
...
function modifier_time_walk_lua_slow:GetModifierMoveSpeedBonus_Constant() return self.ms_steal end
function modifier_time_walk_lua_slow:GetModifierAttackSpeedBonus_Constant() return self.as_steal end
...
function modifier_time_lock_lua:OnAttackLanded(event)
	...
	if target:IsOther() or target:IsInvulnerable() then return end
...
LinkLuaModifier( "modifier_faceless_void_chronosphere_lua_thinker", "heroes/hero_faceless_void/hero_faceless_void", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_faceless_void_chronosphere_lua_effect", "heroes/hero_faceless_void/hero_faceless_void", LUA_MODIFIER_MOTION_NONE )
...
function faceless_void_chronosphere_lua:OnSpellStart()
	...
	self.thinker = CreateModifierThinker( caster, self, "modifier_faceless_void_chronosphere_lua_thinker",  { duration = duration }, point, caster:GetTeamNumber(),	false)
