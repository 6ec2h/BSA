...
function shadow_fiend_shadowraze_a_lua:OnSpellStart() shadowraze.OnSpellStart(self) end
function shadow_fiend_shadowraze_b_lua:OnSpellStart() shadowraze.OnSpellStart(self) end
function shadow_fiend_shadowraze_c_lua:OnSpellStart() shadowraze.OnSpellStart(self) end

if shadowraze == nil then shadowraze = {} end
...
LinkLuaModifier( "modifier_shadow_fiend_necromastery_lua", "heroes/hero_nevermore/hero_nevermore", LUA_MODIFIER_MOTION_NONE )
...
function modifier_shadow_fiend_necromastery_lua:KillLogic( params )
	...
	if not _G.excludedUnitsLookup[target:GetUnitName()] or attacker:HasModifier("modifier_guild_event") then return end
