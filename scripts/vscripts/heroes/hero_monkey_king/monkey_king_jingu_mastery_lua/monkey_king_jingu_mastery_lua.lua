LinkLuaModifier("modifier_monkey_king_jingu_mastery_lua", "heroes/hero_monkey_king/monkey_king_jingu_mastery_lua/monkey_king_jingu_mastery_lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_monkey_king_jingu_mastery_lua_counter", "heroes/hero_monkey_king/monkey_king_jingu_mastery_lua/monkey_king_jingu_mastery_lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_monkey_king_jingu_mastery_lua_active", "heroes/hero_monkey_king/monkey_king_jingu_mastery_lua/monkey_king_jingu_mastery_lua", LUA_MODIFIER_MOTION_NONE)
...
function modifier_monkey_king_jingu_mastery_lua:OnAttackLanded(data)
    ...
    data.target:AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_monkey_king_jingu_mastery_lua_counter", {duration = self.max_duration})
end
...
function modifier_monkey_king_jingu_mastery_lua_active:OnCreated()
    ...
    self.special_bonus_unique_special_bonus_monkey_king_str9 = self:GetCaster():FindAbilityByName("special_bonus_unique_special_bonus_monkey_king_str9") ~= nil
...
function modifier_monkey_king_jingu_mastery_lua_active:OnAttackLanded(data)
    if data.attacker == self:GetParent() and not data.target:IsBuilding() and not data.attacker:PassivesDisabled() then
        ...
        if self:GetCaster().cast_boundless_strike == true or not self.special_bonus_unique_special_bonus_monkey_king_str9 then
