...
function legion_odds:Precache( context )
	PrecacheResource( "particle", "particles/units/heroes/hero_legion_commander/legion_commander_odds_hero_arrow_group.vpcf", context )
	PrecacheResource( "particle", "particles/units/heroes/hero_legion_commander/legion_commander_odds_hero_arrow_start_pos.vpcf", context )
end
    ...
    for _, enemy in pairs(enemies) do
        ...
        if not enemy:IsMagicImmune() and not enemy:IsInvulnerable() then
            if not enemy:IsInvisible() or caster:CanEntityBeSeenByMyTeam(enemy) then
                ...
                local strike_pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_legion_commander/legion_commander_odds_hero_arrow_start_pos.vpcf", PATTACH_ABSORIGIN_FOLLOW, enemy)
...
function modifier_legion_press_the_attack:OnCreated()
	...
	if IsServer() then
        self.particle_good_fx = ParticleManager:CreateParticle("particles/econ/items/legion/legion_fallen/legion_fallen_press.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.parent)
...
function legion_courage:Precache( context )
	...
	PrecacheResource( "particle", "particles/units/heroes/hero_legion_commander/legion_commander_courage_tgt_flash.vpcf", context )
...
function modifier_legion_courage:OnAttackLanded( params )
    ...
    if not ability or not ability:IsFullyCastable() or parent:PassivesDisabled() then return end

    if params.target == parent and not params.attacker:IsOther() and params.attacker:GetTeamNumber() ~= parent:GetTeamNumber() then
