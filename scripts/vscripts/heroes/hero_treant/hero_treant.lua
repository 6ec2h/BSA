LinkLuaModifier("modifier_treant_natures_grasp_lua_creation_thinker", "heroes/hero_treant/hero_treant", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_treant_natures_grasp_lua_damage", "heroes/hero_treant/hero_treant", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_treant_natures_grasp_lua_damage_bonus", "heroes/hero_treant/hero_treant", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_treant_natures_grasp_lua_latch_thinker", "heroes/hero_treant/hero_treant", LUA_MODIFIER_MOTION_NONE)
...
LinkLuaModifier("modifier_treant_living_armor_lua_passive_effect", "heroes/hero_treant/hero_treant", LUA_MODIFIER_MOTION_NONE)
...
function treant_natures_grasp_lua:OnSpellStart()
	...
	for thicket = 1, math.floor((self:GetCastRange(cursor_position, self:GetCaster()) - 100) / self:GetSpecialValueFor("vine_spawn_interval")) do
		self:GetCaster():SetContextThink(DoUniqueString("grasp_thinker"), function()
			thicket_thinker = CreateModifierThinker(self:GetCaster(), self, "modifier_treant_natures_grasp_lua_creation_thinker", {
...
function modifier_treant_natures_grasp_lua_creation_thinker:IsHidden()		return true end
function modifier_treant_natures_grasp_lua_creation_thinker:IsPurgable()	return false end
...
function modifier_treant_natures_grasp_lua_creation_thinker:IsAura()						return true end
function modifier_treant_natures_grasp_lua_creation_thinker:IsAuraActiveOnDeath() 			return false end

function modifier_treant_natures_grasp_lua_creation_thinker:GetAuraRadius()				return self.latch_range end

function modifier_treant_natures_grasp_lua_creation_thinker:GetAuraSearchFlags()
	return DOTA_UNIT_TARGET_FLAG_NONE
end

function modifier_treant_natures_grasp_lua_creation_thinker:GetAuraSearchTeam()			return DOTA_UNIT_TARGET_TEAM_ENEMY end
function modifier_treant_natures_grasp_lua_creation_thinker:GetAuraSearchType()			return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_CREEP end
...
function modifier_treant_natures_grasp_lua_damage:OnCreated()
	...
	if self:GetAbility() then
		self.damage_per_second	= self:GetAbility():GetSpecialValueFor("damage_per_second")
		self.movement_slow	= self:GetAbility():GetSpecialValueFor("movement_slow")
	elseif self:GetAuraOwner() and self:GetAuraOwner():HasModifier("modifier_treant_natures_grasp_lua_creation_thinker") then
		self.damage_per_second	= self:GetAuraOwner():FindModifierByName("modifier_treant_natures_grasp_lua_creation_thinker").damage_per_second
		self.movement_slow		= self:GetAuraOwner():FindModifierByName("modifier_treant_natures_grasp_lua_creation_thinker").movement_slow
		self.damage_type		= self:GetAuraOwner():FindModifierByName("modifier_treant_natures_grasp_lua_creation_thinker").damage_type
	else
		self:Destroy()
	end
...
function treant_leech_seed_lua:OnSpellStart()
	...
	self.hTarget:AddNewModifier(self:GetCaster(), self, "modifier_treant_leech_seed_lua", {duration = self:GetSpecialValueFor("duration") - FrameTime()})
end
...
function treant_leech_seed_lua:OnProjectileHit_ExtraData(target, location, ExtraData)
	if target:GetTeamNumber() == self:GetCaster():GetTeamNumber() then
		...
		target:AddNewModifier(self:GetCaster(), self, "modifier_treant_leech_seed_lua_slow", {duration = self:GetSpecialValueFor("slow_duration")})
		dealdamage(target, self:GetSpecialValueFor("leech_damage") * self:GetSpecialValueFor("damage_interval"), self:GetCaster(), self)
	end
end
...
function treant_overgrowth_lua:OnSpellStart()
	...
	for _, enemy in pairs(overgrowth_primary_enemies) do
		enemy:Stop()
		enemy:AddNewModifier(self:GetCaster(), self, "modifier_treant_overgrowth_lua", {duration = self:GetSpecialValueFor("duration") * (1 - enemy:GetStatusResistance())})
	end
end
