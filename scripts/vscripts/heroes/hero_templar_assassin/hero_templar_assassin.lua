LinkLuaModifier("modifier_templar_assassin_refraction_lua", "heroes/hero_templar_assassin/hero_templar_assassin", LUA_MODIFIER_MOTION_NONE)
...
function templar_assassin_refraction_lua:HandleAutocast()
    if not self:GetAutoCastState() then return 0.5 end
    if not self:IsFullyCastable() then return 0.5 end
    if self:GetCaster():IsInvisible() then return 0.5 end 
...
function modifier_templar_assassin_refraction_lua:IsPurgable() return false end
function modifier_templar_assassin_refraction_lua:GetTexture() return "templar_assassin_refraction" end
...
function modifier_templar_assassin_refraction_lua:OnCreated()
    ...
    local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_templar_assassin/templar_assassin_refraction.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
    ParticleManager:SetParticleControlEnt(pfx, 1, self:GetParent(), PATTACH_ABSORIGIN_FOLLOW, "attach_hitloc", Vector(0,0,0), true)
...
function modifier_templar_assassin_refraction_lua:GetAbsoluteNoDamagePhysical(keys) return self:HandleBlock(keys) end
function modifier_templar_assassin_refraction_lua:GetAbsoluteNoDamageMagical(keys) return self:HandleBlock(keys) end
function modifier_templar_assassin_refraction_lua:GetAbsoluteNoDamagePure(keys) return self:HandleBlock(keys) end
...
function modifier_templar_assassin_refraction_lua:PlayHitEffects()
    ...
    local hit = ParticleManager:CreateParticle("particles/units/heroes/hero_templar_assassin/templar_assassin_refract_hit.vpcf", PATTACH_ABSORIGIN_FOLLOW, parent)
    ParticleManager:SetParticleControlEnt(hit, 2, parent, PATTACH_ABSORIGIN_FOLLOW, "attach_hitloc", Vector(0,0,0), true)
...
LinkLuaModifier("modifier_templar_assassin_meld_lua", "heroes/hero_templar_assassin/hero_templar_assassin", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_templar_assassin_meld_lua_debuff", "heroes/hero_templar_assassin/hero_templar_assassin", LUA_MODIFIER_MOTION_NONE)
...
function modifier_templar_assassin_meld_lua:IsHidden() return true end
function modifier_templar_assassin_meld_lua:IsPurgable() return false end
function modifier_templar_assassin_meld_lua:RemoveOnDeath() return false end
...
function modifier_templar_assassin_meld_lua_debuff:IsDebuff() return true end
function modifier_templar_assassin_meld_lua_debuff:IsPurgable() return true end
...
function modifier_templar_assassin_meld_lua_debuff:OnCreated()
    ...
    if not IsServer() then return end
    local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_templar_assassin/templar_meld_overhead.vpcf", PATTACH_OVERHEAD_FOLLOW, self:GetParent())
...
LinkLuaModifier("modifier_templar_assassin_psi_blades_lua_damage", "heroes/hero_templar_assassin/hero_templar_assassin", LUA_MODIFIER_MOTION_NONE)
...
function modifier_templar_assassin_psi_blades_lua_damage:OnAttackLanded(keys)
    ...
    for _, enemy in pairs(enemies) do
        if enemy ~= target then
            ...
            local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_templar_assassin/templar_assassin_psi_blade.vpcf", PATTACH_ABSORIGIN_FOLLOW, target)
            ParticleManager:SetParticleControlEnt(pfx, 0, target, PATTACH_POINT_FOLLOW, "attach_hitloc", target_pos, true)
            ...
            ApplyDamage({
                ...
                damage_type     = DAMAGE_TYPE_PURE,
                damage_flags    = DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION + DOTA_DAMAGE_FLAG_HPLOSS + DOTA_DAMAGE_FLAG_DONT_DISPLAY_DAMAGE_IF_SOURCE_HIDDEN,
...
LinkLuaModifier("modifier_templar_assassin_trap_lua", "heroes/hero_templar_assassin/hero_templar_assassin", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_templar_assassin_trap_lua_slow", "heroes/hero_templar_assassin/hero_templar_assassin", LUA_MODIFIER_MOTION_NONE)
...
function templar_assassin_trap_lua:OnSpellStart()
    ...
    if RollPercentage(50) then
        local sound = RollPercentage(2) and "templar_assassin_temp_psionictrap_04" or "templar_assassin_temp_psionictrap_0"..RandomInt(1, 3)
        caster:EmitSound(sound)
    end

    local trap = CreateUnitByName("npc_dota_templar_assassin_psionic_trap", position, false, caster, caster, caster:GetTeamNumber())
...
function modifier_templar_assassin_trap_lua:IsHidden() return true end

function modifier_templar_assassin_trap_lua:OnCreated()
    if not IsServer() then return end
    local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_templar_assassin/templar_assassin_trap.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
...
function modifier_templar_assassin_trap_lua:OnDestroy()
    ...
    local explode = ParticleManager:CreateParticle("particles/units/heroes/hero_templar_assassin/templar_assassin_trap_explode.vpcf", PATTACH_ABSORIGIN_FOLLOW, parent)
...
function templar_assassin_trap_lua_teleport:OnSpellStart()
    ...
    if not trap_ability then return end
    
    local radius = 250
    local damage = trap_ability:GetSpecialValueFor("damage")
    local slow_dur = trap_ability:GetSpecialValueFor("slow_duration")

    local all_units = FindUnitsInRadius(caster:GetTeamNumber(), origin_pos, nil, FIND_UNITS_EVERYWHERE, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_ALL, DOTA_UNIT_TARGET_FLAG_INVULNERABLE, FIND_CLOSEST, false)
    ...
    for _, unit in pairs(all_units) do
        if unit:GetUnitName() == "npc_dota_templar_assassin_psionic_trap" and unit:IsAlive() then
            ...
            local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_templar_assassin/templar_assassin_trap_explode.vpcf", PATTACH_WORLDORIGIN, nil)
