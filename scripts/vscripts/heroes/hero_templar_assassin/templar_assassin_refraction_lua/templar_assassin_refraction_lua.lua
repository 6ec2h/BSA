...
function modifier_templar_assassin_refraction_lua_damage:GetAbsoluteNoDamagePhysical(keys)
	if keys.attacker and (keys.damage and keys.damage >= 5) and keys.damage_type == DAMAGE_TYPE_PHYSICAL and bit.band(keys.damage_flags, DOTA_DAMAGE_FLAG_HPLOSS) ~= DOTA_DAMAGE_FLAG_HPLOSS then
		print("phys")
		self:PlayEffect()
		return 1
	end
...
function modifier_templar_assassin_refraction_lua_damage:GetAbsoluteNoDamageMagical(keys)
	if keys.attacker and (keys.damage and keys.damage >= 5) and keys.damage_type == DAMAGE_TYPE_MAGICAL and bit.band(keys.damage_flags, DOTA_DAMAGE_FLAG_HPLOSS) ~= DOTA_DAMAGE_FLAG_HPLOSS then
		print("magic")
		self:PlayEffect()
		return 1
	end
...
function modifier_templar_assassin_refraction_lua_damage:GetAbsoluteNoDamagePure(keys)
	if keys.attacker and (keys.damage and keys.damage >= 5) and keys.damage_type == DAMAGE_TYPE_PURE and bit.band(keys.damage_flags, DOTA_DAMAGE_FLAG_HPLOSS) ~= DOTA_DAMAGE_FLAG_HPLOSS then
		print("magic")
		self:PlayEffect()
		return 1
	end
...
function modifier_templar_assassin_refraction_lua_damage:PlayEffect()
