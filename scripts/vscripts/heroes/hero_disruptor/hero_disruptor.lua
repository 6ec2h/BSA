LinkLuaModifier( "modifier_disruptor_thunder_strike_lua", "heroes/hero_disruptor/hero_disruptor", LUA_MODIFIER_MOTION_NONE )
...
function disruptor_magnetic_collect:Precache( context )
	PrecacheResource( "particle", "particles/econ/items/razor/razor_arcana/razor_arcana_eye_of_the_storm_rain_v2.vpcf", context )
...
function modifier_magnetic_collect_thinker:IsHidden() return false end
function modifier_magnetic_collect_thinker:IsDebuff() return false end
function modifier_magnetic_collect_thinker:IsPurgable() return false end
...
function modifier_magnetic_collect_thinker:OnCreated(kv)
    ...
    self.damage = self:GetAbility():GetSpecialValueFor( "strike_damage" ) + self:GetCaster():ExtraIntelligenceDamage() * self:GetAbility():GetSpecialValueFor("ExtraIntelligenceDamage")
...
function modifier_magnetic_collect_thinker:IsAura() return true end
function modifier_magnetic_collect_thinker:GetModifierAura() return "modifier_magnetic_collect_effect" end
function modifier_magnetic_collect_thinker:GetAuraRadius() return self.radius end
function modifier_magnetic_collect_thinker:GetAuraSearchTeam() return DOTA_UNIT_TARGET_TEAM_ENEMY end
function modifier_magnetic_collect_thinker:GetAuraSearchType() return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC end
function modifier_magnetic_collect_thinker:GetAuraDuration() return 0.1 end
...
function modifier_magnetic_collect_effect:IsHidden() return false end
function modifier_magnetic_collect_effect:IsDebuff() return true end
function modifier_magnetic_collect_effect:IsPurgable() return false end
...
function disruptor_kinetic_field_lua:OnSpellStart()
	...
	CreateModifierThinker(caster, self, "modifier_disruptor_kinetic_field_lua", {}, point, caster:GetTeamNumber(), false)
