...
function lina_dragon_slave_lua:OnProjectileHitHandle( target, location, projectile )
	...
	if self:GetCaster():FindAbilityByName("npc_dota_hero_luna_1")~=nil then
		if self:GetCaster():FindAbilityByName("npc_dota_hero_luna_1"):GetLevel() > 0 then  
			damage_slave = damage_slave + 140
		end
	end
