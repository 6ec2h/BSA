...
function magnus_shockwave_lua:PlayEffects1()
    local caster = self:GetCaster()
    self.effect_cast = ParticleManager:CreateParticle( "particles/units/heroes/hero_magnataur/magnataur_shockwave_cast.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster )
    ParticleManager:SetParticleControlEnt( self.effect_cast, 1, caster, PATTACH_POINT_FOLLOW, "attach_attack1", Vector(0,0,0), true )
...
function magnus_shockwave_lua:PlayEffects2( target )
    local effect_cast = ParticleManager:CreateParticle( "particles/units/heroes/hero_magnataur/magnataur_shockwave_hit.vpcf", PATTACH_ABSORIGIN_FOLLOW, target )
...
function modifier_magnus_shockwave_lua:IsHidden() return false end
function modifier_magnus_shockwave_lua:IsDebuff() return true end
function modifier_magnus_shockwave_lua:IsPurgable() return true end
...
function modifier_magnus_empower_lua_aura:GetAuraSearchType() return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC end
function modifier_magnus_empower_lua_aura:GetAuraSearchFlags() return DOTA_UNIT_TARGET_FLAG_NONE end
...
function modifier_magnus_empower_lua:OnAttackLanded(params)
    if not IsServer() or params.attacker ~= self:GetParent() or self:GetParent():IsRangedAttacker() then return end
...
function modifier_magnataur_skin:IsPurgable() return false end
function modifier_magnataur_skin:IsDebuff() return false end
...
function magnus_reverse_polarity_lua:PlayEffects1()
    ...
    self.effect_cast = ParticleManager:CreateParticle( "particles/units/heroes/hero_magnataur/magnataur_reverse_polarity.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster )
    ...
    ParticleManager:SetParticleControlEnt( self.effect_cast, 3, caster, PATTACH_ABSORIGIN_FOLLOW, "attach_hitloc", Vector(0,0,0), true )
