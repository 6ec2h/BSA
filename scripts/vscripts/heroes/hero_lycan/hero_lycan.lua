...
function modifier_lycan_double_hit_sabre:OnAttackLanded(keys)
	...
	if keys.attacker ~= parent or not self:GetAbility() then return end
	if parent:IsIllusion() or parent:PassivesDisabled() then return end
	if not keys.target or keys.target:IsNull() then return end

	keys.target:AddNewModifier(parent, self:GetAbility(), "modifier_lycan_double_hit_slow",
	...
	if self.attack_count < hits then return end
	self.attack_count = 0
	...
	if abil ~= nil and abil:GetLevel() > 0 then
		DoCleaveAttack( parent, keys.target, self:GetAbility(), keys.original_damage, 150, 360, 360, 'particles/econ/items/sven/sven_ti7_sword/sven_ti7_sword_spell_great_cleave.vpcf' )
	end
end
...
LinkLuaModifier("modifier_lycan_shapeshift_lua_transform_stun", "heroes/hero_lycan/hero_lycan", LUA_MODIFIER_MOTION_NONE)
