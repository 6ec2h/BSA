...
LinkLuaModifier( "modifier_bristleback_passive_armor", "heroes/hero_bristleback/hero_bristleback", LUA_MODIFIER_MOTION_NONE )
...
LinkLuaModifier("modifier_bristleback_quill_spray_lua", "heroes/hero_bristleback/hero_bristleback", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_bristleback_quill_spray_lua_stack", "heroes/hero_bristleback/hero_bristleback", LUA_MODIFIER_MOTION_NONE)
...
function bristleback_quill_spray_lua:PlayEffects1()
    local p = ParticleManager:CreateParticle("particles/units/heroes/hero_bristleback/bristleback_quill_spray.vpcf", PATTACH_ABSORIGIN, self:GetCaster())
...
function modifier_bristleback_quill_spray_lua:IsHidden() return false end
...
function modifier_bristleback_quill_spray_lua_stack:IsHidden() return true end
function modifier_bristleback_quill_spray_lua_stack:IsPurgable() return false end
function modifier_bristleback_quill_spray_lua_stack:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE end
...
LinkLuaModifier("modifier_bristleback_bristleback_lua", "heroes/hero_bristleback/hero_bristleback", LUA_MODIFIER_MOTION_NONE)
...
function modifier_bristleback_bristleback_lua:IsHidden() return true end
...
function modifier_bristleback_bristleback_lua:GetModifierIncomingDamage_Percentage(keys)
    ...
    if keys.attacker == self.parent or bit.band(keys.damage_flags, DOTA_DAMAGE_FLAG_REFLECTION) == DOTA_DAMAGE_FLAG_REFLECTION then return 0 end
...
LinkLuaModifier("modifier_bristleback_warpath_lua", "heroes/hero_bristleback/hero_bristleback", LUA_MODIFIER_MOTION_NONE)
...
function modifier_bristleback_warpath_lua:OnCreated()
	...
	if self.parent:IsIllusion() then
		...
		for _, owner in pairs(owners) do
			if not owner:IsIllusion() and owner:HasModifier("modifier_bristleback_warpath_lua") and owner:GetTeam() == self.parent:GetTeam() then
