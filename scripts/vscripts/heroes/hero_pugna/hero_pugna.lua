...
function pugna_nether_blast_lua:OnSpellStart()
	...
	local damage = ability:GetSpecialValueFor("damage") + self:GetCaster():ExtraIntelligenceDamage() * self:GetSpecialValueFor("ExtraIntelligenceDamage") 
...
function pugna_decrepify_lua:OnSpellStart()
	...
	target:AddNewModifier(caster, self, "modifier_decrepify_lua", {duration = duration * (1 - target:GetStatusResistance()) })	
end
...
function modifier_pugna_nether_ward_lua:IsHidden() return true end
function modifier_pugna_nether_ward_lua:IsPurgable() return false end
