...
if IsServer() then
	function modifier_effect:OnCreated(table)
		...
		if self:GetCaster():FindAbilityByName("npc_dota_hero_pugna_int10")~=nil then
			if self:GetCaster():FindAbilityByName("npc_dota_hero_pugna_int10"):GetLevel() > 0 then 
				interval = 1
			end	
		end
	...
	function modifier_effect:OnIntervalThink()
		...
		if self:GetCaster():FindAbilityByName("npc_dota_hero_pugna_int6")~=nil then
			if self:GetCaster():FindAbilityByName("npc_dota_hero_pugna_int6"):GetLevel() > 0 then 
				max_count = 5
			end	
		end
