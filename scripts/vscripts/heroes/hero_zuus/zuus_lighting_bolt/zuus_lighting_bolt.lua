...
function zuus_lightning_bolt_lua:CastLightningBolt(caster, ability, target, target_point, nimbus)
	if IsServer() then
		...
		local bolt_damage 			= ability:GetSpecialValueFor("damage") + caster:ExtraIntelligenceDamage() * ability:GetSpecialValueFor("ExtraIntelligenceDamage") 
		...
		if target == nil then
			...
			local nearby_enemy_units = FindUnitsInRadius(caster:GetTeamNumber(), target_point, nil, spread_aoe, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_ALL, target_flags, FIND_CLOSEST, false)
