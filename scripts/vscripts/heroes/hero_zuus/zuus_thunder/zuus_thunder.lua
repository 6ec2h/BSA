...
function zuus_thundergods_wrath_lua:OnSpellStart()
	...
	Timers:CreateTimer(0, function()
		...
		local hEnemies = FindUnitsInRadius(
			caster:GetTeamNumber(), position, caster, range,
		...
		for _, enemy in pairs(hEnemies) do
			if enemy:IsAlive() then
				...
				local strike_pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_zuus/zuus_thundergods_wrath.vpcf", PATTACH_ABSORIGIN_FOLLOW, enemy)
				ParticleManager:SetParticleControl(strike_pfx, 0, Vector(target_point.x, target_point.y, target_point.z + enemy:GetBoundingMaxs().z))
				ParticleManager:SetParticleControl(strike_pfx, 1, Vector(target_point.x, target_point.y, 2000))
				ParticleManager:SetParticleControl(strike_pfx, 2, Vector(target_point.x, target_point.y, target_point.z + enemy:GetBoundingMaxs().z))
				...
				if not enemy:IsMagicImmune() and not enemy:IsInvisible() then
					...
					Timers:CreateTimer(FrameTime(), function()
						if not enemy:IsNull() and not enemy:IsAlive() then
							local kill_pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_zeus/zues_kill_empty.vpcf", PATTACH_WORLDORIGIN, nil)
