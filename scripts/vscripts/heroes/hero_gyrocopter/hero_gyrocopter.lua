...
function modifier_gyrocopter_rocket_barrage_lua:IsHidden() return false end
function modifier_gyrocopter_rocket_barrage_lua:IsPurgable() return false end
...
function modifier_gyrocopter_rocket_barrage_lua:OnIntervalThink()
    if self.parent:IsOutOfGame() or self.parent:IsStunned() or self.parent:IsSilenced() then return end
    ...
    if #enemies > 0 then
        ...
        local pfx = ParticleManager:CreateParticle("particles/econ/items/gyrocopter/hero_gyrocopter_gyrotechnics/gyro_rocket_barrage.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.parent)
...
LinkLuaModifier("modifier_gyrocopter_homing_missile_autocast", "heroes/hero_gyrocopter/hero_gyrocopter", LUA_MODIFIER_MOTION_NONE)
...
function modifier_gyrocopter_homing_missile_autocast:IsHidden() return true end
function modifier_gyrocopter_homing_missile_autocast:IsPurgable() return false end
function modifier_gyrocopter_homing_missile_autocast:RemoveOnDeath() return false end
...
LinkLuaModifier("modifier_gyrocopter_flak_cannon_lua", "heroes/hero_gyrocopter/hero_gyrocopter", LUA_MODIFIER_MOTION_NONE)

gyrocopter_flak_cannon_lua = class({})

function gyrocopter_flak_cannon_lua:GetCooldown(level)
    local talent self:GetCaster():FindAbilityByName("special_bonus_unique_gyrocopter_7")
    if talent and talent:GetLevel() > 0 then 
...
function modifier_gyrocopter_flak_cannon_lua:IsHidden() return false end
...
LinkLuaModifier("modifier_gyrocopter_call_down_lua_thinker", "heroes/hero_gyrocopter/hero_gyrocopter", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_gyrocopter_call_down_lua_slow", "heroes/hero_gyrocopter/hero_gyrocopter", LUA_MODIFIER_MOTION_NONE)
...
function modifier_gyrocopter_call_down_lua_thinker:OnCreated()
    ...
    self.marker_pfx = ParticleManager:CreateParticleForTeam("particles/units/heroes/hero_gyrocopter/gyro_calldown_marker.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent(), self.caster:GetTeamNumber())
...
function modifier_gyrocopter_call_down_lua_thinker:CreateMissileEffect(particle_name, attach_name)
    ...
    ParticleManager:SetParticleControl(pfx, 0, self.caster:GetAttachmentOrigin(self.caster:ScriptLookupAttachment(attach_name)))
