...
function modifier_clinkz_strafe_lua:GetModifierAttackSpeedBonus_Constant() return self.as end
function modifier_clinkz_strafe_lua:GetModifierAttackRangeBonus() return self.range end

function modifier_clinkz_strafe_lua:OnAttack(params)
    if not IsServer() then return end
    if params.attacker ~= self:GetParent() or not self.split_attack then return end

    local talent = self:GetCaster():FindAbilityByName("special_bonus_unique_clinkz_7")
    if not (talent and talent:GetLevel() > 0) then return end
...
LinkLuaModifier('modifier_npc_dota_hero_clinkz_permanent_ability_hp', "heroes/hero_clinkz/hero_clinkz", LUA_MODIFIER_MOTION_NONE)
...
function modifier_npc_dota_hero_clinkz_permanent_ability:IsHidden() return true end
...
function modifier_npc_dota_hero_clinkz_permanent_ability:OnAttackLanded(params)
    if not IsServer() or params.attacker ~= self.caster or self.caster:PassivesDisabled() then return end
...
function modifier_npc_dota_hero_clinkz_permanent_ability:SpawnArcher()
    ...
    if #self.archers_list >= self.ability:GetSpecialValueFor("archers") then return end

    local pos = self.caster:GetAbsOrigin() + RandomVector(100)
    local archer = CreateUnitByName("npc_dota_clinkz_skeleton_archer", pos, true, self.caster, self.caster, self.caster:GetTeamNumber())
    ...
    archer:AddNewModifier(self.caster, self.ability, "modifier_kill", {duration = self.ability:GetSpecialValueFor("duration")})
...
function modifier_npc_dota_hero_clinkz_permanent_ability_effect:GetModifierPreAttack_BonusDamage() return self:GetStackCount() end
...
function modifier_npc_dota_hero_clinkz_permanent_ability_hp:GetModifierIncomingDamage_Percentage() return -100 end
function modifier_npc_dota_hero_clinkz_permanent_ability_hp:GetDisableHealing() return 1 end
