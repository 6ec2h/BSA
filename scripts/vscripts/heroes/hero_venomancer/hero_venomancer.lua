LinkLuaModifier( "modifier_venomancer_venomous_gale_lua", "heroes/hero_venomancer/hero_venomancer", LUA_MODIFIER_MOTION_NONE )
...
LinkLuaModifier( "modifier_venomancer_poison_tick_debuff", "heroes/hero_venomancer/hero_venomancer", LUA_MODIFIER_MOTION_NONE )
...
function modifier_venomancer_poison_tick:IsHidden() return true end
...
function modifier_venomancer_poison_tick:OnAttackLanded( params )
    ...
    if not modifier then
        params.target:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_venomancer_poison_tick_debuff", {duration = duration})
    else
...
function modifier_venomancer_poison_tick_debuff:IsHidden() return false end
function modifier_venomancer_poison_tick_debuff:IsDebuff() return true end
function modifier_venomancer_poison_tick_debuff:IsPurgable() return true end
...
LinkLuaModifier( "modifier_venomancer_plague_ward_lua", "heroes/hero_venomancer/hero_venomancer", LUA_MODIFIER_MOTION_NONE )
...
function venomancer_plague_ward_lua:OnProjectileHit(target)
    ...
    for i = 1, count do
        ...
        local ward = CreateUnitByName("npc_dota_venomancer_plague_ward_1_custom", spawn_pos, true, caster, caster, caster:GetTeamNumber())
        ...
        local fx = ParticleManager:CreateParticle("particles/units/heroes/hero_venomancer/venomancer_ward_cast.vpcf", PATTACH_ABSORIGIN, ward)
...
LinkLuaModifier( "modifier_venomancer_poison_nova_lua", "heroes/hero_venomancer/hero_venomancer", LUA_MODIFIER_MOTION_NONE )
...
function venomancer_poison_nova_lua:OnSpellStart()
    ...
    if talent and talent:GetLevel() > 0 then
        ...
        for _, unit in pairs(units) do
            if unit:GetUnitName() == "npc_dota_venomancer_plague_ward_1_custom" and unit:GetOwner() == caster and unit:IsAlive() then
...
function venomancer_poison_nova_lua:SpawnPoisonRing(source)
    ...
    local ring = source:AddNewModifier(
        caster,
        self, 
    ...
    local effect_cast = ParticleManager:CreateParticle( "particles/units/heroes/hero_venomancer/venomancer_poison_nova.vpcf", PATTACH_ABSORIGIN_FOLLOW, source )
