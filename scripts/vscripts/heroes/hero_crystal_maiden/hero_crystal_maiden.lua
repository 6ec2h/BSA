LinkLuaModifier( "modifier_crystal_maiden_crystal_nova_lua", "heroes/hero_crystal_maiden/hero_crystal_maiden", LUA_MODIFIER_MOTION_NONE )
...
function crystal_maiden_crystal_nova_lua:OnSpellStart()
	...
	local damage = self:GetSpecialValueFor("nova_damage") + self:GetCaster():ExtraIntelligenceDamage() * self:GetSpecialValueFor("ExtraIntelligenceDamage") 
...
LinkLuaModifier( "modifier_crystal_maiden_frostbite_lua", "heroes/hero_crystal_maiden/hero_crystal_maiden", LUA_MODIFIER_MOTION_NONE )
...
function crystal_maiden_frostbite_lua:OnSpellStart()
	...
	if talent_ability ~= nil and talent_ability:GetLevel() > 0 then
		...
		for _,unit in pairs(units) do
			unit:AddNewModifier(caster, self, "modifier_crystal_maiden_frostbite_lua", { duration = unit_debuff_duration(self, unit) * (1 - unit:GetStatusResistance()) })
			unit:AddNewModifier(caster, self, "modifier_stunned", {duration = 0.1})
			self:PlayEffects( caster, unit )
		end
	else
		target:AddNewModifier(caster, self, "modifier_crystal_maiden_frostbite_lua", { duration = unit_debuff_duration(self, target) * (1 - target:GetStatusResistance())})
...
function modifier_crystal_maiden_frostbite_lua:OnCreated( kv )
	...
	if not IsServer() then return end
	tick_damage = (tick_damage + self:GetCaster():ExtraIntelligenceDamage() * self:GetAbility():GetSpecialValueFor("ExtraIntelligenceDamage") ) / 2
	self.interval = 0.5
...
LinkLuaModifier( "modifier_crystal_maiden_arcane_aura_lua", "heroes/hero_crystal_maiden/hero_crystal_maiden", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_crystal_maiden_arcane_aura_lua_effect", "heroes/hero_crystal_maiden/hero_crystal_maiden", LUA_MODIFIER_MOTION_NONE )
...
LinkLuaModifier( "modifier_crystal_maiden_freezing_field_lua", "heroes/hero_crystal_maiden/hero_crystal_maiden", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_crystal_maiden_freezing_field_lua_effect", "heroes/hero_crystal_maiden/hero_crystal_maiden", LUA_MODIFIER_MOTION_NONE )

function crystal_maiden_freezing_field_lua:OnSpellStart()
	local caster = self:GetCaster()
	self.modifier = caster:AddNewModifier(caster, self, "modifier_crystal_maiden_freezing_field_lua", { duration = self:GetChannelTime() })
end
