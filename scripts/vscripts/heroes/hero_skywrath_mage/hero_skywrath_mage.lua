LinkLuaModifier( "modifier_skywrath_mage_arcane_bolt_lua", "heroes/hero_skywrath_mage/hero_skywrath_mage", LUA_MODIFIER_MOTION_NONE )
...
LinkLuaModifier("modifier_skywrath_mage_concussive_shot_lua", "heroes/hero_skywrath_mage/hero_skywrath_mage", LUA_MODIFIER_MOTION_NONE)
...
function skywrath_mage_concussive_shot_lua:OnProjectileHit(target, location)
    if not target or not IsServer() then return end

    local caster = self:GetCaster()
    local radius = self:GetSpecialValueFor("slow_radius")
    local duration = self:GetSpecialValueFor("slow_duration")
    local damage = self:GetSpecialValueFor("damage") + (caster:ExtraIntelligenceDamage() * self:GetSpecialValueFor("ExtraIntelligenceDamage"))
    ...
    AddFOWViewer(caster:GetTeamNumber(), target:GetOrigin(), self:GetSpecialValueFor("shot_vision"), self:GetSpecialValueFor("vision_duration"), false)
    EmitSoundOn("Hero_SkywrathMage.ConcussiveShot.Target", target)
end

function skywrath_mage_concussive_shot_lua:PlayEffects1(target)
    local caster = self:GetCaster()
    local effect_cast = ParticleManager:CreateParticle("particles/units/heroes/hero_skywrath_mage/skywrath_mage_concussive_shot_cast.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster)
...
LinkLuaModifier("modifier_skywrath_mage_ancient_seal_lua", "heroes/hero_skywrath_mage/hero_skywrath_mage", LUA_MODIFIER_MOTION_NONE)
...
function modifier_skywrath_mage_ancient_seal_lua:IsHidden() return false end
function modifier_skywrath_mage_ancient_seal_lua:IsDebuff() return true end
function modifier_skywrath_mage_ancient_seal_lua:IsPurgable() return true end
...
LinkLuaModifier("modifier_skywrath_mage_mystic_flare_lua_thinker", "heroes/hero_skywrath_mage/hero_skywrath_mage", LUA_MODIFIER_MOTION_NONE)
