...
function luna_starfall:OnSpellStart()
    ...
    local function FireBeam(unit)
        ...
        ParticleManager:SetParticleControlEnt(particle, 5, unit, PATTACH_POINT_FOLLOW, "attach_hitloc", unit:GetAbsOrigin(), true)
    ...
    if main_target:TriggerSpellAbsorb(self) then return end
    FireBeam(main_target)

    Timers:CreateTimer(delay, function()
        if beams_fired >= beam_count then return nil end
...
function luna_moon_glaive_lua:OnProjectileHit_ExtraData(hTarget, vLocation, ExtraData)
    ...
    if ExtraData.bounces >= self:GetSpecialValueFor("bounces") then return end
...
function modifier_luna_moon_glaive_damage_handler:IsHidden() return true end
function modifier_luna_moon_glaive_damage_handler:IsPurgable() return false end
...
function modifier_luna_lunar_blessing_lua_aura:IsHidden() return true end
function modifier_luna_lunar_blessing_lua_aura:IsPurgable() return false end
...
function modifier_luna_lunar_blessing_lua_buff:IsHidden() return false end
function modifier_luna_lunar_blessing_lua_buff:IsDebuff() return false end
function modifier_luna_lunar_blessing_lua_buff:IsPurgable() return false end
