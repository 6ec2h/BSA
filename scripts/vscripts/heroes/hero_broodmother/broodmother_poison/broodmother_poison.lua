...
function modifier_broodmother_poison:GetModifierProcAttack_Feedback( params )
	if IsServer() then
		if params.attacker == self:GetParent() then
			modifier = params.target:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_broodmother_poison_debuff", { duration = self:GetAbility():GetSpecialValueFor("duration") })
