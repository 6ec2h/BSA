LinkLuaModifier("modifier_windrunner_attack_triggers", "heroes/hero_windranger/hero_windranger", LUA_MODIFIER_MOTION_NONE)
...
function windrunner_attack_triggers:OnProjectileHit_ExtraData(target, location, ExtraData)
    if not target or target:IsMagicImmune() or target:TriggerSpellAbsorb(self) then return end
    ...
    for i = 1, max_chains do
        ...
        if found_next then
            ...
            if i == 1 then
                target:AddNewModifier(caster, self, "modifier_stunned", {duration = self:GetSpecialValueFor("fail_stun_duration")})
            end
            break
        end
    end
...
function windrunner_attack_triggers:DealShackleDamage(target)
    if not target or not target:IsAlive() or not target.GetAbsOrigin then return end
...
function windrunner_attack_triggers:ApplyShackle(unit1, unit2)
    ...
    local function IsTree(ent)
        if not ent or ent:IsNull() then return true end
        if ent:GetClassname() == "ent_dota_tree" then return true end
        return false
    end
    ...
    if not u2_is_tree then
        ...
        if u1_is_tree then unit2:EmitSound("Hero_Windrunner.ShackleshotBind") end
    end

    local shackleshot_particle = ParticleManager:CreateParticle("particles/units/heroes/hero_windrunner/windrunner_shackleshot_pair.vpcf", PATTACH_POINT_FOLLOW, unit1)

    if not u1_is_tree then
        ParticleManager:SetParticleControlEnt(shackleshot_particle, 0, unit1, PATTACH_ABSORIGIN_FOLLOW, "attach_hitloc", unit1:GetAbsOrigin(), true)
    else
        ParticleManager:SetParticleControl(shackleshot_particle, 0, unit1:GetAbsOrigin())
    end

    if not u2_is_tree then
        ParticleManager:SetParticleControlEnt(shackleshot_particle, 1, unit2, PATTACH_ABSORIGIN_FOLLOW, "attach_hitloc", unit2:GetAbsOrigin(), true)
    else
...
function windrunner_powershot_lua:OnProjectileHit(hTarget, vLocation)
    if not hTarget then 
        AddFOWViewer(self:GetCaster():GetTeamNumber(), vLocation, self:GetSpecialValueFor("vision_radius"), self:GetSpecialValueFor("vision_duration"), false)
        return false 
    end
...
LinkLuaModifier( "modifier_windrunner_windrun_lua_effect", "heroes/hero_windranger/hero_windranger", LUA_MODIFIER_MOTION_NONE )
