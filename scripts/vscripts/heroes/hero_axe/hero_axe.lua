...
function modifier_axe_counter_helix_lua:OnAttackLanded(params)
    ...
    if not ability or not caster or caster:PassivesDisabled() or not ability:IsFullyCastable() then return end
    ...
    if not (is_being_attacked or is_attacking) then return end
...
function modifier_axe_blood_lua:IsHidden() return true end
function modifier_axe_blood_lua:IsPurgable() return false end
function modifier_axe_blood_lua:RemoveOnDeath() return false end
...
function modifier_axe_blood_lua_stacks:IsHidden() return false end
function modifier_axe_blood_lua_stacks:IsPurgable() return false end
function modifier_axe_blood_lua_stacks:GetTexture() return "axepassive" end
...
function axe_enrage_lua:OnSpellStart()	
	self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_axe_enrage_lua", { duration = self:GetSpecialValueFor("duration") })
