...
function modifier_spectre_dispersion_lua:IsHidden() return self:GetStackCount() == 0 end
function modifier_spectre_dispersion_lua:IsPurgable() return false end
function modifier_spectre_dispersion_lua:AllowIllusionDuplicate() return true end
...
function modifier_spectre_dispersion_lua:OnTakeDamage(params)
    if not IsServer() or params.unit ~= self.parent or self.parent:PassivesDisabled() then return end
...
function modifier_spectre_dispersion_lua:OnIntervalThink()
    ...
    for _, enemy in pairs(enemies) do
        ApplyDamage({
            ...
            damage = stacks,
            damage_type = DAMAGE_TYPE_PURE,
            damage_flags = DOTA_DAMAGE_FLAG_REFLECTION + DOTA_DAMAGE_FLAG_HPLOSS + DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION + DOTA_DAMAGE_FLAG_DONT_DISPLAY_DAMAGE_IF_SOURCE_HIDDEN,
