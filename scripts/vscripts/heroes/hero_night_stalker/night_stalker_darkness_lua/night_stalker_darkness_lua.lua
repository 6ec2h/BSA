...
LinkLuaModifier("modifier_imba_darkness_night", "heroes/hero_night_stalker/night_stalker_darkness_lua/night_stalker_darkness_lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_darkness_vision", "heroes/hero_night_stalker/night_stalker_darkness_lua/night_stalker_darkness_lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier( "modifier_night_fear_thinker", "heroes/hero_night_stalker/night_stalker_darkness_lua/night_stalker_darkness_lua", LUA_MODIFIER_MOTION_NONE )

function night_stalker_darkness_lua:IsNetherWardStealable()	return false end
function night_stalker_darkness_lua:IsHiddenWhenStolen() 		return false end
...
function night_stalker_darkness_lua:OnSpellStart()
	...
	if self:GetCaster():FindAbilityByName("npc_dota_night_stalker_int4")~=nil then
		if self:GetCaster():FindAbilityByName("npc_dota_night_stalker_int4"):GetLevel() > 0 then 
			CreateModifierThinker(self:GetCaster(), self, "modifier_night_fear_thinker", {duration = 1600 / 1000}, self:GetCaster():GetAbsOrigin(), self:GetCaster():GetTeamNumber(), false)
		end
	end
end
...
function modifier_night_fear_thinker:OnIntervalThink()
	if not self.bLaunched then
		...
		for _, enemy in pairs(FindUnitsInRadius(self:GetCaster():GetTeamNumber(), self:GetParent():GetAbsOrigin(), self:GetParent(), math.min(self.speed * (self:GetElapsedTime() - self.spawn_delay), self.radius), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NONE, FIND_ANY_ORDER, false)) do
