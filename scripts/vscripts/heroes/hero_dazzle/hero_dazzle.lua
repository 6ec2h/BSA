...
function modifier_dazzle_poison_touch_lua:IsHidden() return true end
function modifier_dazzle_poison_touch_lua:IsPurgeException() return false end
function modifier_dazzle_poison_touch_lua:IsPurgable() return false end
function modifier_dazzle_poison_touch_lua:RemoveOnDeath() return false end
...
function modifier_dazzle_poison_touch_lua:FindUnitsInCone( nTeamNumber, vCenterPos, vStartPos, vEndPos, fStartRadius, fEndRadius, hCacheUnit, nTeamFilter, nTypeFilter, nFlagFilter, nOrderFilter, bCanGrowCache )
...
function dazzle_shallow_grave_lua:OnSpellStart()
	...
	if talent_ability ~= nil and talent_ability:GetLevel() > 0 then
		...
		local units = FindUnitsInRadius(caster:GetTeamNumber(), target_point, nil, radius, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NONE, FIND_ANY_ORDER, false)
...
function dazzle_shadow_wave_lua:OnSpellStart()
	...
	local range = self:GetCastRange(self:GetCaster():GetAbsOrigin(), self:GetCaster()) + self:GetCaster():GetCastRangeBonus()


	local enemies = FindUnitsInRadius(caster:GetTeamNumber(), caster:GetOrigin(), nil, range, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, 0, false)
	for _,enemy in pairs(enemies) do
...
function dazzle_shadow_wave_lua:PlayEffects1( source, target )
	local effect_cast = ParticleManager:CreateParticle( "particles/units/heroes/hero_dazzle/dazzle_shadow_wave.vpcf", PATTACH_ABSORIGIN_FOLLOW, source )
