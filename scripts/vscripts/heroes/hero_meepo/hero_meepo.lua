...
function modifier_meepo_earthbind_lua:IsDebuff() return true end
...
function modifier_meepo_ransack_lua:IsHidden() return true end
...
function modifier_meepo_ransack_lua:OnAttackLanded(params)
    if not IsServer() then return end
    if params.attacker ~= self:GetParent() or params.target:IsBuilding() or params.target:IsOther() then return end
    ...
    ApplyDamage({ victim = target, attacker = params.attacker, damage = total_damage, damage_type = DAMAGE_TYPE_PURE, ability = ability, damage_flags = DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION + DOTA_DAMAGE_FLAG_DONT_DISPLAY_DAMAGE_IF_SOURCE_HIDDEN })

    local p_heal = ParticleManager:CreateParticle("particles/generic_gameplay/generic_lifesteal.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster)
...
function modifier_meepo_ransack_stack_instance:IsHidden() return true end
function modifier_meepo_ransack_stack_instance:IsPurgable() return false end
function modifier_meepo_ransack_stack_instance:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE end
...
function modifier_meepo_ransack_stack_instance:OnDestroy()
    ...
    if main_mod then main_mod:UpdateStacks() end
end
...
function modifier_meepo_ransack_stack:OnCreated()
    ...
    if IsServer() then self:UpdateStacks() end
end
...
function modifier_meepo_ransack_stack:UpdateStacks()
    ...
    if count > self.max_stacks then count = self.max_stacks end
    
    self:SetStackCount(count)
    if count <= 0 then self:Destroy() end
end
...
function modifier_meepo_ransack_stack:GetTexture() return "meepo_ransack" end
...
function meepo_king_lua:OnSpellStart()
    ...
    local p_burst = ParticleManager:CreateParticle("particles/units/heroes/hero_meepo/meepo_poof_end.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster)
...
function modifier_meepo_king_passive:IsHidden() return true end
function modifier_meepo_king_passive:IsPurgable() return false end
...
function modifier_meepo_king_passive:GetModifierBonusStats_Strength()
    ...
    if self.lock_str then return 0 end
    self.lock_str = true
...
function modifier_meepo_king_passive:GetModifierBonusStats_Agility()
    ...
    if self.lock_agi then return 0 end
    self.lock_agi = true
...
function modifier_meepo_king_passive:GetModifierBonusStats_Intellect()
    ...
    if self.lock_int then return 0 end
    self.lock_int = true
...
function modifier_meepo_king_active:OnCreated()
    if not IsServer() then return end
    self.nFXIndex = ParticleManager:CreateParticle("particles/units/heroes/hero_brewmaster/brewmaster_thunder_clap.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
...
function modifier_meepo_king_active:IsAura() return true end
function modifier_meepo_king_active:GetModifierAura() return "modifier_meepo_king_debuff" end
function modifier_meepo_king_active:GetAuraRadius() return 500 end
function modifier_meepo_king_active:GetAuraSearchTeam() return DOTA_UNIT_TARGET_TEAM_ENEMY end
function modifier_meepo_king_active:GetAuraSearchType() return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC end
...
function modifier_meepo_king_debuff:OnIntervalThink()
    ...
    local p = ParticleManager:CreateParticle("particles/units/heroes/hero_meepo/meepo_earthbind_target.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
...
function modifier_meepo_king_shield:IsHidden() return true end
function modifier_meepo_king_shield:IsPurgable() return false end
