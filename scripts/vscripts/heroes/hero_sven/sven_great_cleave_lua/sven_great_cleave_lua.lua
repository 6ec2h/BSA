LinkLuaModifier( "modifier_sven_great_cleave_lua", "heroes/hero_sven/sven_great_cleave_lua/sven_great_cleave_lua", LUA_MODIFIER_MOTION_NONE )
	...
	if IsServer() then
		if params.attacker == self:GetParent() and ( not self:GetParent():IsIllusion() ) then
			...
			if target ~= nil and target:GetTeamNumber() ~= self:GetParent():GetTeamNumber() then
				...
				DoCleaveAttack( self:GetParent(), target, self:GetAbility(), cleaveDamage, 150, 360, self.great_cleave_radius, "particles/units/heroes/hero_sven/sven_spell_great_cleave.vpcf" )
			end
		end
	end
	return 0
