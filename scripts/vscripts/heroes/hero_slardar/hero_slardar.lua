...
function lua_slardar_slithereen_crush:OnSpellStart()
	...
	local particle_splash_fx = ParticleManager:CreateParticle("particles/units/heroes/hero_slardar/slardar_crush.vpcf", PATTACH_ABSORIGIN, caster)
	...
	for _, enemy in pairs(enemies) do
		if not enemy:IsMagicImmune() then
			...
			enemy:AddNewModifier(caster, self, "modifier_lua_slithereen_crush_stun", {duration = stun_duration * (1 - enemy:GetStatusResistance())})
			enemy:AddNewModifier(caster, self, "modifier_lua_slithereen_crush_slow", {duration = (stun_duration + slow_duration) * (1 - enemy:GetStatusResistance())})
		end
	end
end
...
function modifier_lua_guardian_sprint_buff:IsHidden() return false end
function modifier_lua_guardian_sprint_buff:IsDebuff() return false end
	...
	if IsServer() then
		...
		if caster == attacker then
			...
			if RandomInt(1,100) <= chance_pct then
				...
				target:AddNewModifier(caster, self:GetAbility(), 'modifier_lua_bash_of_the_deep_disarm', {duration = duration * (1 - target:GetStatusResistance())})
				return damage
			end
		end
		return 0
...
LinkLuaModifier("modifier_lua_corrosive_haze_debuff", "heroes/hero_slardar/hero_slardar", LUA_MODIFIER_MOTION_HORIZONTAL)
...
function lua_slardar_corrosive_haze:OnSpellStart()
	...
	if talent_ability ~= nil and talent_ability:GetLevel() > 0 then
		...
		target:AddNewModifier(caster, self, 'modifier_lua_corrosive_haze_debuff', {duration = duration * (1 - target:GetStatusResistance())})
	end
end
