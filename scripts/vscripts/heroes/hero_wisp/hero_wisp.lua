...
function modifier_wisp_spirits_lua:IsHidden() return true end
function modifier_wisp_spirits_lua:IsPurgable() return false end
...
function modifier_wisp_spirits_lua:OnRefresh(params)
    ...
    if self.max_spirits > current_count then
        for i = current_count + 1, self.max_spirits do
            local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_wisp/wisp_guardian.vpcf", PATTACH_WORLDORIGIN, nil)
...
function modifier_wisp_spirits_lua:OnIntervalThink()
    ...
    for i, pfx in ipairs(self.spirit_particles) do
        ...
        local rel_pos = RotatePosition(Vector(0,0,0), QAngle(0, -rotation_angle, 0), Vector(0, self.current_radius, 0))
...
function modifier_wisp_spirits_lua_creep_hit:OnCreated() 
    ...
    local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_wisp/wisp_guardian_explosion_small.vpcf", PATTACH_ABSORIGIN_FOLLOW, target)
    ParticleManager:ReleaseParticleIndex(pfx)
end
...
function wisp_event_horizon:OnSpellStart()
    ...
    local particle = ParticleManager:CreateParticle("particles/econ/items/keeper_of_the_light/kotl_ti10_immortal/kotl_ti10_blinding_light.vpcf", PATTACH_POINT_FOLLOW, caster)
    ...
    for _, enemy in pairs(enemies) do
        ...
        local hit_pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_wisp/wisp_relocate_teleport_flash.vpcf", PATTACH_ABSORIGIN_FOLLOW, enemy)
