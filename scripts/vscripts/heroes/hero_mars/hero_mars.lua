...
function modifier_mars_atrophy_aura_lua:IsHidden() return self:GetStackCount() == 0 end
function modifier_mars_atrophy_aura_lua:IsPurgable() return false end
function modifier_mars_atrophy_aura_lua:RemoveOnDeath() return false end
...
function modifier_mars_atrophy_aura_lua:OnDeath(params)
    if not IsServer() then return end
    if params.unit:IsIllusion() or params.unit == self:GetParent() then return end
    if self:GetParent():PassivesDisabled() then return end
    if not params.unit:FindModifierByNameAndCaster("modifier_mars_atrophy_aura_lua_debuff", self:GetParent()) then return end
	if not _G.excludedUnitsLookup[params.unit:GetUnitName()] then return end

	self:GetParent():AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_mars_atrophy_aura_lua_stack", {duration = self.duration})
...
function modifier_mars_atrophy_aura_lua_debuff:IsHidden() return true end
function modifier_mars_atrophy_aura_lua_debuff:IsDebuff() return true end
...
function modifier_mars_atrophy_aura_lua_stack:IsHidden() return true end
function modifier_mars_atrophy_aura_lua_stack:IsPurgable() return false end
function modifier_mars_atrophy_aura_lua_stack:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE end
