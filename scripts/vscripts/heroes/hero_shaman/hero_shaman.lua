...
function modifier_shaman_hex_thinker:IsHidden() return true end
...
function modifier_shaman_hex_thinker:OnIntervalThink()
    ...
    local spawn_hex = CreateUnitByName("npc_shaman_hex", point + RandomVector(RandomInt(50, 150)), true, caster, caster, caster:GetTeamNumber())
...
function modifier_shaman_hex_logic:IsHidden() return false end
function modifier_shaman_hex_logic:IsPurgable() return false end
...
function modifier_shaman_shackles:OnCreated()
    ...
    self.nFXIndex = ParticleManager:CreateParticle("particles/units/heroes/hero_shadowshaman/shadowshaman_shackle.vpcf", PATTACH_POINT_FOLLOW, parent)
    ParticleManager:SetParticleControlEnt(self.nFXIndex, 0, caster, PATTACH_POINT_FOLLOW, "attach_attack1", caster:GetAbsOrigin(), true)
    ParticleManager:SetParticleControlEnt(self.nFXIndex, 1, parent, PATTACH_POINT_FOLLOW, "attach_hitloc", parent:GetAbsOrigin(), true)
    ParticleManager:SetParticleControlEnt(self.nFXIndex, 4, parent, PATTACH_POINT_FOLLOW, "attach_hitloc", parent:GetAbsOrigin(), true)
    ParticleManager:SetParticleControlEnt(self.nFXIndex, 5, caster, PATTACH_POINT_FOLLOW, "attach_attack2", caster:GetAbsOrigin(), true)
    ParticleManager:SetParticleControlEnt(self.nFXIndex, 6, parent, PATTACH_POINT_FOLLOW, "attach_hitloc", parent:GetAbsOrigin(), true)
    ...
    local damage = ability:GetSpecialValueFor("total_damage") + caster:ExtraIntelligenceDamage() * ability:GetSpecialValueFor("ExtraIntelligenceDamage") 
...
function modifier_shadow_shaman_ward_logic:IsHidden() return true end
function modifier_shadow_shaman_ward_logic:IsPurgable() return false end
function modifier_shadow_shaman_ward_logic:GetAttributes() return MODIFIER_ATTRIBUTE_PERMANENT end
