LinkLuaModifier( "modifier_earthshaker_fissure_lua_thinker", "heroes/hero_earthshaker/hero_earthshaker", LUA_MODIFIER_MOTION_NONE )
...
LinkLuaModifier( "modifier_earthshaker_enchant_totem_lua", "heroes/hero_earthshaker/hero_earthshaker", LUA_MODIFIER_MOTION_NONE )
	...
	if IsServer() then
		...
		if talent ~= nil and talent:GetLevel() > 0 then
			if params.attacker == self:GetParent() and (not self:GetParent():IsIllusion()) and not self:GetParent():PassivesDisabled() then
				...
				DoCleaveAttack( self:GetParent(), params.target, self:GetAbility(), params.damage, 360, 150, 360, 'particles/econ/items/sven/sven_ti7_sword/sven_ti7_sword_spell_great_cleave.vpcf' )
				end
			end
		end
		self:Destroy()
...
LinkLuaModifier( "modifier_earthshaker_aftershock_lua", "heroes/hero_earthshaker/hero_earthshaker", LUA_MODIFIER_MOTION_NONE )
