...
function phantom_assassin_knifes:OnProjectileHit( target, vLocation )
    ...
    if target ~= nil then
        local modifier = self:GetCaster():AddNewModifier( self:GetCaster(), self, "modifier_phantom_assassin_knifes_attack", {} )
...
LinkLuaModifier( "modifier_phantom_assassin_phantom_strike_crit", "heroes/hero_phantom/hero_phantom", LUA_MODIFIER_MOTION_NONE )
...
function phantom_assassin_phantom_strike_lua:OnSpellStart()
	...
	if target:GetModelName() == "models/items/earthshaker/totem_dragon_wall/fissure_body.vmdl" or target:GetModelName() == "models/machinegun_new.vmdl" then return end
...
function phantom_assassin_phantom_strike_lua:PlayEffects( origin )
	local effect_cast_start = ParticleManager:CreateParticle("particles/units/heroes/hero_phantom_assassin/phantom_assassin_phantom_strike_start.vpcf", PATTACH_WORLDORIGIN, self:GetCaster())
...
function modifier_blur_active:IsHidden() return false end
...
function modifier_blur_active:GetModifierInvisibilityLevel() return 1 end
...
function modifier_blur_aura:IsHidden() return true end
function modifier_blur_aura:IsPurgable() return false end
...
function modifier_blur_aura:GetAuraSearchFlags() return DOTA_UNIT_TARGET_FLAG_NONE end
function modifier_blur_aura:GetAuraSearchTeam() return DOTA_UNIT_TARGET_TEAM_FRIENDLY end
function modifier_blur_aura:GetAuraSearchType() return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC end
function modifier_blur_aura:GetModifierAura() return "modifier_blur_passive" end
function modifier_blur_aura:IsAura() return true end

-------------------------------------------------------------------------------

modifier_blur_passive = class({})

function modifier_blur_passive:IsHidden() return false end
function modifier_blur_passive:IsPurgable() return false end
...
LinkLuaModifier( "modifier_phantom_assassin_coup_de_grace_lua", "heroes/hero_phantom/hero_phantom", LUA_MODIFIER_MOTION_NONE )
...
function modifier_phantom_assassin_coup_de_grace_lua:IsHidden() return true end
function modifier_phantom_assassin_coup_de_grace_lua:IsPurgable() return false end
