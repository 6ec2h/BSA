...
LinkLuaModifier("modifier_lua_dark_seer_vacuum", "heroes/hero_dark_seer/hero_dark_seer.lua", LUA_MODIFIER_MOTION_HORIZONTAL)
LinkLuaModifier("modifier_lua_dark_seer_vacuum_wormhole", "heroes/hero_dark_seer/hero_dark_seer.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_lua_dark_seer_vacuum_refresh_tracker", "heroes/hero_dark_seer/hero_dark_seer.lua", LUA_MODIFIER_MOTION_NONE)
...
function lua_dark_seer_vacuum:OnSpellStart()
	...
	local enemies = FindUnitsInRadius(self:GetCaster():GetTeamNumber(), position, nil, self:GetSpecialValueFor("radius"), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_INVULNERABLE, FIND_ANY_ORDER, false)	
...
function modifier_lua_dark_seer_vacuum:IsDebuff() return true end
function modifier_lua_dark_seer_vacuum:IgnoreTenacity()	return true end
...
LinkLuaModifier("modifier_lua_dark_seer_ion_shell", "heroes/hero_dark_seer/hero_dark_seer.lua", LUA_MODIFIER_MOTION_NONE)
...
function lua_dark_seer_ion_shell:OnSpellStart()
	...
	local special_bonus_unique_dark_seer_6 = self:GetCaster():FindAbilityByName("special_bonus_unique_dark_seer_6")
	if special_bonus_unique_dark_seer_6 and special_bonus_unique_dark_seer_6:GetLevel() > 0 then
		local units = FindUnitsInRadius(self:GetCaster():GetTeamNumber(), self:GetCursorPosition(), nil, 250, DOTA_UNIT_TARGET_TEAM_BOTH, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NONE, FIND_ANY_ORDER, false)
		for _,unit in pairs(units) do
			unit:AddNewModifier(self:GetCaster(), self, "modifier_lua_dark_seer_ion_shell", {duration = self:GetSpecialValueFor("duration")})
			unit:EmitSound("Hero_Dark_Seer.Ion_Shield_Start")
		end
	else
		self:GetCursorTarget():AddNewModifier(self:GetCaster(), self, "modifier_lua_dark_seer_ion_shell", {duration = self:GetSpecialValueFor("duration")})
...
function modifier_lua_dark_seer_ion_shell:OnIntervalThink()
	...
	local enemies = FindUnitsInRadius(self:GetCaster():GetTeamNumber(), self:GetParent():GetAbsOrigin(), self:GetParent(), self.radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NONE, FIND_ANY_ORDER, false)
	for _, enemy in pairs(enemies) do
		if enemy ~= self:GetParent() then
...
LinkLuaModifier("modifier_lua_dark_seer_surge_slow", "heroes/hero_dark_seer/hero_dark_seer.lua", LUA_MODIFIER_MOTION_NONE)
...
function lua_dark_seer_surge:OnSpellStart()
	...
	for _, enemy in pairs(FindUnitsInRadius(self:GetCaster():GetTeamNumber(), position, nil, radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NONE, FIND_ANY_ORDER, false)) do
		enemy:EmitSound("Hero_Dark_Seer.Surge_Sonic_Boom")
		
		enemy:AddNewModifier(self:GetCaster(), self, "modifier_lua_dark_seer_surge_slow", {duration = duration * (1 - enemy:GetStatusResistance())})
...
LinkLuaModifier("modifier_lua_dark_seer_wall_of_replica", "heroes/hero_dark_seer/hero_dark_seer.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_lua_dark_seer_wall_of_replica_pw", "heroes/hero_dark_seer/hero_dark_seer.lua", LUA_MODIFIER_MOTION_NONE)
...
function lua_dark_seer_wall_of_replica:OnSpellStart()
    ...
    CreateModifierThinker(caster, self, "modifier_lua_dark_seer_wall_of_replica", {duration = duration}, pos, caster:GetTeamNumber(), false)
    CreateModifierThinker(caster, self, "modifier_lua_dark_seer_wall_of_replica_pw", {duration = duration}, pos, caster:GetTeamNumber(), false)
end
