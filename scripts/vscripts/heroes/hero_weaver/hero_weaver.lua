...
function weaver_shukuchi_custom:OnSpellStart()
    ...
    caster:AddNewModifier(caster, self, "modifier_weaver_shukuchi_custom", {duration = self:GetSpecialValueFor("duration")})
end
...
function modifier_weaver_shukuchi_custom:OnIntervalThink()
    ...
    for _, enemy in pairs(enemies) do
        ...
        if not ability.hero_hit_targets[entIndex] then
            ...
            local p = ParticleManager:CreateParticle("particles/units/heroes/hero_weaver/weaver_shukuchi_damage.vpcf", PATTACH_ABSORIGIN_FOLLOW, enemy)
...
function modifier_weaver_shukuchi_custom:CreateWallSegment(vStart, vEnd)
    ...
    CreateModifierThinker(
        self:GetCaster(),
        ability,
        "modifier_weaver_shukuchi_thread_node",
        { 
            duration = ability:GetSpecialValueFor("thread_duration"), 
            start_x = vStart.x, start_y = vStart.y, start_z = vStart.z,
            end_x = vEnd.x,   end_y = vEnd.y,   end_z = vEnd.z
        },
        vStart,
...
function modifier_weaver_shukuchi_custom:GetModifierMoveSpeedBonus_Constant() return self.speed end
function modifier_weaver_shukuchi_custom:GetModifierInvisibilityLevel() return 1 end
function modifier_weaver_shukuchi_custom:OnAttack(keys)
    if keys.attacker == self:GetParent() then self:Destroy() end
end
function modifier_weaver_shukuchi_custom:OnAbilityFullyCast(keys)
    if keys.unit == self:GetParent() and keys.ability ~= self:GetAbility() then self:Destroy() end
end
...
function modifier_weaver_geminate_attack_custom:OnAttack(params)
    ...
    for _, enemy in pairs(candidates) do
        if enemy ~= params.target then
            if count >= extra_targets then break end
            count = count + 1
            local e = enemy
...
function weaver_the_swarm_custom:OnSpellStart()
    ...
    for beetles = 1, self:GetSpecialValueFor("count") do
        local start_pos = self:GetCaster():GetAbsOrigin() + RandomVector(RandomInt(0, self:GetSpecialValueFor("spawn_radius")))
        local beetle_dummy = CreateModifierThinker(self:GetCaster(), self, nil, {}, self:GetCaster():GetAbsOrigin(), self:GetCaster():GetTeamNumber(), false)
        if beetles == 1 then
...
function weaver_the_swarm_custom:OnProjectileHit_ExtraData(target, location, data)
    ...
    if not beetle_dummy or beetle_dummy:IsNull() then return end
...
function modifier_weaver_the_swarm_custom_unit:GetOverrideAnimation() return ACT_DOTA_IDLE end
function modifier_weaver_the_swarm_custom_unit:GetAbsoluteNoDamageMagical() return 1 end
function modifier_weaver_the_swarm_custom_unit:GetAbsoluteNoDamagePhysical() return 1 end
function modifier_weaver_the_swarm_custom_unit:GetAbsoluteNoDamagePure() return 1 end
...
function modifier_weaver_the_swarm_custom_larva:IsDebuff() return true end
function modifier_weaver_the_swarm_custom_larva:IsPurgable() return false end
...
function weaver_time_lapse_custom:GetBehavior()
    local base = DOTA_ABILITY_BEHAVIOR_DONT_RESUME_MOVEMENT + DOTA_ABILITY_BEHAVIOR_DONT_RESUME_ATTACK + DOTA_ABILITY_BEHAVIOR_IGNORE_BACKSWING
...
function weaver_time_lapse_custom:OnSpellStart()
    ...
    if target and target ~= self:GetCaster() then
        ...
        if track and track.instances_health and track.instances_health[1] then
            ...
            local p = ParticleManager:CreateParticle("particles/units/heroes/hero_weaver/weaver_timelapse.vpcf", PATTACH_WORLDORIGIN, target)
...
function modifier_weaver_time_lapse_custom:IsAura() return true end
function modifier_weaver_time_lapse_custom:GetModifierAura() return "modifier_weaver_time_lapse_ally_tracking" end
function modifier_weaver_time_lapse_custom:GetAuraRadius() return 99999 end
function modifier_weaver_time_lapse_custom:GetAuraSearchTeam() return DOTA_UNIT_TARGET_TEAM_FRIENDLY end
function modifier_weaver_time_lapse_custom:GetAuraSearchType() return DOTA_UNIT_TARGET_HERO end
function modifier_weaver_time_lapse_custom:GetAuraEntityReject(entity)
    if entity == self:GetParent() then return true end
    local talent = self:GetCaster():FindAbilityByName("special_bonus_unique_weaver_7")
...
function modifier_weaver_time_lapse_ally_tracking:IsHidden() return true end
function modifier_weaver_time_lapse_ally_tracking:IsPurgable() return false end
function modifier_weaver_time_lapse_ally_tracking:RemoveOnDeath() return false end
...
function modifier_weaver_time_lapse_phantom:IsHidden()     return true  end
function modifier_weaver_time_lapse_phantom:IsDebuff()     return false end
function modifier_weaver_time_lapse_phantom:IsStunDebuff() return false end
function modifier_weaver_time_lapse_phantom:IsPurgable()   return false end
