LinkLuaModifier( "modifier_hero_destroyer_totem_thinker", "heroes/hero_destroyer/hero_destroyer", LUA_MODIFIER_MOTION_NONE )
...
function hero_destroyer_totem:OnSpellStart()
	...
	local center = CreateModifierThinker( self:GetCaster(), self, "modifier_hero_destroyer_totem_thinker", { duration = duration +0.2}, self.position, self:GetCaster():GetTeamNumber(), false )
...
function modifier_hero_destroyer_totem_thinker:OnIntervalThink()
	...
	for _,unit in pairs(all_units) do
		if unit:GetTeamNumber() == self:GetCaster():GetTeamNumber() then
			...
			SendOverheadEventMessage( unit:GetPlayerOwner(), OVERHEAD_ALERT_HEAL , unit, unit:GetMaxHealth()/100 * regen, nil )
		else
			local damageTable = {
				victim = unit,
...
LinkLuaModifier("modifier_hero_destroyer_second_skill_armor", "heroes/hero_destroyer/hero_destroyer", LUA_MODIFIER_MOTION_NONE)
...
LinkLuaModifier("modifier_hero_destroyer_second_skill_hp", "heroes/hero_destroyer/hero_destroyer", LUA_MODIFIER_MOTION_NONE)
...
LinkLuaModifier("modifier_hero_destroyer_second_skill_resist", "heroes/hero_destroyer/hero_destroyer", LUA_MODIFIER_MOTION_NONE)
...
LinkLuaModifier("modifier_hero_destroyer_third_skill_debuff", "heroes/hero_destroyer/hero_destroyer", LUA_MODIFIER_MOTION_NONE)
...
function modifier_hero_destroyer_third_skill:OnAttackLanded(keys)
	...
	if self:GetParent() == keys.attacker and keys.attacker:GetTeamNumber() ~= keys.target:GetTeamNumber() and not self:GetParent():PassivesDisabled() then
		...
		for _,enemy in pairs(enemies) do
			enemy:AddNewModifier(self:GetParent(), ability, "modifier_hero_destroyer_third_skill_debuff", {duration = ability:GetSpecialValueFor("duration")})
			if keys.target ~= enemy then
				ApplyDamage({victim = enemy, attacker = self:GetParent(), damage = damage, damage_type = DAMAGE_TYPE_PHYSICAL, damage_flags = DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION + DOTA_DAMAGE_FLAG_DONT_DISPLAY_DAMAGE_IF_SOURCE_HIDDEN, ability = ability})
			end
		end
...
function modifier_hero_destroyer_overkill:IsHidden() return true end
...
function modifier_hero_destroyer_overkill:OnTakeDamage(params)
    ...
    if current_health <= 0 then
        ...
        local particle = ParticleManager:CreateParticle("particles/units/heroes/hero_elder_titan/elder_titan_echo_stomp_physical.vpcf", PATTACH_ABSORIGIN, target)
