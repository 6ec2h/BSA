...
function dado_storm_lua:OnSpellStart()
    ...
    local thinker = CreateModifierThinker(caster, self, "modifier_dado_storm_lua_thinker", {}, caster:GetOrigin(), caster:GetTeamNumber(), false)
...
function modifier_dado_storm_lua_thinker:IsHidden() return true end
function modifier_dado_storm_lua_thinker:IsPurgable() return false end
...
function dado_tp_out:OnSpellStart()
    ...
    self.last_thinker = CreateModifierThinker(
        caster, 
        self, 
        "modifier_dado_tp_out_thinker", 
        {},
        position, 
        caster:GetTeamNumber(), 
        false
...
function modifier_dado_ampl:IsHidden() return false end
function modifier_dado_ampl:IsPurgable() return false end
...
function modifier_dado_ampl:OnCreated( kv )
    ...
    if IsServer() then
        local field_fx = ParticleManager:CreateParticle("particles/dado_buff.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.parent)
...
function modifier_dado_ampl:OnTakeDamage( keys )
    ...
    if self.hit_counter >= self.attack_life then
        ...
        local back_fx = ParticleManager:CreateParticle("particles/units/heroes/hero_faceless_void/faceless_void_backtrack.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.parent)
...
function modifier_dado_passive:IsHidden() return true end
function modifier_dado_passive:IsPurgable() return false end
