...
function sand_stun:PlayEffects()
	local effect_cast = ParticleManager:CreateParticle("particles/sandking.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetCaster() )
...
LinkLuaModifier( "modifier_sand_king_sand_storm_lua_debuff", "heroes/hero_sand_king/hero_sand_king", LUA_MODIFIER_MOTION_NONE )

sand_king_sand_storm_lua = class({})

function sand_king_sand_storm_lua:Precache(context)
    PrecacheResource( "particle", "particles/instanced/pudge_desert_sequence/shot3b2/sandking_sandstorm_eruption_dust_low_instanced.vpcf", context )
end
...
function sand_king_sand_storm_lua:OnSpellStart()
	...
	sand_king_sand_storm_lua.mod = CreateModifierThinker(caster, self, "modifier_sand_king_sand_storm_lua", { duration = duration }, position, caster:GetTeamNumber(), false)
...
function modifier_sand_king_sand_storm_lua:OnIntervalThink()
	...
	if self.damage_counter == 16 then
		...
		for _,enemy in pairs(enemies) do
			...
			if talent ~= nil and talent:GetLevel() > 0 then 
				enemy:AddNewModifier(self:GetCaster(), self, "modifier_sand_king_sand_storm_lua_debuff", {duration = 3})
			end
		end
	end
...
function modifier_sand_king_sand_storm_lua:PlayEffects( radius )
	self.effect_cast = ParticleManager:CreateParticle( "particles/units/heroes/hero_sandking/sandking_sandstorm.vpcf", PATTACH_WORLDORIGIN, nil )
	ParticleManager:SetParticleControl( self.effect_cast, 0, self:GetCaster():GetOrigin() )
...
LinkLuaModifier( "modifier_sandking_custom_trembling_waves", "heroes/hero_sand_king/hero_sand_king", LUA_MODIFIER_MOTION_NONE )
...
function modifier_sandking_custom_trembling_waves:OnIntervalThink()
	if IsServer() and self:GetAbility() and self.caster:IsRealHero() and self.caster:IsAlive() and not self.caster:PassivesDisabled() and self:GetAbility():IsCooldownReady() then
		...
		Timers:CreateTimer(0.01, function() 
			self.caster.ShieldParticle = ParticleManager:CreateParticle("particles/units/heroes/hero_sandking/sandking_epicenter.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.caster)
			ParticleManager:SetParticleControl(self.caster.ShieldParticle, 1, Vector(radius,0,radius))
			ParticleManager:SetParticleControlEnt(self.caster.ShieldParticle, 0, self.caster, PATTACH_POINT_FOLLOW, "attach_hitloc", self.caster:GetAbsOrigin(), true)
		end)
