...
function modifier_return_passive:OnTakeDamage(keys)
	if IsServer() and self:GetAbility() then
		...
		if self:GetCaster():FindAbilityByName("npc_dota_hero_centaur_str11")~=nil then
			if self:GetCaster():FindAbilityByName("npc_dota_hero_centaur_str11"):GetLevel() > 0 then 
