	...
	if IsServer() and self:GetAbility() then
		...
		if attacker and attacker:GetTeamNumber() ~= parent:GetTeamNumber() and parent == target and not attacker:IsOther() and attacker:GetName() ~= "npc_dota_unit_undying_zombie" and not attacker:IsBuilding() then
			if keys.damage_category == 1 then
				local particle_return_fx = ParticleManager:CreateParticle("particles/units/heroes/hero_centaur/centaur_return.vpcf", PATTACH_ABSORIGIN, parent)
...
function modifier_centaur_stampede_lua_debuff:OnDestroy( kv )
end
