LinkLuaModifier("modifier_outworld_devourer_flux_lua", "heroes/hero_outworld_devourer/outworld_devourer_flux_lua/outworld_devourer_flux_lua", LUA_MODIFIER_MOTION_NONE)
...
function modifier_outworld_devourer_flux_lua:IsHidden()		return true end
function modifier_outworld_devourer_flux_lua:IsPurgable()	return false end
function modifier_outworld_devourer_flux_lua:RemoveOnDeath()	return false end
...
function modifier_outworld_devourer_flux_lua:OnAbilityFullyCast(keys)
	if keys.unit == self:GetParent() and not keys.ability:IsToggle() and not keys.ability:IsItem() then
		...
		if RandomInt(1,100) < chance then
			...
			self:GetParent():GiveMana(self:GetParent():GetMaxMana() * self:GetAbility():GetSpecialValueFor("mp_back") * 0.01)
		end
	end
end