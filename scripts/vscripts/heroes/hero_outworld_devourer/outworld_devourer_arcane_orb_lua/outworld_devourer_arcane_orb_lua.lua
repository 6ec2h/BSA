...
function outworld_devourer_arcane_orb_lua:OnOrbFire( params )
	...
	if abil ~= nil and abil:GetLevel() > 0 then	
		...
		if RandomInt(1,100) < chance then
			self.proc_particle = ParticleManager:CreateParticle("particles/units/heroes/hero_obsidian_destroyer/obsidian_destroyer_essence_effect.vpcf", PATTACH_ABSORIGIN, self:GetCaster())
			ParticleManager:SetParticleControlEnt(self.proc_particle, 0, self:GetCaster(), PATTACH_ABSORIGIN_FOLLOW, "attach_hitloc", self:GetCaster():GetAbsOrigin(), true)
