...
function PlayersSummary:Init()
	if PlayersSummary.ready then return end
	
	CustomGameEventManager:RegisterListener("RequestPlayersSummary", function(_, data) self:RequestPlayersSummary(data) end)
	GameRules:GetGameModeEntity():SetModifyExperienceFilter(Dynamic_Wrap(PlayersSummary, "ModifyExperienceFilter" ), self)

	PlayersSummary.ready = true
end

function PlayersSummary:InitPlayerHero(playerId)
	if self.data[playerId] then return self.data[playerId] end
...
function PlayersSummary:SyncPlayersSummaryWithClient(loseReason)
	...
	CustomNetTables:SetTableValue("players_summary", "extra_data", {difficulty = Game_Difficulty, loseReason = LoseReason})
end
...
function PlayersSummary:HandleDamage(handleType, playerId, damage, damageType, originalDamage)
	...
	if not playerSummaryDamageCategory then return end
	...
	if damageKey == "pure" then
		...
		playerSummaryDamageCategory[damageKey .. "Original"] = playerSummaryDamageCategory[damageKey .. "Original"] + originalDamage
...
PlayersSummary.CDOTA_BaseNPC_Hero_ModifyGold = PlayersSummary.CDOTA_BaseNPC_Hero_ModifyGold or CDOTA_BaseNPC_Hero.ModifyGold
...
function CDOTA_BaseNPC_Hero:ModifyGold(goldChange, reliable, reason)
	...
	if goldChange <= 0 then return end
...
function PlayersSummary:ModifyExperienceFilter(data)
	...
	if playerId < 0 or playerId > 4 then return end
...
function PlayersSummary:UpdatePlayersAccountLevel()
	if not Account_stats then return end
