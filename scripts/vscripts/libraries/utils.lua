...
local batManipulateFuncs = {
	"GetModifierBaseAttackTimeConstant",
	"GetModifierBaseAttackTimeConstant_Adjust",
	"GetModifierBaseAttackTimePercentage",
}

function CDOTA_BaseNPC:GetOriginalBaseAttackTime()
	local modBatManipulateFuncs = {}

	for i = 0, self:GetModifierCount() - 1 do
		local modName = self:GetModifierNameByIndex(i)
		if not modName then
			goto continue
		end

		local mod = self:FindModifierByName(modName)
		if not mod then
			goto continue
		end

		for i = 1, #batManipulateFuncs do
			local funcName = batManipulateFuncs[i]
				
			local batManipulateFunc = mod[funcName]
			if batManipulateFunc then
				modBatManipulateFuncs[mod] = modBatManipulateFuncs[mod] or {}
				modBatManipulateFuncs[mod][funcName] = batManipulateFunc
				mod[funcName] = function() end
			end
		end
	...
	local bat = self:GetBaseAttackTime()

	for mod, funcs in pairs(modBatManipulateFuncs) do
