...
function guild_events:IsAnyEventActiveForPlayer(unit)
    if _G.guild_event_team then return true end
...
function guild_events:EventTeleport(hero, point_name, status)
    if not hero or hero:IsNull() or not status then return end
...
function guild_events:StartTeamEventLogic()
    ...
    if not enemy_point_ent then return end
    local enemy_point = enemy_point_ent:GetAbsOrigin()

    Timers:CreateTimer(5, function()
        if not _G.guild_event_team then return nil end
        ...
        for i = #self.event_units, 1, -1 do
            ...
            if not u or u:IsNull() or not u:IsAlive() then table.remove(self.event_units, i) end
        end
...
function guild_events:CheckTeamEventStatus()
    ...
    for _, hero in pairs(heroes) do
        if hero:IsRealHero() and hero:GetTeamNumber() == DOTA_TEAM_GOODGUYS then
            if not hero:HasModifier("modifier_wait") then return false end
        end
    end
    return true
end
...
function guild_events:CanStartSoloEvent(nPlayerID)
    if not PlayerResource:IsValidPlayerID(nPlayerID) then return end
...
function guild_events:StartSoloEventLogic(nPlayerID)
    ...
    if not spawn_ent then return end
    local enemy_point = spawn_ent:GetAbsOrigin()

    Timers:CreateTimer(5, function()
        if not self.solo_active_players[nPlayerID] then return nil end

        for i = #self.player_units[nPlayerID], 1, -1 do
            local u = self.player_units[nPlayerID][i]
            if not u or u:IsNull() or not u:IsAlive() then table.remove(self.player_units[nPlayerID], i) end
        end
