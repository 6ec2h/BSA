...
function pet_orchid:OnSpellStart()
	...
	target:AddNewModifier(self:GetCaster(), self, "modifier_pet_orchid_passive_debuff", {duration = self:GetSpecialValueFor("duration") * (1 - target:GetStatusResistance())})
end

-----------------------------------------------------------------------------------------------------------

modifier_pet_orchid_passive_debuff = modifier_pet_orchid_passive_debuff or class({})
function modifier_pet_orchid_passive_debuff:IsHidden() return false end
function modifier_pet_orchid_passive_debuff:IsDebuff() return true end
function modifier_pet_orchid_passive_debuff:IsPurgable() return true end
