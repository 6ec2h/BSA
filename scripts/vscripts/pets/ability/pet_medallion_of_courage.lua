LinkLuaModifier("modifier_pet_medallion_of_courage_passive_debuff", "pets/ability/pet_medallion_of_courage", LUA_MODIFIER_MOTION_NONE)
...
function pet_medallion_of_courage:OnSpellStart()
	...
	target:AddNewModifier(self:GetCaster(), self, "modifier_pet_medallion_of_courage_passive_debuff", {duration = self:GetSpecialValueFor("duration") * (1 - target:GetStatusResistance())})
end

-----------------------------------------------------------------------------------------------------------

modifier_pet_medallion_of_courage_passive_debuff = modifier_pet_medallion_of_courage_passive_debuff or class({})
function modifier_pet_medallion_of_courage_passive_debuff:IsHidden() return false end
function modifier_pet_medallion_of_courage_passive_debuff:IsDebuff() return true end
function modifier_pet_medallion_of_courage_passive_debuff:IsPurgable() return true end
