...
function boss_ursa_earthshock_lua:OnSpellStart()
    ...
    local shock_pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_ursa/ursa_earthshock.vpcf", PATTACH_WORLDORIGIN, nil)
    ...
    Timers:CreateTimer(0.03, function()
        ...
        local wave_enemies = FindUnitsInRadius(caster:GetTeamNumber(), caster:GetAbsOrigin(), nil, current_wave_radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_ALL, 0, 0, false)
            ...
            if not enemy:HasModifier("modifier_ursa_earthshock_hit_by_wave") then
                ...
                if swipes_ability then
                    ...
                    if modifier then
                        ...
                        enemy:AddNewModifier(caster, swipes_ability, "modifier_ursa_fury_swipes_debuff_lua", {duration = swipes_ability:GetSpecialValueFor("bonus_reset_time")})
                    end
                end
            end
        end
        if current_wave_radius < wave_max_radius then return 0.03 end
    end)
end
...
function boss_ursa_roar_lua:OnSpellStart()
    ...
    local roar_pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_beastmaster/beastmaster_primal_roar.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster)
    ...
    if furthest_enemy then
        ...
        Timers:CreateTimer(0.5, function()
            ...
            local hit_pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_ursa/ursa_fury_swipes.vpcf", PATTACH_ABSORIGIN_FOLLOW, furthest_enemy)
		...
		if params.attacker == self:GetParent() and ( not self:GetParent():IsIllusion() ) then
			...
			if target ~= nil and target:GetTeamNumber() ~= self:GetParent():GetTeamNumber() then
                if self:GetStackCount() == 3 then
                    DoCleaveAttack( params.attacker, target, self:GetAbility(), params.damage, 150, 360, 400, "particles/units/heroes/hero_sven/sven_spell_great_cleave.vpcf" )
                end
			end
		end
