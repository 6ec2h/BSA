...
LinkLuaModifier("modifier_boss_slardar_slithereen_crush_lua_slow", "abilities/bosses/slardar/slardar", LUA_MODIFIER_MOTION_NONE )
...
function boss_slardar_slithereen_crush_lua:OnSpellStart()
	...
	for _,enemy in pairs(enemies) do
		...
		enemy:AddNewModifier( self:GetCaster(), self, "modifier_boss_slardar_slithereen_crush_lua_slow", { duration = duration } )
	end
	self:PlayEffects()
end
...
function boss_slardar_slithereen_crush_lua:PlayEffects()
	local radius = self:GetSpecialValueFor("crush_radius")
	local nFXIndex = ParticleManager:CreateParticle( "particles/units/heroes/hero_slardar/slardar_crush.vpcf", PATTACH_WORLDORIGIN, nil )
...
LinkLuaModifier("modifier_boss_slardar_torrential_waters", "abilities/bosses/slardar/slardar", LUA_MODIFIER_MOTION_VERTICAL)
...
function boss_slardar_torrential_waters:OnSpellStart()    
    local duration = self:GetSpecialValueFor("duration") 
    self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_boss_slardar_torrential_waters", {duration = duration})
end
...
function modifier_boss_slardar_torrential_waters:IsHidden() return false end
function modifier_boss_slardar_torrential_waters:IsPurgable() return false end
...
function modifier_boss_slardar_torrential_waters:OnIntervalThink()  
    ...
    local indicator_pfx = ParticleManager:CreateParticle("particles/ui_mouseactions/range_display.vpcf", PATTACH_WORLDORIGIN, nil)
    ...
    Timers:CreateTimer(delay, function()
        ...
        local particle_blast_fx = ParticleManager:CreateParticle("particles/econ/items/kunkka/divine_anchor/hero_kunkka_dafx_skills/kunkka_spell_torrent_splash_fxset.vpcf", PATTACH_WORLDORIGIN, nil)
...
function modifier_boss_slardar_sprint_buff:IsHidden() return true end
function modifier_boss_slardar_sprint_buff:IsPurgable() return false end
...
function modifier_water_blast_push:UpdateHorizontalMotion(me, dt)
    ...
    if GridNav:IsTraversable(next_pos) and not GridNav:IsBlocked(next_pos) then
        ...
        local max_damage = self.ability:GetSpecialValueFor("damage") + self.ability:GetSpecialValueFor("diff_boost_damage")
...
function boss_slardar_bubble:OnSpellStart()
    ...
    local units = FindUnitsInRadius(caster:GetTeamNumber(), caster:GetOrigin(), nil, radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, 0, 0, false)
