	...
	if #enemies > 0 then
		for _, enemy in pairs( enemies ) do
			if not enemy:TriggerSpellAbsorb(self) then
				...
				enemy:AddNewModifier(self:GetCaster(), self, "modifier_boss_arc_flux", {duration = self:GetSpecialValueFor("duration")})
			end
		end	
	end	
end
...
function modifier_boss_arc_flux:OnCreated()
	...
	self.damage_per_second		= self:GetAbility():GetSpecialValueFor("damage_per_second") + self:GetAbility():GetSpecialValueFor("diff_boost_damage")
...
LinkLuaModifier("modifier_boss_arc_magnetic_field_thinker_evasion", "abilities/bosses/arc/arc", LUA_MODIFIER_MOTION_NONE)
...
function modifier_boss_arc_magnetic_field_thinker_evasion:IsAura()						return true end
function modifier_boss_arc_magnetic_field_thinker_evasion:IsAuraActiveOnDeath() 		return false end
function modifier_boss_arc_magnetic_field_thinker_evasion:GetAuraDuration()				return 0.1 end
function modifier_boss_arc_magnetic_field_thinker_evasion:GetAuraRadius()				return self.radius end
function modifier_boss_arc_magnetic_field_thinker_evasion:GetAuraSearchFlags()			return DOTA_UNIT_TARGET_FLAG_NONE end
function modifier_boss_arc_magnetic_field_thinker_evasion:GetAuraSearchTeam()			return DOTA_UNIT_TARGET_TEAM_FRIENDLY end
function modifier_boss_arc_magnetic_field_thinker_evasion:GetAuraSearchType()			return DOTA_UNIT_TARGET_ALL end
function modifier_boss_arc_magnetic_field_thinker_evasion:GetModifierAura()				return "modifier_boss_arc_magnetic_field_evasion" end

-------------------------------------------------------------------------------------

modifier_boss_arc_magnetic_field_evasion = class({})

function modifier_boss_arc_magnetic_field_evasion:GetAttributes()	return MODIFIER_ATTRIBUTE_MULTIPLE end
...
function modifier_boss_arc_magnetic_field_evasion:GetModifierEvasion_Constant(keys)
	if keys.attacker and self:GetAuraOwner() and self:GetAuraOwner():HasModifier("modifier_boss_arc_magnetic_field_thinker_evasion") and self:GetAuraOwner():FindModifierByName("modifier_boss_arc_magnetic_field_thinker_evasion").radius and (keys.attacker:GetAbsOrigin() - self:GetAuraOwner():GetAbsOrigin()):Length2D() > self:GetAuraOwner():FindModifierByName("modifier_boss_arc_magnetic_field_thinker_evasion").radius then
		return 100
	end
end
...
function boss_arc_spark_wraith:Precache( context )
	...
	PrecacheResource( "particle", "particles/econ/items/arc_warden/arc_warden_ti9_immortal/arc_warden_ti9_wraith_prj_burst.vpcf", context )
...
function boss_arc_spark_wraith:OnProjectileHit_ExtraData(target, location, ExtraData)
	if target then
		if not target:IsMagicImmune() then
			...
			target:AddNewModifier(self:GetCaster(), self, "modifier_boss_arc_spark_wraith_purge", {duration = self:GetSpecialValueFor("tick") * (1 - target:GetStatusResistance())})
		end
		return true
	end
end
...
function modifier_boss_arc_spark_wraith_thinker:OnCreated()
	...
	self.spark_damage		= self:GetAbility():GetSpecialValueFor("spark_damage") + self:GetAbility():GetSpecialValueFor("diff_boost_damage")
...
function boss_arc_hide_lua:JumpToTarget()
    ...
    local enemies = FindUnitsInRadius(
        caster:GetTeamNumber(), caster:GetAbsOrigin(), nil, search_radius,
    ...
    local ufoMoveIndex = ParticleManager:CreateParticle("particles/ui_mouseactions/range_display.vpcf", PATTACH_WORLDORIGIN, nil)
    ...
    caster:SetContextThink(DoUniqueString("Impact"), function()
        ...
        local expFX = ParticleManager:CreateParticle("particles/heroes/nue/ability_nue_04.vpcf", PATTACH_WORLDORIGIN, nil)
        ...
        local targets = FindUnitsInRadius(
            caster:GetTeamNumber(), targetPoint, nil, impact_radius,
            DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC,
...
function modifier_boss_arc_hide_lua:IsHidden() return true end
function modifier_boss_arc_hide_lua:IsPurgable() return false end
function modifier_boss_arc_hide_lua:GetAttributes() return MODIFIER_ATTRIBUTE_PERMANENT end
...
function modifier_boss_arc_clone_passive:IsHidden() return true end
...
function modifier_boss_arc_clone_passive:SpawnClone()
    ...
    local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_arc_warden/arc_warden_tempest_cast.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster)
    ParticleManager:ReleaseParticleIndex(pfx)

    local clone = CreateUnitByName("npc_dota_boss_arc_warden_clone", caster:GetAbsOrigin() + RandomVector(200), true, caster, caster:GetOwner(), caster:GetTeamNumber())
...
function modifier_boss_arc_clone_target_logic:IsHidden() return true end
...
function modifier_boss_arc_clone_target_logic:OnTakeDamage(params)
    ...
    if params.unit:GetHealth() <= 1 then
        local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_arc_warden/arc_warden_tempest_death.vpcf", PATTACH_ABSORIGIN_FOLLOW, params.unit)
        ParticleManager:ReleaseParticleIndex(pfx)
...
function modifier_boss_arc_clone_immortal:IsHidden() return false end
function modifier_boss_arc_clone_immortal:GetTexture() return "arc_warden_tempest_double" end
