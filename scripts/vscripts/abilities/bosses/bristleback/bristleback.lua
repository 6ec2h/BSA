...
LinkLuaModifier("modifier_boss_bristleback_sand_shell_lua", "abilities/bosses/bristleback/bristleback", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_creep_sand_shell_debuff_lua", "abilities/bosses/bristleback/bristleback", LUA_MODIFIER_MOTION_NONE)
...
function modifier_boss_bristleback_sand_shell_lua:OnRefresh()
    ...
    self.armor_per_stack = ability:GetSpecialValueFor("armor_per_stack") + ability:GetSpecialValueFor("diff_boost_additional")
...
function modifier_boss_bristleback_sand_shell_lua:TriggerNova(dis_arm)
    ...
    local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_bristleback/bristleback_quill_spray.vpcf", PATTACH_ABSORIGIN, caster)
...
function modifier_creep_sand_shell_debuff_lua:IsDebuff() return true end
function modifier_creep_sand_shell_debuff_lua:IsPurgable() return true end
...
LinkLuaModifier("modifier_boss_bristleback_sand_mirage_lua", "abilities/bosses/bristleback/bristleback", LUA_MODIFIER_MOTION_NONE)
...
function modifier_boss_bristleback_sand_mirage_lua:OnCreated()
    ...
    self.pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_phantom_assassin/phantom_assassin_blur.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
end
...
LinkLuaModifier("modifier_boss_bristleback_goo_lua_thinker", "abilities/bosses/bristleback/bristleback", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_boss_bristleback_goo_lua_debuff", "abilities/bosses/bristleback/bristleback", LUA_MODIFIER_MOTION_NONE)
...
function modifier_boss_bristleback_goo_lua_debuff:OnCreated()
    ...
    self.armor_loss = self:GetAbility():GetSpecialValueFor("dis_arm") + self:GetAbility():GetSpecialValueFor("diff_boost_additional")
...
LinkLuaModifier("modifier_boss_bristleback_summon_swarm_debuff", "abilities/bosses/bristleback/bristleback", LUA_MODIFIER_MOTION_NONE)

boss_bristleback_summon_swarm = class({})

function boss_bristleback_summon_swarm:Precache( context )
end
...
function boss_bristleback_summon_swarm:OnSpellStart()
    ...
    for i = 1, beetle_count do
        local beetle = CreateUnitByName("npc_dota_boss_bristleback_minion", caster:GetAbsOrigin() + RandomVector(500), true, caster, caster, caster:GetTeamNumber())
...
LinkLuaModifier("modifier_boss_bristleback_summon_swarm_passive", "abilities/bosses/bristleback/bristleback", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_boss_bristleback_summon_swarm_debuff", "abilities/bosses/bristleback/bristleback", LUA_MODIFIER_MOTION_NONE)
...
function boss_bristleback_hairball_lua:OnProjectileHit(target, location)
    if not target then
        ...
        local units = FindUnitsInRadius(caster:GetTeamNumber(), location, nil, radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, 0, 0, false)
