...
function boss_medusa_mystic_snake_lua:OnProjectileHit_ExtraData(target, location, extraData)
    ...
    local impact_pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_medusa/medusa_mystic_snake_impact.vpcf", PATTACH_ABSORIGIN_FOLLOW, target)
    ...
    target:AddNewModifier(caster, self, "modifier_boss_medusa_mystic_snake_lua", { duration = self:GetSpecialValueFor("duration") })
...
function modifier_boss_medusa_mystic_snake_lua:IsHidden() return false end
function modifier_boss_medusa_mystic_snake_lua:IsDebuff() return true end
function modifier_boss_medusa_mystic_snake_lua:IsStunDebuff() return true end
function modifier_boss_medusa_mystic_snake_lua:IsPurgable() return true end
...
function boss_medusa_spawn_minion:OnSpellStart()
    ...
    for i = 1, unit_count do
        if my_ward_count < unit_limit then
            ...
            local ward = CreateUnitByName("npc_dota_boss_medusa_minion", spawn_pos, true, caster, caster, caster:GetTeamNumber())
...
function modifier_boss_medusa_minion_lua:IsHidden() return false end
function modifier_boss_medusa_minion_lua:IsPurgable() return false end
...
function modifier_boss_medusa_minion_lua_effect:OnCreated( kv )
    ...
    if IsServer() then
        self.nFXIndex = ParticleManager:CreateParticle( "particles/units/heroes/hero_pugna/pugna_life_drain.vpcf", PATTACH_CUSTOMORIGIN, self:GetParent() )
...
function boss_medusa_poison_nova_lua:OnSpellStart()
	...
	ring:SetCallback( function( enemy )
		enemy:AddNewModifier(
			caster,
			self,
			"modifier_boss_medusa_poison_nova_lua",
			{ duration = duration }
		)
...
function modifier_boss_medusa_poison_nova_lua:OnCreated( kv )
	self.parent = self:GetParent()
	self.resistance = self:GetAbility():GetSpecialValueFor( "resistance" ) + self:GetAbility():GetSpecialValueFor( "diff_boost_damage" )
	if not IsServer() then return end
