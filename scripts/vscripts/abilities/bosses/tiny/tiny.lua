...
function modifier_boss_tiny_shifting_quake_lua:IsDebuff() return true end
function modifier_boss_tiny_shifting_quake_lua:IsHidden() return false end
function modifier_boss_tiny_shifting_quake_lua:IsPurgable() return true end
...
function modifier_boss_tiny_shifting_quake_lua:OnIntervalThink()
    ...
    if not caster or not parent or not ability then return end
...
function boss_tiny_gavnina_lua:OnSpellStart()
    self.mod_caster = self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_boss_tiny_gavnina_lua", {duration = 3})
end
...
function modifier_boss_tiny_gavnina_lua:OnIntervalThink()
    ...
    CreateModifierThinker(self:GetCaster(), self:GetAbility(), "modifier_boss_tiny_gavnina_lua_thinker", {}, target_point, self:GetCaster():GetTeamNumber(), false)
...
function modifier_boss_tiny_gavnina_lua_thinker:Burn()
    ...
    local enemies = FindUnitsInRadius( self:GetCaster():GetTeamNumber(), self.parent_origin, nil, self.area_of_effect, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, 0, 0, false )
...
function boss_tiny_stone_wall_lua:OnSpellStart()
    ...
    for i = 1, count do
        ...
        local units = FindUnitsInRadius(caster:GetTeamNumber(), spawn_pos, nil, 120, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, 0, 0, false)
        for _, unit in pairs(units) do
...
function modifier_geocron_boulder_blocker:OnDestroy()
    ...
    local pos = self:GetParent():GetAbsOrigin()
    local units = FindUnitsInRadius(self:GetCaster():GetTeamNumber(), pos, nil, 120, DOTA_UNIT_TARGET_TEAM_BOTH, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, 0, 0, false)
    for _, unit in pairs(units) do
        FindClearSpaceForUnit(unit, unit:GetAbsOrigin(), true)
    end
end
...
function modifier_trembling_waves_passive:OnIntervalThink()
    ...
    local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_sandking/sandking_epicenter.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster)
