...
function boss_nyx_assassin_mana_burn:OnSpellStart()
    ...
    local particle_manaburn_fx = ParticleManager:CreateParticle("particles/units/heroes/hero_nyx_assassin/nyx_assassin_mana_burn.vpcf", PATTACH_ABSORIGIN_FOLLOW, target)
...
function boss_nyx_assassin_impale:OnProjectileHit_ExtraData(target, location, ExtraData)
	...
	local particle_impact_fx = ParticleManager:CreateParticle("particles/units/heroes/hero_nyx_assassin/nyx_assassin_impale_hit.vpcf", PATTACH_ABSORIGIN, target)
...
function modifier_boss_nyx_assassin_passive:IsHidden() return true end
function modifier_boss_nyx_assassin_passive:IsPurgable() return false end
...
function modifier_boss_nyx_assassin_passive:PlayEffects()
    local particle = ParticleManager:CreateParticle("particles/nyx_phisical.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
...
function modifier_boss_nyx_assassin_passive:PlayEffects2()
    local particle = ParticleManager:CreateParticle("particles/nyx_magical.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
    ParticleManager:SetParticleControl(particle, 1, Vector(150, 150, 150))
    self:AddParticle(particle, false, false, -1, false, false)
end
...
function boss_nyx_assassin_swap:PlayEffects(position)
    local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_vengeful/vengeful_nether_swap.vpcf", PATTACH_WORLDORIGIN, nil)
