...
function modifier_undy_spray_lua:OnCreated( kv )
	...
	self.armor = self:GetAbility():GetSpecialValueFor( "armor_reduction" ) + self:GetAbility():GetSpecialValueFor( "diff_boost_additional" ) 
...
function modifier_undy_skeletons_on_death_lua:IsHidden() return true end
...
function undying_str_steal_lua:OnSpellStart()
    ...
    for _, hero in pairs(heroes) do
        local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_undying/undying_soul_rip_damage.vpcf", PATTACH_ABSORIGIN_FOLLOW, hero)
    ...
    if total_str_stolen > 0 then
        ...
        local heal_pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_undying/undying_soul_rip_heal.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster)
...
function modifier_undying_str_steal_lua_debuff:IsHidden() return false end
function modifier_undying_str_steal_lua_debuff:IsDebuff() return true end
function modifier_undying_str_steal_lua_debuff:IsPurgable() return false end
