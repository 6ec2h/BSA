...
function boss_lich_frost_blast_lua:OnSpellStart()
	...
	for _,enemy in pairs(enemies) do
		...
		enemy:AddNewModifier(
			caster,
			self,
			"modifier_boss_lich_frost_blast_lua",
			{ duration = duration }
		)
	end
...
function boss_lich_chain_frost_lua:OnProjectileHit_ExtraData(target, location, extraData)
    if not target or target:IsInvulnerable() or target:IsOutOfGame() then return end
...
function modifier_boss_lich_chain_frost_lua:GetModifierMoveSpeedBonus_Percentage() return -self.ms_slow end
function modifier_boss_lich_chain_frost_lua:GetModifierAttackSpeedBonus_Constant() return -self.as_slow end
function modifier_boss_lich_chain_frost_lua:GetStatusEffectName() return "particles/status_fx/status_effect_frost_lich.vpcf" end
...
function modifier_boss_lich_ice_aura:IsHidden() return true end
function modifier_boss_lich_ice_aura:IsAura() return true end
...
function modifier_boss_lich_ice_aura_debuff:IsDebuff() return true end
function modifier_boss_lich_ice_aura_debuff:IsPurgable() return false end
    ...
    if distance <= min_dist then
        ...
        if self.counter >= duration then
            if not parent:HasModifier("modifier_boss_lich_ice_aura_debuff_timeout") then
                ...
                parent:AddNewModifier(caster, ability, "modifier_boss_lich_ice_aura_debuff_timeout", {duration = duration + 3})
                self.counter = 0
            end
        end
    else
...
function modifier_boss_lich_ice_aura_freeze:IsDebuff() return true end
function modifier_boss_lich_ice_aura_freeze:IsPurgable() return true end
...
function modifier_boss_lich_ice_aura_debuff_timeout:IsHidden() return false end
function modifier_boss_lich_ice_aura_debuff_timeout:IsDebuff() return false end
function modifier_boss_lich_ice_aura_debuff_timeout:IsPurgable() return false end
...
function modifier_boss_lich_pull_enemy:IsHidden() return false end
function modifier_boss_lich_pull_enemy:IsDebuff() return true end
function modifier_boss_lich_pull_enemy:IsStunDebuff() return true end
function modifier_boss_lich_pull_enemy:IsPurgable() return true end
