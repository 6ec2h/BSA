...
LinkLuaModifier( "modifier_boss_lich_chain_frost_lua", "abilities/bosses/lich/boss_lich_chain_frost_lua", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_boss_lich_chain_frost_lua_thinker", "abilities/bosses/lich/boss_lich_chain_frost_lua", LUA_MODIFIER_MOTION_NONE )
...
function boss_lich_chain_frost_lua:OnProjectileHit_ExtraData( target, location, kv )
	...
	target:AddNewModifier(
		...
		self, -- ability source
		"modifier_boss_lich_chain_frost_lua_thinker", -- modifier name
		{
			key = kv.key,
	...
	if (not target:IsMagicImmune()) and (not target:IsInvulnerable()) then
		...
		local damageTable = {
			...
			damage = math.min(target_hp/100*self:GetSpecialValueFor("damage"), target_hp*0.9),
		...
		target:AddNewModifier(
			self:GetCaster(), -- player source
			self, -- ability source
			"modifier_boss_lich_chain_frost_lua", -- modifier name
			{
...
function boss_lich_chain_frost_lua:PlayProjectile( info )
...
function boss_lich_chain_frost_lua:StopProjectile( kv )
...
function modifier_boss_lich_chain_frost_lua:IsHidden()
	return false
end

function modifier_boss_lich_chain_frost_lua:IsDebuff()
	return true
end

function modifier_boss_lich_chain_frost_lua:IsStunDebuff()
	return false
end

function modifier_boss_lich_chain_frost_lua:IsPurgable()
	return true
end

function modifier_boss_lich_chain_frost_lua:OnCreated( kv )
...
function modifier_boss_lich_chain_frost_lua:OnRefresh( kv )
	self.as_slow = self:GetAbility():GetSpecialValueFor( "slow_attack_speed" ) -- special value
	self.ms_slow = self:GetAbility():GetSpecialValueFor( "slow_movement_speed" ) -- special value	
end

function modifier_boss_lich_chain_frost_lua:OnDestroy( kv )
...
modifier_boss_lich_chain_frost_lua_thinker = class({})
--local tempTable = require( "util/tempTable" )

function modifier_boss_lich_chain_frost_lua_thinker:IsHidden()
	return false
end

function modifier_boss_lich_chain_frost_lua_thinker:IsPurgable()
	return false
end

function modifier_boss_lich_chain_frost_lua_thinker:RemoveOnDeath()
	return false
end

function modifier_boss_lich_chain_frost_lua_thinker:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end

function modifier_boss_lich_chain_frost_lua_thinker:OnCreated( kv )
	if IsServer() then
		self.key = kv.key
	end
end

function modifier_boss_lich_chain_frost_lua_thinker:OnRefresh( kv )
	
end

function modifier_boss_lich_chain_frost_lua_thinker:OnDestroy( kv )
	if IsServer() then
...
function modifier_boss_lich_chain_frost_lua_thinker:PlayProjectile( info )
	local tracker = info.Target:AddNewModifier(
		info.Source, -- player source
