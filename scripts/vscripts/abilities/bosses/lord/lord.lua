...
function boos_doom_pit_lua:OnSpellStart()    
	...
	if #hEnemies > 0 then
		for _, target in ipairs(hEnemies) do
			...
			CreateModifierThinker(caster, self, "modifier_boos_doom_pit_lua_thinker", { duration = duration }, point, caster:GetTeamNumber(), false)
...
function modifier_boos_doom_pit_lua_thinker:OnIntervalThink()
	...
	for _,enemy in pairs(enemies) do
		...
		if not modifier then
			enemy:AddNewModifier(self.caster, self:GetAbility(),  "modifier_boos_doom_pit_lua", { duration = self.duration })
		end
	end
end
...
function modifier_boos_doom_pit_lua:OnCreated( kv )
	...
	local damage = self:GetAbility():GetSpecialValueFor( "pit_damage" ) + self:GetAbility():GetSpecialValueFor( "diff_boost_damage" )
...
function modifier_boos_doom_pit_lua:OnRefresh( kv )
	
end

function modifier_boos_doom_pit_lua:OnRemoved()
end

function modifier_boos_doom_pit_lua:OnDestroy()
end
...
function modifier_boos_doom_pit_lua_cooldown:OnCreated( kv )
end

function modifier_boos_doom_pit_lua_cooldown:OnRefresh( kv )
end

function modifier_boos_doom_pit_lua_cooldown:OnRemoved()
end

function modifier_boos_doom_pit_lua_cooldown:OnDestroy()
end
...
function modifier_boos_doom_doom_lua:OnRemoved()
end
