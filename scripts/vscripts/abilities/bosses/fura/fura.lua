...
function boss_furion_custom_forest:OnSpellStart()
	...
	self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_boss_furion_custom_forest", { duration = duration } )
end
...
function modifier_boss_furion_custom_forest:OnIntervalThink()
	...
	Timers:CreateTimer(delay, function()
		...
		local particleIndex = ParticleManager:CreateParticle("particles/econ/items/treant_protector/treant_ti10_immortal_head/treant_ti10_immortal_overgrowth_cast.vpcf", PATTACH_WORLDORIGIN, nil)
...
local creeps = {"npc_dota_zone_8_unit_5", "npc_dota_zone_8_unit_3", "npc_dota_zone_8_unit_4", "npc_dota_zone_8_unit_2", "npc_dota_zone_8_unit_6"}
...
function modifier_custom_seed:OnCreated()
	...
	self.leech_damage		= self:GetAbility():GetSpecialValueFor("tick_damage") + self:GetAbility():GetSpecialValueFor("diff_boost_additional")
...
function boss_furion_vine_grab:LaunchRandomParticle(caster, start_pos, target_pos)
    ...
    local particle = ParticleManager:CreateParticle("particles/units/heroes/hero_enchantress/enchantress_enchant_slow_grass_long.vpcf", PATTACH_WORLDORIGIN, nil)
...
function boss_furion_vine_grab:MoveBack(target)
    if target and not target:IsMagicImmune() then
        ...
        local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_treant/treant_overgrowth_vines.vpcf", PATTACH_ABSORIGIN_FOLLOW, target)
