...
function hidden_snow_boss_ice_torrent:OnSpellStart()
    ...
    for _, hero in pairs(HeroList:GetAllHeroes()) do
        if hero:IsRealHero() and hero:GetTeamNumber() == team then
            ...
            if player then
                local fxIndex = ParticleManager:CreateParticleForPlayer("particles/econ/items/ancient_apparition/ancient_apparation_ti8/ancient_ice_vortex_ti8.vpcf", PATTACH_WORLDORIGIN, nil, player)
    ...
    Timers:CreateTimer(delay, function()
        ...
        local nFXIndex = ParticleManager:CreateParticle("particles/econ/items/crystal_maiden/crystal_maiden_cowl_of_ice/maiden_crystal_nova_cowlofice.vpcf", PATTACH_WORLDORIGIN, nil)
