...
function creep_sticky_napalm_lua:OnSpellStart()
    ...
    local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_batrider/batrider_stickynapalm_projectile.vpcf", PATTACH_WORLDORIGIN, nil)
...
function modifier_creep_sticky_napalm_lua:IsHidden() return false end
function modifier_creep_sticky_napalm_lua:IsPurgable() return true end
...
function modifier_creep_sticky_napalm_lua:OnCreated()
    ...
    self.mr_reduction = self:GetAbility():GetSpecialValueFor("mr_reduction") + self:GetAbility():GetSpecialValueFor("diff_boost_additional")
    self.turn_rate_pct = self:GetAbility():GetSpecialValueFor("turn_rate_pct") + self:GetAbility():GetSpecialValueFor("diff_boost_additional")
    self.move_speed = self:GetAbility():GetSpecialValueFor("move_speed") + self:GetAbility():GetSpecialValueFor("diff_boost_additional")
...
function modifier_creep_unit_walking_lua:IsHidden() return true end
function modifier_creep_unit_walking_lua:IsPurgable() return false end
...
function creep_flaming_lasso_lua:OnSpellStart()
    ...
    ApplyDamage({
        victim = target, attacker = caster, damage = base_damage,
...
function modifier_creep_flaming_lasso_lua:IsDebuff() return true end
function modifier_creep_flaming_lasso_lua:IsStunDebuff() return true end
function modifier_creep_flaming_lasso_lua:IsPurgable() return false end
...
function modifier_creep_flaming_lasso_lua:OnCreated()
    ...
    self.pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_batrider/batrider_flaming_lasso.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.parent)
...
function modifier_creep_flaming_lasso_self_lua:IsHidden() return false end
function modifier_creep_flaming_lasso_self_lua:IsPurgable() return false end
...
function creep_fireball_lua:OnSpellStart()
	...
	self.projectile_table.vVelocity    = (self:GetCursorPosition() - caster:GetAbsOrigin()):Normalized() * speed * Vector(1, 1, 0)
...
function modifier_creep_fireball_burn:OnCreated()
    if not IsServer() then return end
    local dps = self:GetAbility():GetSpecialValueFor("damage_per_second") + self:GetAbility():GetSpecialValueFor("diff_boost_damage")
...
function modifier_creep_fatal_bonds_lua:GetTexture() return "warlock_creep_fatal_bonds_lua" end

function modifier_creep_fatal_bonds_lua:IsHidden() return false end
function modifier_creep_fatal_bonds_lua:IsPurgable() return true end
function modifier_creep_fatal_bonds_lua:RemoveOnDeath() return true end
...
function modifier_creep_fatal_bonds_lua:OnCreated(keys)
	...
	if IsServer() then
		...
		self.damageShare = (self.ability:GetSpecialValueFor("damage_share_pct") + self:GetAbility():GetSpecialValueFor("diff_boost_damage")) / 100
...
if IsServer() then
	...
	function modifier_creep_fatal_bonds_lua:OnDestroy()
		...
		if parent:HasModifier("modifier_creep_fatal_bonds_lua") then return end
...
function creep_rain_of_chaos_lua:OnSpellStart()
	...
	Timers:CreateTimer(0.5, function()
		...
		local golem = CreateUnitByName("npc_zone_9_creep_2_minion", target_point, true, self:GetCaster(), self:GetCaster(), self:GetCaster():GetTeamNumber())
...
function modifier_creep_flaming_fists_lua:IsHidden() return true end
