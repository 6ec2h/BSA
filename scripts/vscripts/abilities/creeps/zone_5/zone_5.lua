...
function modifier_creep_corrosive_skin_lua_debuff:OnRemoved()
end

function modifier_creep_corrosive_skin_lua_debuff:OnDestroy()
end
...
function modifier_creep_stone_form_lua:OnTakeDamage(params)
    ...
    if self:GetParent():HasModifier("modifier_creep_stone_form_lua_active") then return end
    ...
    if health_pct < self:GetAbility():GetSpecialValueFor( "threshold_pct" )  then
        ...
        self:GetParent():AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_creep_stone_form_lua_active", { duration = duration })
...
function modifier_creep_stone_form_lua_active:OnCreated()
    ...
    self.hp_regen = self:GetAbility():GetSpecialValueFor( "hp_regen_pct" ) + self:GetAbility():GetSpecialValueFor( "diff_boost_damage" )
    self:StartIntervalThink(1)
end
...
LinkLuaModifier("modifier_creep_heartstopper_aura_lua_damage", "abilities/creeps/zone_5/zone_5", LUA_MODIFIER_MOTION_NONE)
...
function modifier_creep_reapers_scythe_stun:OnDestroy()
    ...
    if not target:IsAlive() or not ability then return end
