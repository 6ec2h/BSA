...
function modifier_zombie_ice_hit:IsHidden() return true end
function modifier_zombie_ice_hit:IsPurgable() return false end
...
function modifier_movespeed_slow:IsHidden() return false end
function modifier_movespeed_slow:IsDebuff() return true end
function modifier_movespeed_slow:IsPurgable() return true end
...
function fat_zombie_ultimate:Precache(context)
    PrecacheResource("particle", "particles/units/heroes/hero_life_stealer/life_stealer_infest_emerge_blood01_lv.vpcf", context)
...
function modifier_fat_zombie_passive:OnDeath(keys)
    ...
    local blood_pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_life_stealer/life_stealer_infest_emerge_blood01_lv.vpcf", PATTACH_ABSORIGIN, caster)
    ParticleManager:ReleaseParticleIndex(blood_pfx)
    caster:EmitSound("Hero_LifeStealer.Assimilate.Destroy")

    local radius = ability:GetSpecialValueFor("radius")
    local duration = ability:GetSpecialValueFor("duration")
    local base_damage = ability:GetSpecialValueFor("quill_base_damage") + ability:GetSpecialValueFor("diff_boost_damage")
    ...
    for _, enemy in pairs(enemies) do
        ...
        if mod then mod:IncrementStackCount() end
    end
end
...
function modifier_fat_zombie_quill_stack:OnRefresh()
    local ability = self:GetAbility()
    if not ability then return end
    self.base_armor_red = ability:GetSpecialValueFor("armor_reduction") + ability:GetSpecialValueFor("diff_boost_additional")
end
