...
function modifier_creep_forest_quest_lua:OnCreated( kv )
end
...
function modifier_creep_untouchable_lua_debuff:OnDestroy( kv )
...
function creep_enchant_lua:OnSpellStart()
    ...
    local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_enchantress/enchantress_enchant.vpcf", PATTACH_ABSORIGIN_FOLLOW, target)
...
function creep_land_mines_lua:OnSpellStart()
    ...
    local mine = CreateUnitByName("npc_zone_8_creep_land_mines", point, true, caster, caster:GetOwner(), caster:GetTeamNumber())
...
function modifier_creep_land_mines_lua_thinker:IsHidden() return true end
...
function modifier_creep_land_mines_lua_thinker:OnCreated()
    ...
    local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_techies/techies_land_mine.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.parent)
...
function creep_stasis_lua:OnSpellStart()
    ...
    local caster = self:GetCaster()
    local point = self:GetCursorPosition()
    local mine = CreateUnitByName("npc_zone_8_creep_stasis", point, true, caster, caster:GetOwner(), caster:GetTeamNumber())
...
function modifier_creep_stasis_lua_thinker:IsHidden() return true end
...
function modifier_creep_stasis_lua_thinker:Explode()
    ...
    local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_techies/techies_stasis_trap_explode.vpcf", PATTACH_WORLDORIGIN, nil)
    ParticleManager:SetParticleControl(pfx, 0, origin)
    ParticleManager:SetParticleControl(pfx, 2, Vector(self.radius, 1, 1))
    ParticleManager:ReleaseParticleIndex(pfx)
    ...
    for _, enemy in pairs(enemies) do
        if not enemy:HasFlyMovementCapability() then
            enemy:AddNewModifier(self.parent, self.ability, "modifier_creep_stasis_lua_effect", { duration = self.duration })
        end
    end
...
function modifier_creep_stasis_lua_effect:IsHidden() return false end

function modifier_creep_stasis_lua_effect:IsPurgable() return true end

function modifier_creep_stasis_lua_effect:IsDebuff() return true end
...
function modifier_creep_suicide_lua_passive:IsHidden() return true end
...
function modifier_creep_suicide_lua_passive:OnTakeDamage(keys)
    ...
    if keys.unit ~= parent or not self:GetAbility():IsCooldownReady() then return end
...
function modifier_creep_suicide_lua_movement:VerticalMotion( me, dt )
end
...
function modifier_creep_suicide_lua_movement:BlastOffLanded()
    ...
    local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_techies/techies_blast_off.vpcf", PATTACH_WORLDORIGIN, nil)
...
function modifier_creep_treant_seed_lua:OnIntervalThink()
    ...
    local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_treant/treant_leech_seed_damage_pulse.vpcf", PATTACH_ABSORIGIN_FOLLOW, parent)
    ParticleManager:ReleaseParticleIndex(pfx)
...
function creep_overgrowth_lua:OnSpellStart()
	...
	for _, enemy in pairs(overgrowth_primary_enemies) do
		enemy:Stop()
		enemy:AddNewModifier(self:GetCaster(), self, "modifier_creep_overgrowth_lua", {duration = self:GetSpecialValueFor("duration") * (1 - enemy:GetStatusResistance())})
	end
end
...
function modifier_treant_invis:IsHidden() return true end
function modifier_treant_invis:IsPermanent() return true end
...
function modifier_treant_invis_effect:IsHidden() return false end
function modifier_treant_invis_effect:IsPurgable() return false end
