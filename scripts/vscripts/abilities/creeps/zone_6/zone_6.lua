...
LinkLuaModifier("modifier_creep_blob_die_spawn_lua_effect", "abilities/creeps/zone_6/zone_6", LUA_MODIFIER_MOTION_VERTICAL)
	...
	if IsServer() then
		if keys.unit == self:GetParent() then
			...
			for i = 1, self.count do
				local unit = CreateUnitByName("npc_dota_zone_6_unit_2", self:GetParent():GetAbsOrigin()+ RandomVector( RandomInt( 150, 150 )), true, self:GetParent(), self:GetParent(), DOTA_TEAM_NEUTRALS)
...
function modifier_creep_blob_mana_burn_lua:IsHidden() return true end
...
function modifier_creep_blob_mana_burn_lua:OnAttackLanded(params)
    ...
    if params.attacker ~= parent or not target or target:IsNull() or not target:IsAlive() then return end
    if target:IsMagicImmune() or target:IsInvulnerable() or target:IsBuilding() then return end
    
    local target_mana = target:GetMana()
    if not target_mana or target:GetMaxMana() <= 0 then return end
...
function modifier_creep_waveform_lua:IsHidden() return true end
function modifier_creep_waveform_lua:IsPurgable() return false end
function modifier_creep_waveform_lua:GetAttributes() return MODIFIER_ATTRIBUTE_PERMANENT end
...
function modifier_creep_waveform_lua:OnCreated(kv)
    ...
    self.effect_cast = ParticleManager:CreateParticle("particles/units/heroes/hero_morphling/morphling_waveform.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.parent)
...
function creep_torrent_lua:OnSpellStart()
    ...
    local fx_pre = ParticleManager:CreateParticle("particles/econ/items/kunkka/divine_anchor/hero_kunkka_dafx_skills/kunkka_spell_torrent_bubbles_fxset.vpcf", PATTACH_CUSTOMORIGIN, nil)
    ...
    Timers:CreateTimer(delay, function()
        ...
        local fx_main = ParticleManager:CreateParticle("particles/econ/items/kunkka/divine_anchor/hero_kunkka_dafx_skills/kunkka_spell_torrent_splash_fxset.vpcf", PATTACH_CUSTOMORIGIN, nil)
...
function modifier_creep_undercurrent_passive:IsHidden() return false end
...
function modifier_creep_undercurrent_passive:IsAura() return true end
function modifier_creep_undercurrent_passive:GetModifierAura() return "modifier_creep_undercurrent_aura_debuff" end
function modifier_creep_undercurrent_passive:GetAuraRadius() return self.radius end
function modifier_creep_undercurrent_passive:GetAuraSearchTeam() return DOTA_UNIT_TARGET_TEAM_ENEMY end
function modifier_creep_undercurrent_passive:GetAuraSearchType() return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC end
