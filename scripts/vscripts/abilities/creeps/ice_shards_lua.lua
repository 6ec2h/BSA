LinkLuaModifier("modifier_tusk_ice_shards_dummy", "abilities/creeps/ice_shards_lua",LUA_MODIFIER_MOTION_NONE)

ice_shards_lua = class({})

function ice_shards_lua:OnSpellStart()
    local caster = self:GetCaster()
    local point = self:GetCursorPosition()
	
	local caster_pos = self:GetCaster():GetAbsOrigin()
	local line_pos = caster_pos + self:GetCaster():GetForwardVector() * 500
...
function ice_shards_lua:CreateShard(point)	
	local caster = self:GetCaster()
...
function ice_shards_lua:OnProjectileThink(vLocation)
end

function ice_shards_lua:OnProjectileHit(hTarget, vLocation)
    if hTarget then
        local damage_table = {
            victim = hTarget,
