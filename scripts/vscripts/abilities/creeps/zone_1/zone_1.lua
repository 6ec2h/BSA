...
function creep_earthbind_lua:OnProjectileHit_ExtraData( target, location, data )
    ...
    if target:IsMagicImmune() or target:TriggerSpellAbsorb( self ) then return end
...
function creep_mana_burn:OnSpellStart()
    ...
    local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_nyx_assassin/nyx_assassin_mana_burn.vpcf", PATTACH_ABSORIGIN_FOLLOW, target)
...
function creep_thunder_clap:OnSpellStart()
    ...
    local pfx = ParticleManager:CreateParticle("particles/neutral_fx/ursa_thunderclap.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster)
...
function modifier_creep_thunder_clap_debuff:IsHidden() return false end
function modifier_creep_thunder_clap_debuff:IsDebuff() return true end
function modifier_creep_thunder_clap_debuff:IsPurgable() return true end
...
function creep_howl_lua:OnSpellStart()
    ...
    local cast_pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_lycan/lycan_howl_cast.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster)
...
function modifier_creep_howl_lua_debuff:IsHidden() return false end
function modifier_creep_howl_lua_debuff:IsDebuff() return true end
function modifier_creep_howl_lua_debuff:IsPurgable() return true end
...
function modifier_creep_howl_lua_debuff:OnCreated(kv)
    ...
    self.total_armor_reduction = self:GetAbility():GetSpecialValueFor("armor") + self:GetAbility():GetSpecialValueFor("diff_boost_damage")
end
...
function modifier_creep_critical_strike_passive:OnAttackLanded(keys)
    if not IsServer() then return end
    if keys.attacker ~= self:GetParent() or keys.attacker:IsIllusion() or keys.target:IsBuilding() then return end
    ...
    if RollPercentage(chance) then
        keys.target:AddNewModifier(keys.attacker, ability, "modifier_creep_critical_strike_effect", { duration = duration })
    end
end
...
function modifier_creep_critical_strike_effect:IsHidden() return false end
function modifier_creep_critical_strike_effect:IsDebuff() return true end
function modifier_creep_critical_strike_effect:IsPurgable() return true end
