LinkLuaModifier("modifier_phoenix_nova_egg_thinker", "abilities/creeps/phoenix_nova", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_phoenix_nova_dmg", "abilities/creeps/phoenix_nova", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_phoenix_nova_dummy", "abilities/creeps/phoenix_nova", LUA_MODIFIER_MOTION_NONE)

phoenix_nova = class({})

function phoenix_nova:OnSpellStart()
	...
	local flDist = ( caster_pos - target_pos ):Length2D()
	
	
	
	local ellipseCenter	= caster_pos + forwardDir * ( flDist )
	
	local dummy_modifier = "modifier_phoenix_nova_dummy" -- This is used to determain if dive can countinue
	caster:AddNewModifier(caster, self, dummy_modifier, { duration = 1 })
	...
	caster:SetContextThink( DoUniqueString("updateIcarusDive"), function ( )
		...
		if not caster:HasModifier( dummy_modifier ) then
			...
			for i = 1, 2 do
				...
				egg:AddNewModifier(egg, self, "modifier_phoenix_nova_egg_thinker", {duration = egg_duration + 0.3 })
		...
		local x =  math.sin( theta ) * 500 * 0.5
		local y = -math.cos( theta ) * flDist
...
modifier_phoenix_nova_dummy = class({})

function modifier_phoenix_nova_dummy:IsHidden() return true end
function modifier_phoenix_nova_dummy:IsPurgable() return false end

----------------------------------------------------------------------------------------------------------

modifier_phoenix_nova_egg_thinker = class({})

function modifier_phoenix_nova_egg_thinker:IsHidden() return true end
function modifier_phoenix_nova_egg_thinker:IsPurgable() return false end

function modifier_phoenix_nova_egg_thinker:DeclareFunctions()
	return {
...
function modifier_phoenix_nova_egg_thinker:GetModifierIncomingDamage_Percentage()
	return -100
end

function modifier_phoenix_nova_egg_thinker:OnCreated()
...
function modifier_phoenix_nova_egg_thinker:OnDestroy()
	if not IsServer() then return end
	if self.death then 
...
function modifier_phoenix_nova_egg_thinker:OnIntervalThink()
	if not IsServer() then return end
