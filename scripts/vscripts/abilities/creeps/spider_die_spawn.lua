...
LinkLuaModifier("modifier_spider_die_spawn", "abilities/creeps/spider_die_spawn", LUA_MODIFIER_MOTION_VERTICAL)
LinkLuaModifier("modifier_spider_die_spawn_effect", "abilities/creeps/spider_die_spawn", LUA_MODIFIER_MOTION_VERTICAL)
	...
	if IsServer() then
		if keys.unit == self:GetParent() then
			...
			Timers:CreateTimer(1.1, function()
				...
				for i = 1, 3 do
					local unit = CreateUnitByName("icespider_mini", self:GetParent():GetAbsOrigin()+ RandomVector( RandomInt( 150, 150 )), true, nil, nil, DOTA_TEAM_NEUTRALS)
