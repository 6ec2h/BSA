...
function modifier_creep_walrus_punch_lua_logic:GetModifierPreAttack_BonusDamage()
    if self.punch_attack then
        return self:GetAbility():GetSpecialValueFor("bonus_damage") + self:GetAbility():GetSpecialValueFor("diff_boost_damage")
    end
    return 0
end
...
function modifier_creep_walrus_punch_lua_logic:OnAttackLanded(keys)
    ...
    if self.punch_attack then
        ...
        if target and target:IsAlive() then
            ...
            local pfx = ParticleManager:CreateParticle("particles/econ/items/tuskarr/tusk_ti9_immortal/tusk_ti9_walruspunch_start.vpcf", PATTACH_ABSORIGIN, target)
...
function modifier_creep_walrus_punch_lua_airtime:IsStunDebuff() return true end
function modifier_creep_walrus_punch_lua_airtime:IsPurgable() return true end
...
function modifier_creep_tag_team_lua:IsHidden() return false end
...
function modifier_creep_tag_team_lua_debuff:IsHidden() return false end
...
function modifier_creep_tag_team_lua_debuff:OnAttackLanded(keys)
    ...
    if attacker:GetTeamNumber() == ability:GetCaster():GetTeamNumber() then
        ...
        target:AddNewModifier(ability:GetCaster(), ability, "modifier_creep_tag_team_lua_slow", {duration = slow_duration})
    end
end
...
function creep_crystal_nova_lua:OnSpellStart()
	...
	local enemies = FindUnitsInRadius(self:GetCaster():GetTeamNumber(), point, nil, radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_ALL, 0, 0, false)
...
function modifier_creep_crystal_nova_lua:OnDestroy( kv )
end
...
function creep_freezing_field_lua:OnSpellStart()
	local caster = self:GetCaster()
	self.modifier = caster:AddNewModifier(caster, self, "modifier_creep_freezing_field_lua", { duration = self:GetChannelTime() })
end
...
function modifier_creep_freezing_field_lua:OnCreated( kv )
	...
	self.explosion_damage = self:GetAbility():GetSpecialValueFor( "damage" ) + self:GetAbility():GetSpecialValueFor( "diff_boost_damage" )
...
function modifier_creep_ice_vortex_lua_thinker:IsHidden() return true end
...
function modifier_creep_ice_vortex_lua_thinker:OnCreated()
    ...
    local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_ancient_apparition/ancient_ice_vortex.vpcf", PATTACH_WORLDORIGIN, nil)
...
function modifier_creep_ice_vortex_lua_thinker:IsAura() return true end
function modifier_creep_ice_vortex_lua_thinker:GetAuraRadius() return self.radius end
function modifier_creep_ice_vortex_lua_thinker:GetAuraSearchTeam() return DOTA_UNIT_TARGET_TEAM_BOTH end
function modifier_creep_ice_vortex_lua_thinker:GetAuraSearchType() return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC end
function modifier_creep_ice_vortex_lua_thinker:GetModifierAura() return "modifier_creep_ice_vortex_lua_debuff" end
