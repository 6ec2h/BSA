...
function creep_rocket_barrage_lua:OnSpellStart()
	...
	self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_creep_rocket_barrage_lua", {duration = self:GetDuration()})
end
...
function modifier_creep_rocket_barrage_lua:OnCreated()
	...
	self.rocket_damage	= self:GetAbility():GetSpecialValueFor("rocket_damage") + self:GetAbility():GetSpecialValueFor("diff_boost_damage")
...
function creep_homing_missile_lua:OnProjectileHit(target, location)
    ...
    local explosion_pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_gyrocopter/gyro_homing_missile_explosion.vpcf", PATTACH_WORLDORIGIN, nil)
...
function creep_assassinate_lua:OnProjectileHit(target, location)
    ...
    local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_sniper/sniper_assassinate_impact_blood.vpcf", PATTACH_ABSORIGIN_FOLLOW, target)
...
function modifier_creep_sunstrike_lua:OnIntervalThink()
    ...
    for _, target in ipairs(hEnemies) do
        ...
        local startFX = ParticleManager:CreateParticle("particles/units/heroes/hero_invoker/invoker_sun_strike_team.vpcf", PATTACH_WORLDORIGIN, nil)
...
function modifier_creep_meteor_lua_thinker:PerformTick()
    ...
    for _, enemy in pairs(enemies) do
        ...
        enemy:AddNewModifier(self.caster, self.ability, "modifier_creep_meteor_lua_burn", { duration = self.burn_duration })
    end
...
function modifier_creep_meteor_lua_thinker:PlayEffects1()
    local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_invoker/invoker_chaos_meteor_fly.vpcf", PATTACH_WORLDORIGIN, nil)
...
function modifier_creep_meteor_lua_thinker:PlayEffects2()
    local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_invoker/invoker_chaos_meteor.vpcf", PATTACH_WORLDORIGIN, nil)
...
function modifier_creep_meteor_lua_burn:OnCreated()
    if not IsServer() then return end
    self.burn_dps = self:GetAbility():GetSpecialValueFor("burn_dps") + self:GetAbility():GetSpecialValueFor("diff_boost_damage")
...
function creep_echo_stomp_lua:OnSpellStart()
    ...
    if #enemies > 0 then
        ...
        self.combined_particle = ParticleManager:CreateParticle("particles/creep_echo_stomp_lua.vpcf", PATTACH_ABSORIGIN_FOLLOW, dummy)
        Timers:CreateTimer(1.4, function()
            ...
            local particle_stomp_fx = ParticleManager:CreateParticle("particles/units/heroes/hero_elder_titan/elder_titan_echo_stomp_physical.vpcf", PATTACH_ABSORIGIN, dummy)
...
function modifier_creep_echo_stomp_sleep:IsHidden() return false end
function modifier_creep_echo_stomp_sleep:IsPurgable() return true end
...
function modifier_creep_echo_stomp_sleep:OnCreated()
    ...
    self.pfx = ParticleManager:CreateParticle("particles/generic_gameplay/generic_sleep.vpcf", PATTACH_OVERHEAD_FOLLOW, self:GetParent())
end
