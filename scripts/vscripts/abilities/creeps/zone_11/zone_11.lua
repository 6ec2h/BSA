...
LinkLuaModifier("modifier_creep_black_hole_lua_pull", "abilities/creeps/zone_11/zone_11", LUA_MODIFIER_MOTION_HORIZONTAL)
...
function creep_black_hole_lua:OnSpellStart()
    ...
    local blink_start_fx = ParticleManager:CreateParticle("particles/items_fx/blink_dagger_start.vpcf", PATTACH_ABSORIGIN, caster)
    ...
    local blink_end_fx = ParticleManager:CreateParticle("particles/items_fx/blink_dagger_end.vpcf", PATTACH_ABSORIGIN, caster)
...
function modifier_creep_black_hole_lua_thinker:IsAura() return true end
function modifier_creep_black_hole_lua_thinker:GetModifierAura() return "modifier_creep_black_hole_lua_pull" end
function modifier_creep_black_hole_lua_thinker:GetAuraRadius() return self.radius end
function modifier_creep_black_hole_lua_thinker:GetAuraSearchTeam() return DOTA_UNIT_TARGET_TEAM_ENEMY end
function modifier_creep_black_hole_lua_thinker:GetAuraSearchType() return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC end
function modifier_creep_black_hole_lua_thinker:GetAuraSearchFlags() return DOTA_UNIT_TARGET_FLAG_NONE end

--------------------------------------------------------------------------------

modifier_creep_black_hole_lua_pull = class({})

function modifier_creep_black_hole_lua_pull:IsHidden() return false end
function modifier_creep_black_hole_lua_pull:IsStunDebuff() return true end
...
function creep_sun_nova_lua:TriggerDive()
    ...
    if distance <= 0 then distance = 900 end
    local speed = 800
...
function creep_sun_nova_lua:SpawnEgg(position)
    ...
    egg:AddNewModifier(self:GetCaster(), self, "modifier_creep_sun_nova_lua_sun", {duration = self:GetSpecialValueFor("duration_sun")})
    self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_creep_sun_nova_lua_reborn", {duration = self:GetSpecialValueFor("duration_sun")})
end
...
if IsServer() then
    function modifier_creep_sun_nova_lua_passive:OnTakeDamage(params)
        ...
        if parent:IsAlive() and ability:IsCooldownReady() and parent:GetHealthPercent() <= ability:GetSpecialValueFor("hp_start") then
...
function modifier_creep_sun_nova_lua_move:IsHidden() return true end
function modifier_creep_sun_nova_lua_move:IsPurgable() return false end
function modifier_creep_sun_nova_lua_move:GetAttributes() return MODIFIER_ATTRIBUTE_PERMANENT end
...
function modifier_creep_sun_nova_lua_move:OnCreated(kv)
    ...
    self.pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_phoenix/phoenix_icarus_dive.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.parent)
...
function modifier_creep_sun_nova_lua_move:UpdateHorizontalMotion(me, dt)
    ...
    local ahead_pos = self.start_pos + (self.direction * (look_ahead * self.total_dist)) - (self.right_vec * math.sin(math.pi * look_ahead) * (self.total_dist * 0.35))
...
function modifier_creep_sun_nova_lua_sun:IsHidden() return true end
function modifier_creep_sun_nova_lua_sun:IsPurgable() return false end
...
function modifier_creep_sun_nova_lua_sun:OnCreated()
	...
	self.damage_per_sec	= self:GetAbility():GetSpecialValueFor("damage_per_sec") + self:GetAbility():GetSpecialValueFor("diff_boost_damage")
...
function creep_telekines_lua:IsHiddenWhenStolen() return false end
function creep_telekines_lua:IsRefreshable() return true end
function creep_telekines_lua:IsStealable() return true end
function creep_telekines_lua:IsNetherWardStealable() return true end
...
function creep_telekines_lua:OnSpellStart( params )
	...
	self.target:AddNewModifier(caster, self, "modifier_creep_telekines_lua_root", { duration = 3 * (1 - self.target:GetStatusResistance())})
	self.target_modifier = self.target:AddNewModifier(caster, self, "modifier_creep_telekines_lua", { duration = 3 * (1 - self.target:GetStatusResistance())})
...
function modifier_creep_telekines_lua:IsHidden() return false end
function modifier_creep_telekines_lua:IsPurgable() return false end
function modifier_creep_telekines_lua:IsPurgeException() return false end
function modifier_creep_telekines_lua:IsStunDebuff() return false end
function modifier_creep_telekines_lua:IgnoreTenacity() return true end
function modifier_creep_telekines_lua:IsMotionController() return true end
function modifier_creep_telekines_lua:GetMotionControllerPriority() return DOTA_MOTION_CONTROLLER_PRIORITY_MEDIUM end
...
function modifier_creep_telekines_lua_stun:IsDebuff() return true end
function modifier_creep_telekines_lua_stun:IsHidden() return false end
function modifier_creep_telekines_lua_stun:IsPurgable() return false end
function modifier_creep_telekines_lua_stun:IsPurgeException() return false end
function modifier_creep_telekines_lua_stun:IsStunDebuff() return true end
...
function modifier_creep_telekines_lua_root:IsDebuff() return false end
function modifier_creep_telekines_lua_root:IsHidden() return true end
function modifier_creep_telekines_lua_root:IsPurgable() return false end
function modifier_creep_telekines_lua_root:IsPurgeException() return false end
...
if IsServer() then
	ApplyDamage( {
		...
		attacker = self:GetCaster(),
		damage = (self:GetAbility():GetSpecialValueFor("damage") + self:GetAbility():GetSpecialValueFor("diff_boost_damage"))/5,
		damage_type	= DAMAGE_TYPE_PURE,
...
function modifier_creep_thunder_strike_lua:IsHidden() return false end
function modifier_creep_thunder_strike_lua:IsDebuff() return true end
function modifier_creep_thunder_strike_lua:IsPurgable() return true end
function modifier_creep_thunder_strike_lua:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE end
...
function modifier_creep_thunder_strike_lua:OnIntervalThink()
	...
	self.strike_particle_fx = ParticleManager:CreateParticle("particles/units/heroes/hero_disruptor/disruptor_thunder_strike_bolt.vpcf", PATTACH_ABSORIGIN, parent)
...
LinkLuaModifier("modifier_creep_glimpse_to_caster_lua", "abilities/creeps/zone_11/zone_11", LUA_MODIFIER_MOTION_HORIZONTAL)
...
function creep_glimpse_to_caster_lua:OnSpellStart()
    ...
    local nFXIndex2 = ParticleManager:CreateParticle("particles/units/heroes/hero_disruptor/disruptor_glimpse_targetend.vpcf", PATTACH_WORLDORIGIN, nil)
...
function modifier_creep_static_storm_lua_thinker:IsAura() return true end
function modifier_creep_static_storm_lua_thinker:GetModifierAura() return "modifier_creep_static_storm_lua_debuff" end
function modifier_creep_static_storm_lua_thinker:GetAuraRadius() return self.radius end
function modifier_creep_static_storm_lua_thinker:GetAuraSearchTeam() return DOTA_UNIT_TARGET_TEAM_ENEMY end
function modifier_creep_static_storm_lua_thinker:GetAuraSearchType() return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC end
function modifier_creep_static_storm_lua_thinker:GetAuraSearchFlags() return DOTA_UNIT_TARGET_FLAG_NONE end

--------------------------------------------------------------------------------

modifier_creep_static_storm_lua_debuff = class({})

function modifier_creep_static_storm_lua_debuff:IsHidden() return false end
function modifier_creep_static_storm_lua_debuff:IsPurgable() return false end
