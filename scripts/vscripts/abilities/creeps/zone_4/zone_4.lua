...
function modifier_creep_arrow_lua_stun:IsStunDebuff() return true end
function modifier_creep_arrow_lua_stun:IsPurgeException() return true end
function modifier_creep_arrow_lua_stun:CheckState() return { [MODIFIER_STATE_STUNNED] = true } end
function modifier_creep_arrow_lua_stun:GetEffectName() return "particles/generic_gameplay/generic_stunned.vpcf" end
function modifier_creep_arrow_lua_stun:GetEffectAttachType() return PATTACH_OVERHEAD_FOLLOW end
...
function modifier_creep_dispersion_lua:OnTakeDamage(params)
    ...
    if params.unit ~= self.parent or params.damage <= 10 then return end
    if bit.band(params.damage_flags, DOTA_DAMAGE_FLAG_REFLECTION) == DOTA_DAMAGE_FLAG_REFLECTION then return end
    if params.attacker == self.parent or not params.attacker then return end
    ...
    for _, enemy in pairs(enemies) do
        ...
        if final_reflected_damage > 0 then
            ...
            if bPlayParticles then
                local pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_spectre/spectre_dispersion.vpcf", PATTACH_POINT_FOLLOW, self.parent)
                ParticleManager:SetParticleControlEnt(pfx, 0, self.parent, PATTACH_POINT_FOLLOW, "attach_hitloc", origin, true)
                ParticleManager:SetParticleControlEnt(pfx, 1, enemy, PATTACH_POINT_FOLLOW, "attach_hitloc", enemy_pos, true)
...
LinkLuaModifier("modifier_creep_spider_die_spawn_lua_effect", "abilities/creeps/zone_4/zone_4", LUA_MODIFIER_MOTION_VERTICAL)
		...
		if keys.unit == self:GetParent() then
			...
			Timers:CreateTimer(self.explosion_delay, function()
				...
				local enemies = FindUnitsInRadius(self:GetParent():GetTeamNumber(), self:GetParent():GetOrigin(), nil, 400, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_CREEP, DOTA_UNIT_TARGET_FLAG_NONE, FIND_ANY_ORDER, false)
					for _, enemy in pairs(enemies) do			
						enemy:AddNewModifier(enemy, self:GetAbility(), "modifier_creep_spider_die_spawn_lua_effect", { duration = self.debuff_duration})
					end
				for i = 1, self.spawn_count do
					local unit = CreateUnitByName("npc_dota_zone_4_unit_4", self:GetParent():GetAbsOrigin()+ RandomVector( RandomInt( 150, 150 )), true, self:GetParent(), self:GetParent(), self:GetParent():GetTeamNumber())
...
function modifier_creep_spider_die_spawn_lua:IsHidden() return false end				
function modifier_creep_spider_die_spawn_lua_effect:IsDebuff() return true end
function modifier_creep_spider_die_spawn_lua_effect:IsPurgable() return false end
...
LinkLuaModifier("modifier_creep_snowball_lua_movement", "abilities/creeps/zone_4/zone_4", LUA_MODIFIER_MOTION_HORIZONTAL)
...
function modifier_creep_snowball_lua_movement:IsHidden() return true end
function modifier_creep_snowball_lua_movement:IsPurgable() return false end
function modifier_creep_snowball_lua_movement:GetPriority() return MODIFIER_PRIORITY_HIGH end
...
function modifier_creep_snowball_lua_movement:OnCreated(params)
    ...
    self.damage = self:GetAbility():GetSpecialValueFor("snowball_damage") + self:GetAbility():GetSpecialValueFor("diff_boost_damage")

    self.effect_cast = ParticleManager:CreateParticle("particles/gavnina/snow_ball.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.parent)
...
function modifier_creep_snowball_lua_movement:Hit(target)
    ...
    target:AddNewModifier(self.caster, self:GetAbility(), "modifier_stunned", {duration = self:GetAbility():GetSpecialValueFor("stun_duration")})
end
