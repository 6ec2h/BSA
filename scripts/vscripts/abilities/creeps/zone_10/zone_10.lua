...
function creep_dismember_lua:OnSpellStart()
	if self:GetCaster():IsAlive() then
		...
		self.target:AddNewModifier(self:GetCaster(), self, "modifier_creep_dismember_lua", {duration = self:GetChannelTime() - FrameTime()})
...
function creep_dismember_lua:OnChannelFinish(bInterrupted)
	...
	local caster_buff = self:GetCaster():FindModifierByNameAndCaster("modifier_creep_dismember_lua_buff", self:GetCaster())
...
function modifier_creep_dismember_lua:IgnoreTenacity()	return true end
function modifier_creep_dismember_lua:IsDebuff() return true end
function modifier_creep_dismember_lua:IsHidden() return false end

function modifier_creep_dismember_lua:OnCreated()
	self.dismember_damage = self:GetAbility():GetSpecialValueFor("damage") + self:GetAbility():GetSpecialValueFor("diff_boost_damage")
	...
	if IsServer() then
		...
		self:GetParent():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_creep_dismember_lua_pull", {duration = self:GetAbility():GetChannelTime() - FrameTime()})
	end
end
...
function modifier_creep_dismember_lua_buff:IsDebuff() return false end
function modifier_creep_dismember_lua_buff:IsHidden() return true end
function modifier_creep_dismember_lua_buff:IsPurgable() return false end
function modifier_creep_dismember_lua_buff:IsStunDebuff() return false end
...
function modifier_creep_rot_lua:IsHidden() return false end
function modifier_creep_rot_lua:IsPurgable() return false end
...
function modifier_creep_rot_lua_effect:IsHidden() return false end
function modifier_creep_rot_lua_effect:IsDebuff() return true end
function modifier_creep_rot_lua_effect:IsPurgable() return true end
...
function modifier_creep_spider_egg_lua:OnIntervalThink()
	if IsServer() then
		local enemies = _G.OldFindUnitsInRadius( self:GetParent():GetTeamNumber(), self:GetParent():GetOrigin(), self:GetParent(), self.trigger_radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, 0, false )
		if #enemies > 0 then
			self:Burst()
...
function modifier_creep_spider_egg_lua:Burst()
	if IsServer() then
		...
		for i=0, RandomInt( self.spider_min, self.spider_max ) do
			local hUnit = CreateUnitByName( "npc_zone_10_creep_2_minion", self:GetParent():GetOrigin(), true, nil, nil, self:GetParent():GetTeamNumber())
...
function modifier_creep_nightmare_lua:IsHidden() return false end
function modifier_creep_nightmare_lua:IsDebuff() return true end
function modifier_creep_nightmare_lua:IsPurgable() return true end
...
function modifier_creep_gale_lua:OnCreated( kv )
	...
	self.init_damage = self:GetAbility():GetSpecialValueFor( "strike_damage" ) + self:GetAbility():GetSpecialValueFor( "diff_boost_damage" )
...
function creep_summon_eggs_lua:OnSpellStart()
	if IsServer() then
		...
		for i = 1, nEggSpawns do
			local hEgg = CreateUnitByName("npc_dota_zone_10_unit_2", self:GetCaster():GetAbsOrigin(), true, self:GetCaster(), self:GetCaster(), self:GetCaster():GetTeamNumber() )
...
function modifier_creep_spider_spray_lua:OnCreated()
	...
	self.radius = self:GetAbility():GetSpecialValueFor("radius")
	self.stack_damage = self:GetAbility():GetSpecialValueFor("stack_damage") + self:GetAbility():GetSpecialValueFor("diff_boost_damage")
