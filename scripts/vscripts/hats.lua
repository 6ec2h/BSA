...
function CBaseEntity:SetupHat(hatType)
	...
	if not hatParticleTemplate then return end

	local unitName = self:GetUnitName()
	unitName = BOSS_NAME_TO_HERO_NAME[unitName] or unitName

	if buggedHats[unitName] then return end

	local hatParticle = ParticleManager:CreateParticle(string.format(hatParticleTemplate, unitName), PATTACH_CUSTOMORIGIN, self)
	if not hatParticle then return end
...
function CBaseEntity:RemoveHat()
	if not self.hatParticle then return end
